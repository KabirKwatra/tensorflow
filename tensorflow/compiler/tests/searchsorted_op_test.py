...
class SearchSorteddOpTest(xla_test.XLATestCase):
    def test1D(self):
        ...
        for side in ["left", "right"]:
            for dtype in [np.float32, np.int32]:
                values = np.random.uniform(low=-1000, high=1000,
                                           size=(10, )).astype(dtype)
                unsorted = np.random.uniform(low=-1000, high=1000,
                                             size=(20, )).astype(dtype)
                ...
                with self.session() as session:
                    with self.test_scope():
                        tf_ans = array_ops.searchsorted(sorted_sequence,
                                                        values,
                                                        side=side)
                    tf_out = session.run(tf_ans)
                    self.assertAllEqual(np_ans, tf_out)

    def _test2DExample(self, dtype, side, sorted_sequence, values,
                       correct_ans):
    ...
    def testLowerBound2DExample(self):
        ...
        for dtype in self.float_types | self.int_types:
            sorted_sequence = np.array([[0, 3, 9, 9, 10], [1, 2, 3, 4, 5]],
                                       dtype)
            values = np.array([[2, 4, 9], [0, 2, 6]], dtype)
            correct_ans = np.array([[1, 2, 2], [0, 1, 5]], dtype)
            self._test2DExample(dtype, "left", sorted_sequence, values,
                                correct_ans)
    ...
    def testUpperBound2DExample(self):
        ...
        for dtype in self.float_types | self.int_types:
            ...
            self._test2DExample(dtype, "right", sorted_sequence, values,
                                correct_ans)
