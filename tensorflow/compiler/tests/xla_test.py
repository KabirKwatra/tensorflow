...
class XLATestCase(test.TestCase):
    ...
    def float_tf_types(self):
        name = "{}.{}".format(type(self).__name__, self._testMethodName)
        return self._float_tf_types - self._method_types_filter.get(
            name, set())
    ...
    def session(self):
        ...
        with session.Session(graph=graph,
                             config=config) as sess, graph.as_default():
            yield sess
...
def Benchmark(tf_bench,
    ...
    with ops.Graph().as_default():
        ...
        with session.Session(config=config) as sess:
            ...
            tf_bench.run_op_benchmark(sess,
                                      targets,
                                      name="%s_%s%s" % (name, xla, device))
