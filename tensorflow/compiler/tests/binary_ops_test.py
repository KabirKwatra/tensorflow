...
class BinaryOpsTest(xla_test.XLATestCase):
        ...
        with self.session() as session:
            with self.test_scope():
                pa = array_ops.placeholder(
                    dtypes.as_dtype(a.dtype), a.shape, name="a")
                pb = array_ops.placeholder(
                    dtypes.as_dtype(b.dtype), b.shape, name="b")
                output = op(pa, pb)
    ...
    def testFloatOps(self):
        for dtype in self.float_types:
            ...
            self._testBinary(
                lambda x, y: math_ops.approximate_equal(
            ...
            if dtype != dtypes.bfloat16.as_numpy_dtype:
                self._testBinary(
                    ...
                    expected=np.array(
                        [0, 0.8, 0.5, 0.285714, 0.125, 0], dtype=dtype),
                    rtol=1e-6,
                    atol=1e-6)
            ...
            if dtype not in self.int_types:
                self._testBinary(
                    ...
                    np.array([1.9131952969218875, 1.596299504298079],
                             dtype=dtype),
                    np.array(
                        [1.1137667913355869, 1.7186636469261405], dtype=dtype),
    ...
    def testComplexOps(self):
        for dtype in self.complex_types:
            ...
            rhs = np.array([2, 3j, 3 + 4j, 2 + 3j, 3 -
                            2j, 2, 3 + 3j], dtype=dtype)
            scalar = dtype(2 + 2j)
            self._testBinary(math_ops.pow, lhs, rhs,
    ...
    def testS64Comparisons(self):
        for op in [(lambda x, y: x < y), (lambda x, y: x <= y),
            ...
            rhs = np.array(
                ...
                dtype=np.int64)
            expected = np.array([op(l, r)
                                 for l, r in zip(lhs, rhs)], dtype=np.bool)
        ...
        for dtype in self.float_types | {np.float64}:
            ...
            if dtype != np.float16 and hasattr(np, "matmul"):
                ...
                if (test_lib.is_built_with_rocm() and
                    ...
                    return
                x = np.arange(0, 3 * 5 * 2 * 7,
                              dtype=dtype).reshape((3, 5, 2, 7))
                self._testBinary(
    ...
    def testSymmetricMirrorPad(self):
        def mirror_pad(t, paddings): return array_ops.pad(
