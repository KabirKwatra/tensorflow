...
class Computation(object):
    ...
    def Compile(self, argument_shapes=None, compile_options=None,
                backend=None):
...
class ComputationBuilder(object):
    ...
    def SetUpAlias(self, output_index, param_number, param_index):
        ...
        """
        return self._builder.SetUpAlias(output_index, param_number,
                                        param_index)
    ...
    def Infeed(self, shape, token=None):
        ...
        return ops.InfeedWithToken(
            token, shape.with_major_to_minor_layout_if_absent())
    ...
    def Outfeed(self, operand, token=None):
        ...
        if token is None:
            token = ops.CreateToken(self._builder)
        return ops.OutfeedWithToken(operand, token,
                                    self._builder.GetShape(operand), "")
    ...
    def ParameterFromNumpy(self, value, name=None, parameter_num=None):
        ...
        Returns:
          An XlaOp.
        """
        return self.ParameterWithShape(shape_from_pyval(value),
                                       name=name,
    ...
    def Pad(self, operand, padding_value, padding_config):
        ...
        """
        return ops.Pad(operand, padding_value,
                       make_padding_config(padding_config))
    ...
    def RngNormal(self, mu, sigma, dims):
        ...
        """
        shape = _xla.Shape.array_shape(
            self.GetShape(mu).xla_element_type(), dims)
    ...
    def RngUniform(self, a, b, dims):
        ...
        """
        shape = _xla.Shape.array_shape(
            self.GetShape(a).xla_element_type(), dims)
    ...
    def Sort(self, operands, dimension=-1, comparator=None, is_stable=False):
        ...
        """
        operands = (list(operands) if isinstance(
            operands, collections.abc.Sequence) else [operands])
        return ops.Sort(
            self._builder,
            operands,
    ...
    def SortKeyVal(self, keys, values, dimension=-1, is_stable=False):
        ...
        """
        return ops.Sort(self._builder, [keys, values], None, dimension,
                        is_stable)
    ...
    def TriangularSolve(
        ...
        if not transpose_a:
            ...
            transpose = (ops.TriangularSolveOptions_Transpose.ADJOINT
                         if conjugate_a else
                         ops.TriangularSolveOptions_Transpose.TRANSPOSE)
        return ops.TriangularSolve(a, b, left_side, lower, unit_diagonal,
                                   transpose)
