...
public:
    ...
    const std::vector<Device*>& local_devices() const {
        return local_devices_;
    }
    const std::map<int, Device*>& id_to_device() const {
...
public:
    ...
    public:
        ...
        SharedDeviceBuffer* operator->() const {
            return buffer().get();
        }
        const SharedDeviceBuffer& operator*() const {
            return *buffer();
        }
