...
ImportRename = collections.namedtuple("ImportRename",
                                      ["new_name", "excluded_prefixes"])
...
class _PastaEditVisitor(ast.NodeVisitor):
    ...
    def _maybe_add_module_deprecation_warning(self, node, full_name,
                                              whole_name):
    ...
    def _maybe_add_arg_names(self, node, full_name):
        ...
        if full_name in function_reorders:
            ...
            idx = 0
            for arg in node.args:
                if sys.version_info[:2] >= (3, 5) and isinstance(
                        arg, ast.Starred):
        ...
        for keyword in node.keywords:
            ...
            if argkey in renamed_keywords:
                ...
                if renamed_keywords[argkey] is None:
                    ...
                    col_offset = getattr(keyword, "col_offset",
                                         node.col_offset)
                    self.add_log(
                        INFO,
                        lineno,
    ...
    def visit_Attribute(self, node):  # pylint: disable=invalid-name
        ...
        if full_name:
            ...
            self._maybe_add_module_deprecation_warning(node, full_name,
                                                       whole_name)
    ...
    def visit_Import(self, node):  # pylint: disable=invalid-name
        ...
        max_submodule_depth = getattr(self._api_change_spec,
        ...
        for import_alias in node.names:
            all_import_components = six.ensure_str(
                import_alias.name).split(".")
            ...
            for i in reversed(list(range(1, max_submodule_depth + 1))):
                ...
                for j in range(1, min(i, len(all_import_components))):
                    import_component += "." + six.ensure_str(
                        all_import_components[j])
                ...
                new_name = (import_rename_spec.new_name +
                ...
                insert_offset = 1
                for line_to_insert in inserts_after_imports.get(
                        full_import, []):
                    assert self._stack[-1] is node
                    parent = self._stack[-2]
                    ...
                    pasta.base.formatting.set(new_line_node, "suffix",
                                              os.linesep)
                    self.add_log(
                        INFO,
                        node.lineno,
    ...
    def visit_ImportFrom(self, node):  # pylint: disable=invalid-name
        ...
        import_renames = getattr(self._api_change_spec, "import_renames", {})
        import_rename_spec = import_renames.get(from_import_first_component,
                                                None)
        ...
        for import_alias in node.names:
            ...
            if excluded_from_module_rename(full_module_name,
                                           import_rename_spec):
        ...
        new_from_import = (import_rename_spec.new_name +
                           from_import[len(from_import_first_component):])
        updated_node = ast.ImportFrom(new_from_import, updated_aliases,
                                      node.level)
        ...
        self.add_log(
            ...
            (pasta.dump(node), pasta.dump(updated_node),
             additional_import_log),
        )
...
class PastaAnalyzeVisitor(_PastaEditVisitor):
    ...
    def visit_Attribute(self, node):  # pylint: disable=invalid-name
        ...
        full_name = self._get_full_name(node)
        if full_name:
            detection = self._api_analysis_spec.symbols_to_detect.get(
                full_name, None)
            if detection:
    ...
    def visit_Import(self, node):  # pylint: disable=invalid-name
        ...
        for import_alias in node.names:
            ...
            full_import = (import_alias.name, import_alias.asname)
            detection = self._api_analysis_spec.imports_to_detect.get(
                full_import, None)
            if detection:
                self.add_result(detection)
                self.add_log(
...
class ASTCodeUpgrader(object):
    ...
    def process_opened_file(self, in_filename, in_file, out_filename,
                            out_file):
        """Process the given python file for incompatible changes.
    ...
    def process_tree(self, root_directory, output_root_directory,
        ...
        for dir_name, _, file_list in os.walk(root_directory):
            py_files = [
                f for f in file_list if six.ensure_str(f).endswith(".py")
            ]
            copy_files = [
            ...
            for filename in py_files:
                ...
                fullpath_output = os.path.join(
                    output_root_directory,
                    os.path.relpath(fullpath, root_directory))
        ...
        for input_path, output_path in files_to_process:
            ...
            if os.path.islink(input_path):
                ...
                link_target_output = os.path.join(
                    output_root_directory,
                    os.path.relpath(link_target, root_directory))
