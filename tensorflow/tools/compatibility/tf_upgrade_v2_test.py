            ...
            for child in children:
                ...
                for name in api_names:
                    if collect:
                        ...
                        _, _, _, text = self._upgrade(
                            "tf." + six.ensure_str(name))
                        if (text and
                            not text.startswith("tf.compat.v1") and
                            not text.startswith("tf.compat.v2") and
...
"""
    ...
    def verify_compat_v1_rename_correctness(self, values, ns_prefix=""):
        ...
        for v in values:
            ...
            self.assertEqual("tf.compat.v1." + ns_prefix +
                             v + "(a, b)", new_text)
    ...
    def testContribXavierInitializer(self):
        for contrib_alias in ["tf.contrib.", "contrib_"]:
            ...
            text = contrib_alias + \
                "layers.xavier_initializer_conv2d(False, 12)\n"
            _, unused_report, unused_errors, new_text = self._upgrade(text)
            self.assertEqual(
                new_text,
                "tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, "
    ...
    def testBaseEstimatorPartitioner(self):
        classes = ["LinearEstimator",
                   "DNNLinearCombinedEstimator", "DNNEstimator"]
        for c in classes:
            ns = "tf.estimator." + c
...
def _log_prob(self, x):
  return tf.reduce_logsumexp(
    ...
    def test_contrib_layers_layer_norm_deprecation(self):
        for contrib_alias in ["tf.contrib.", "contrib_"]:
            _, report, _, _ = self._upgrade(
                contrib_alias + "layers.layer_norm")
            self.assertIn(
    ...
    def test_contrib_summary_full_example(self):
        def deindent(n, s): return "\n".join(
            line[n:] for line in s.split("\n"))
        text = deindent(4, """
    import tensorflow as tf
    ...
    def testPywrapTensorflowWarning(self):
        ...
        self.assertEqual(expected, new_text)
        self.assertIn(
            "`tf.pywrap_tensorflow` will not be distributed", errors[0])
