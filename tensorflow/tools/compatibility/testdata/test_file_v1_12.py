...
class TestUpgrade(test_util.TensorFlowTestCase):
    ...
    def testSerializeSparseTensor(self):
        ...
        with self.cached_session():
            serialized_sp = tf.serialize_sparse(sp_input, "serialize_name",
                                                tf.string)
    ...
    def testSerializeManySparse(self):
        ...
        with self.cached_session():
            serialized_sp = tf.serialize_many_sparse(sp_input,
                                                     "serialize_name",
                                                     tf.string)
    ...
    def testSoftmaxCrossEntropyWithLogits(self):
        out = tf.nn.softmax_cross_entropy_with_logits(logits=[0.1, 0.8],
                                                      labels=[0, 1])
        self.assertAllClose(out, 0.40318608)
        out = tf.nn.softmax_cross_entropy_with_logits_v2(logits=[0.1, 0.8],
                                                         labels=[0, 1])
        self.assertAllClose(out, 0.40318608)
    ...
    def testLinearClassifier(self):
        ...
        scores = classifier.evaluate(input_fn=lambda: (data, target),
                                     steps=100)
    ...
    def testUniformUnitScalingInitializer(self):
        ...
        self.assertArrayNear([-0.45200047, 0.72815341],
                             init((2, )).numpy(),
                             err=1e-6)
