...
class ReshapeTest(trt_test.TfTrtIntegrationTestBase):
    ...
    def ExpectedEnginesToBuild(self, run_params):
        ...
        return {
            "TRTEngineOp_0":
            ["reshape-%d" % i
             for i in range(7)] + ["reshape-%d/shape" % i for i in range(7)]
        }
...
class TransposeTest(trt_test.TfTrtIntegrationTestBase):
    def GraphFn(self, inp):
        ...
        compatible_transpose = array_ops.transpose(inp, [0, 3, 1, 2],
                                                   name="transpose-1")
        compatible_transpose = array_ops.transpose(compatible_transpose,
                                                   [0, 2, 3, 1],
                                                   name="transposeback")
...
class IncompatibleTransposeTest(TransposeTest):
    def GraphFn(self, inp):
        ...
        incompatible_transpose = array_ops.transpose(inp, [2, 1, 0, 3],
                                                     name="transpose-2")
        excluded_transpose = array_ops.transpose(incompatible_transpose,
                                                 [0, 2, 3, 1],
                                                 name="transpose-3")
