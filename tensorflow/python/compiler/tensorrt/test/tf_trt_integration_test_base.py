...
class TfTrtIntegrationTestBase(test_util.TensorFlowTestCase):
    ...
    def BuildParams(self, graph_fn, dtype, input_shapes, output_shapes):
        ...
        input_mask = [[False] + [True] * (len(shape) - 1)
    ...
    def BuildParamsWithMask(
        ...
        for extra_in_shape, extra_out_shape in zip(extra_inputs,
                                                   extra_outputs):
        ...
        return TfTrtIntegrationTestParams(
            ...
            input_specs=[
                ...
                for i, (shape,
                        mask) in enumerate(zip(input_shapes, input_mask))
            ],
            output_specs=[
                self._GetTensorSpec(shape, mask, dtype, "output_%d" % i)
                for i, (shape,
                        mask) in enumerate(zip(output_shapes, output_mask))
            ],
        ...
        for optimizer in rewriter_config.custom_optimizers:
            if optimizer.name == "TensorRTOptimizer":
                optimizer.parameter_map[
                    "use_implicit_batch"].b = use_implicit_batch
    ...
    def _GetFeedDict(self, inputs_data):
        return {
            name: data
            for name, data in zip(self._GetFeedNames(), inputs_data)
        }
            ...
            with self.session(graph=g, config=config, use_gpu=True) as sess:
                ...
                for expected_shapes, current_input_data in zip(
                    ...
                    for _ in range(num_runs):
                        ...
                        for expected_shape, actual_val in zip(
                                expected_shapes, new_val):
                            self.assertEqual(list(expected_shape),
                                             list(actual_val.shape))
                        if val is not None:
                            # Some ops may have nondeterministic output. E.g. Conv2D may use
                            # winograd algorithm. So we set atol/rtol be larger than 1.e-06.
                            self.assertAllClose(val,
                                                new_val,
                                                atol=1.0e-05,
                                                rtol=1.0e-05)
                        val = new_val
    ...
    def _RunGraphV2(self,
        ...
        for expected_shapes, current_input_data in zip(
            ...
            for _ in range(num_runs):
                ...
                self.assertEqual(len(expected_shapes), len(new_val))
                for expected_shape, actual_val in zip(expected_shapes,
                                                      new_val):
                    self.assertEqual(list(expected_shape),
                                     list(actual_val.shape))
                if val is not None:
    ...
    def _RunGraph(self,
        ...
        if run_params.is_v2:
            results = self._RunGraphV2(saved_model_dir, inputs_data,
                                       graph_state, num_runs)
    ...
    def _GetCalibratedInferGraph(self, run_params, saved_model_dir,
                                 inputs_data):
    ...
    def _WriteGraph(self, run_params, gdef, graph_state):
        ...
        graph_name = (self.__class__.__name__ + "_" + run_params.test_name +
                      "_" + self._GetGraphStateLabel(graph_state) + ".pbtxt")
        logging.info("Writing graph to %s/%s", temp_dir, graph_name)
        graph_io.write_graph(gdef, temp_dir, graph_name)

    def _VerifyConnections(self, expected_engines, original_gdef,
                           converted_gdef):
        old_to_new_node_map = {
        ...
        removed_const_nodes = set([
            self._ToString(node.name) for node in original_gdef.node
            if node.op == "Const"
        ])
        for node in original_gdef.node:
    ...
    def _VerifyGraphDefV1(self, run_params, original_gdef, gdef_to_verify,
        ...
        if graph_state == GraphState.ORIGINAL:
            ...
            if isinstance(expected_engines, dict):
                self._VerifyConnections(expected_engines, original_gdef,
                                        gdef_to_verify)
    ...
    def _VerifyGraphDefV2(self, run_params, original_gdef, gdef_to_verify,
        ...
        trt_op_names = [
            node.name for node in gdef_to_verify.node
            if node.op == "TRTEngineOp"
        ]
        for func in gdef_to_verify.library.function:
            if not re.search(r"TRTEngineOp_\d+_native_segment",
                             func.signature.name):
    ...
    def _MakeSavedModelV1(self, run_params):
        ...
        with g.as_default():
            ...
            for spec in params.input_specs:
                inp = array_ops.placeholder(dtype=spec.dtype,
                                            shape=spec.shape,
                                            name=spec.name)
                inputs.append(inp)
            outputs = params.graph_fn(*inputs)
            if not isinstance(outputs, list) and not isinstance(
                    outputs, tuple):
                outputs = [outputs]

        signature_def = signature_def_utils.build_signature_def(
            inputs={
                inp.op.name: utils.build_tensor_info(inp)
                for inp in inputs
            },
            outputs={
                out.op.name: utils.build_tensor_info(out)
                for out in outputs
            },
            method_name=signature_constants.PREDICT_METHOD_NAME,
        )

        saved_model_dir = self._GetSavedModelDir(run_params,
                                                 GraphState.ORIGINAL)
        saved_model_builder = builder.SavedModelBuilder(saved_model_dir)
        with self.session(graph=g,
                          config=self._GetConfigProto(
                              run_params, GraphState.ORIGINAL)) as sess:
    ...
    def _MakeSavedModelV2(self, run_params):
        ...
        root.run = def_function.function(params.graph_fn,
                                         input_signature=params.input_specs)
        saved_model_dir = self._GetSavedModelDir(run_params,
                                                 GraphState.ORIGINAL)
...
def _GetTestConfigsV2():
    ...
    opts = list(
        itertools.product([FP32, FP16, INT8], [convert_offline],
                          [dynamic_engine], [no_calibration]))
    # We always run calibration with offline tool.
