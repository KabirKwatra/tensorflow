        ...
        with ops.device(self._device):
            ...
            if self._eager_mode:
                with context.eager_mode():
                    gen_dataset_ops.delete_iterator(handle=self._handle,
...
class OwnedIterator(trackable.Trackable, composite_tensor.CompositeTensor):
    ...
    def _gather_saveables_for_checkpoint(self):
        def _saveable_factory(name):
            ...
            policy = None
            if self._dataset:
                policy = self._dataset.options(
                ).experimental_external_state_policy
            if policy:
                return _IteratorSaveable(self._iterator_resource,
                                         name,
...
class IteratorSpec(type_spec.TypeSpec):
    ...
    def _from_components(self, components):
        return OwnedIterator(dataset=None,
