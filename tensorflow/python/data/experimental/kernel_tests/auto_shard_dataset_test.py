...
class AutoShardDatasetTest(reader_dataset_ops_test_base.TFRecordDatasetTestBase,
    ...
    def testFlatMapReaderPipeline(self, shuffle):
        ...
        expected = [
            b"Record %d of file %d" % (
                r, f)  # pylint:disable=g-complex-comprehension
            for f in (3, 8)
            for r in range(0, 10)
        ]
    ...
    def testZipReaderPipeline(self):
        ...
        expected = [
            (b"Record %d of file %d" % (r, f), b"Record %d of file %d" %
             (r, f))  # pylint:disable=g-complex-comprehension
            for r in range(0, 10)
            for f in (3, 8)
        ]
    ...
    def testNoReaderPipelines(self):
        ...
        self.assertDatasetProduces(
            dataset, [i for i in range(1024) if i % 2 == 0])
...
class AutoShardTextLineDatasetTest(
    ...
    def setUp(self):
        ...
        self._num_records = 10
        self.test_filenames = self._createFiles(
            self._num_files, self._num_records)
