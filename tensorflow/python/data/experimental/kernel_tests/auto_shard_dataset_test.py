...
class AutoShardDatasetTest(
    ...
    def testZipReaderPipeline(self):
        dataset1 = dataset_ops.Dataset.list_files(self.test_filenames,
                                                  shuffle=False)
        dataset1 = dataset1.apply(
            interleave_ops.parallel_interleave(core_readers.TFRecordDataset,
                                               10))
        dataset2 = dataset_ops.Dataset.list_files(self.test_filenames,
                                                  shuffle=False)
    ...
    @combinations.generate(
        combinations.times(
            test_base.default_test_combinations(),
            combinations.combine(params=[(1, 0, 10, 10), (2, 1, 20,
                                                          5), (10, 1, 1, 10)]),
        ))
    ...
    def testNoReaderPipelines(self):
        ...
        dataset = distribute._AutoShardDataset(dataset, 2, 0)
        self.assertDatasetProduces(dataset,
                                   [i for i in range(1024) if i % 2 == 0])
...
class AutoShardTextLineDatasetTest(
    ...
    def setUp(self):
        ...
        self._num_files = 10
        self._num_records = 10
        self.test_filenames = self._createFiles(self._num_files,
                                                self._num_records)
