...
class WhileV2Test(test.TestCase, parameterized.TestCase):
    ...
    def testSingleLoopVar(self):
        ...
        ret = while_loop_v2(lambda v: v < 8.0,
                            lambda v: v * v, [x],
    ...
    def testGradientTape(self):
        with backprop.GradientTape() as t:
            x = constant_op.constant(2.0)
            t.watch(x)
            ret = while_loop_v2(lambda v: v < 4.0,
                                lambda v: v * v, [x],
    ...
    def testMultipleWhileLoops(self):
        x = constant_op.constant(2.0)
        ret1 = while_loop_v2(lambda v: v < 4.0,
                             lambda v: v * v, [x],
                             return_same_structure=False)  # x**2
        ret2 = while_loop_v2(lambda v: v < 16.0,
                             lambda v: v * v, [ret1],
    ...
    def testCaptureExternalTensorInCond(self):
        ...
        ret = while_loop_v2(lambda v: v + y < 9.0,
                            lambda v: v * 3.0, [x],
                            return_same_structure=False)
        grad = gradients_impl.gradients(ret, [x])
        with self.cached_session():
    ...
    def testLoopWithTensorListPushBack(self):
        ...
        def Body(x, tl):
            ...
            tl = list_ops.tensor_list_push_back(tl,
                                                constant_op.constant(100.0))
            return x**2.0, tl
    ...
    @parameterized.named_parameters(
        ...
        ("RandomGammaWithAlphaBeta", random_gamma_with_alpha_beta,
         [5, 3, 4, 2]),
    ...
    def testDoNotAccumulateConstNodes(self):
        ...
        body_graph = while_v2._get_graph(forward_while_op, "body",
                                         "_body_graph")
        push_back_nodes = [
            o for o in body_graph.get_operations()
            if o.type == "TensorListPushBack"
        ]
    ...
    def testDisableLowering(self):
        ...
        with self.session() as sess:
            ...
            self.assertEqual(
                sess.run(ret, options=opts, run_metadata=run_metadata), 16)
