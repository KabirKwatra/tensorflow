...
class WhileV2Test(test.TestCase, parameterized.TestCase):
    ...
    def testMultipleLoopVars(self):
        ...
        gradx_2 = gradients_impl.gradients(
            ret, [x])  # [2*x*y + y**2 + 2*y + 1]
        ...
        def MidBodyBuilder(iterations):
            ...
            def MidBody(i, x):
                r = control_flow_ops.while_loop(
                    lambda *_: True,
                    lambda i, x: (
                        i + 1, math_ops.multiply(v, x, name="my_mul")),
                    (0, x),
    ...
    def testDuplicateAccumulator(self):
        ...
        body_graph = while_v2._get_graph(while_op, "body", "_body_graph")
        x_input_index = [i for i, inp in enumerate(
            while_op.inputs) if inp == x][0]
    ...
    @parameterized.named_parameters(
        ...
        ("RandomGammaWithAlphaBeta",
         random_gamma_with_alpha_beta, [5, 3, 4, 2]),
