...
class ListOpsTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testPopUninitializedTensorWithInvalidElementShapeFails(self):
        ...
        with self.assertRaisesRegexp(
            ...
            _, e = list_ops.tensor_list_pop_back(
    ...
    def testPushGetGrad(self):
        with backprop.GradientTape() as tape:
            ...
            t1 = list_ops.tensor_list_get_item(
                l, 1, element_dtype=dtypes.float32)
    ...
    def testGatherGrad(self, max_num_elements):
        with backprop.GradientTape() as tape:
            ...
            l = list_ops.tensor_list_push_back(l, c0)
            l = list_ops.tensor_list_push_back(l, constant_op.constant(2.0))
            t = list_ops.tensor_list_gather(
                l, [1, 0], element_dtype=dtypes.float32)
    ...
    def testGatherWithUnknownElementShape(self, max_num_elements):
        ...
        with self.assertRaisesRegexp(errors.InvalidArgumentError,
                                     "Incompatible ranks during merge: 0 vs. 1"):
            t = list_ops.tensor_list_gather(
                l, [0, 2], element_dtype=dtypes.float32)
            self.evaluate(t)
    ...
    def testGatherWithPartiallyDefinedElementShape(self, max_num_elements):
        ...
        t = list_ops.tensor_list_gather(
            l, [1, 2], element_dtype=dtypes.float32)
    ...
    def testGatherReservedListWithNoElementsAndPartialElementShapeFails(self):
        ...
        with self.assertRaisesRegexp(
            ...
            t = list_ops.tensor_list_gather(
                l, [0], element_dtype=dtypes.float32)
            self.evaluate(t)
    ...
    def testScatterIntoExistingList(self):
        ...
        l = list_ops.tensor_list_scatter(
            tensor=[1.], indices=[0], element_shape=[])
        l = list_ops.tensor_list_scatter(
    ...
    def testScatterGrad(self):
        with backprop.GradientTape() as tape:
            ...
            t0 = list_ops.tensor_list_get_item(
                l, 0, element_dtype=dtypes.float32)
            t1 = list_ops.tensor_list_get_item(
                l, 1, element_dtype=dtypes.float32)
    ...
    def testSetGetGrad(self):
        with backprop.GradientTape() as tape:
            ...
            e = list_ops.tensor_list_get_item(
                l, 1, element_dtype=dtypes.float32)
    ...
    def testGetUninitializedTensorWithInvalidElementShapeFails(self):
        ...
        with self.assertRaisesRegexp(
            ...
            "Trying to read an uninitialized tensor but "
                "element_shape is not fully defined"):
            e0 = list_ops.tensor_list_get_item(
                l, 0, element_dtype=dtypes.float32)
            self.evaluate(e0)
    ...
    def testSkipEagerStackSwitchDtype(self):
        with self.cached_session():
            ...
            s1 = list_ops.tensor_list_stack(
                list_, element_dtype=dtypes.float32)
    ...
    def testPushPopGradients(self):
        with backprop.GradientTape() as tape:
            ...
            l, e = list_ops.tensor_list_pop_back(
                l, element_dtype=dtypes.float32)
            e = 2 * e
    ...
    def testGetSetGradients(self):
        with backprop.GradientTape() as tape:
            ...
            e = list_ops.tensor_list_get_item(
                l, 0, element_dtype=dtypes.float32)
            ee = list_ops.tensor_list_get_item(
                l, 1, element_dtype=dtypes.float32)
            y = e * e + ee * ee
    ...
    def testSkipEagerSetItemWithMismatchedShapeFails(self):
        with self.cached_session() as sess:
            ...
            l_0 = list_ops.tensor_list_get_item(
                l, 0, element_dtype=dtypes.float32)
            with self.assertRaisesRegexp(errors.InvalidArgumentError,
                                         "incompatible shape"):
                sess.run(l_0, {ph: [3.0]})
    ...
    def testResourceVariableScatterGather(self):
        ...
        l_new_0 = list_ops.tensor_list_from_tensor(
            [3.0, 4.0], element_shape=[])
        l_new_1 = list_ops.tensor_list_from_tensor(
            [5.0, 6.0], element_shape=[])
    ...
    def testPushBackBatch(self):
        ...
        with self.assertRaisesRegexp(*expected_error):
            self.evaluate(
                list_ops.tensor_list_push_back_batch(l_batch, [3, 4]))
    ...
    def testZerosLikeNested(self):
        for dtype in (dtypes.uint8, dtypes.uint16, dtypes.int8, dtypes.int16,
            ...
            for _ in range(3):
                ...
                outputs.append(list_ops.tensor_list_stack(
                    out, element_dtype=dtype))
    ...
    def testSkipEagerTensorListGetItemGradAggregation(self):
        ...
        l_read1 = list_ops.tensor_list_get_item(
            l, 0, element_dtype=dtypes.float32)
        l_read2 = list_ops.tensor_list_get_item(
            l, 0, element_dtype=dtypes.float32)
    ...
    def testAddN(self):
        ...
        result_t = list_ops.tensor_list_stack(
            result, element_dtype=dtypes.float32)
    ...
    def testAddNNestedList(self):
        ...
        result_0 = list_ops.tensor_list_stack(
            list_ops.tensor_list_get_item(
                result, 0, element_dtype=dtypes.variant),
            element_dtype=dtypes.float32)
        result_1 = list_ops.tensor_list_stack(
            list_ops.tensor_list_get_item(
                result, 1, element_dtype=dtypes.variant),
            element_dtype=dtypes.float32)
    ...
    def testAddTensorListsFailsIfLeadingDimsMismatch(self):
        ...
        with self.assertRaisesRegexp(
            ...
            self.evaluate(list_ops.tensor_list_stack(
                l, element_dtype=dtypes.float32))
    ...
    def testSkipEagerConcatShapeInference(self):
        ...
        self.assertAllEqual(BuildTensor(
            [None, 2, 3]).shape.as_list(), [None, 2, 3])
        self.assertAllEqual(
            BuildTensor([None, 2, None]).shape.as_list(), [None, 2, None])
        self.assertAllEqual(BuildTensor(
            [1, 2, 3]).shape.as_list(), [None, 2, 3])
    ...
    def testSkipEagerSplitWithInvalidTensorShapeFails(self):
        with self.cached_session():
            ...
            l = list_ops.tensor_list_split(
                tensor, element_shape=None, lengths=[1])
            with self.assertRaisesRegexp(
                    errors.InvalidArgumentError,
    ...
    def testSplitWithInvalidLengthsFails(self):
        ...
        with self.assertRaisesRegexp(
            ...
            l = list_ops.tensor_list_split(
                [1., 2.], element_shape=None, lengths=[3])
            self.evaluate(l)
        with self.assertRaisesRegexp(
                errors.InvalidArgumentError,
                r"Unused values in tensor. Length of tensor: 2 Values used: 1"):
            l = list_ops.tensor_list_split(
                [1., 2.], element_shape=None, lengths=[1])
            self.evaluate(l)
    ...
    def testNestedListDevicetoDeviceCopy(self):
        ...
        with ops.device("gpu:1"):
            ...
            t = list_ops.tensor_list_stack(
                inner_l, element_dtype=dtypes.float32)
    ...
    def testTensorListStringsNoInline(self):
        ...
        def f():
            ...
            def upper(i):
                e = list_ops.tensor_list_get_item(
                    l, i, element_dtype=dtypes.string)
