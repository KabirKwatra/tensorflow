...
class BinaryOpTest(test.TestCase):
    ...
    def testFloatBasic(self):
        ...
        try:
            ...
            a_pos_small = np.linspace(0.1, 2, 15).reshape(1, 3,
                                                          5).astype(np.float32)
            x_pos_small = np.linspace(0.1, 10,
                                      15).reshape(1, 3, 5).astype(np.float32)
    ...
    def testFloatDifferentShapes(self):
        ...
        self.assertAllEqual(gy,
                            np.array([3, 7]).reshape(2, 1).astype(np.float32))
    ...
    def testInt64Basic(self):
        x = np.arange(1 << 40, 13 << 40, 2 << 40).reshape(1, 3,
                                                          2).astype(np.int64)
    ...
    def _compareBCast(self, xs, ys, dtype, np_func, tf_func):
        if dtype in (np.complex64, np.complex128):
            x = (1 +
                 np.linspace(0, 2 + 3j, np.prod(xs))).astype(dtype).reshape(xs)
            y = (1 +
                 np.linspace(0, 2 - 2j, np.prod(ys))).astype(dtype).reshape(ys)
        else:
    ...
    def testZeroPowerPowGrad(self):
        ...
        for dtype in (np.float16, np.float32, np.float64):
            sym_jac, num_jac = gradient_checker_v2.compute_gradient(
                lambda x: math_ops.pow(x, 0.0),
                [constant_op.constant([-1.0, 0.0, 1.0], dtype=dtype)],
            )
            self.assertAllClose(sym_jac, num_jac)
            power = constant_op.constant([0.0, 0.0, 0.0], dtype=dtype)
            sym_jac, num_jac = gradient_checker_v2.compute_gradient(
            ...
            with backprop.GradientTape() as tape:
                ...
                tape.watch(x)
                y = x**0.0
            self.assertAllClose(0.0, tape.gradient(y, x))
        # pylint: enable=cell-var-from-loop

        x = constant_op.constant(0.0)
        with backprop.GradientTape(persistent=True) as tape:
            tape.watch(x)
            y = math_ops.pow(x, 2.0)
            x_g = tape.gradient(y, x)
            self.assertAllClose(2.0 * 0.0**1.0, x_g)
            x_gg = tape.gradient(x_g, x)
            self.assertAllClose(2.0 * 0.0**0.0, x_gg)
            x_ggg = tape.gradient(x_gg, x)
            self.assertAllClose(0.0, x_ggg)
