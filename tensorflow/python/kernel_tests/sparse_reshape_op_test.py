...
class SparseReshapeTest(test.TestCase):
    ...
    def testWorksWellWithTfShape(self):
        with self.session(use_gpu=False) as sess:
            sp_input = self._SparseTensorPlaceholder()
            input_val = self._SparseTensorValue_5x6()
            # tf.shape generates int32 output
            shape = array_ops.shape(sp_input)
    ...
    def testFeedPartialShapes(self):
        with self.session(use_gpu=False):
            ...
            self.assertListEqual(
                sp_output.indices.get_shape().as_list(), [None, 3])
            self.assertListEqual(
                sp_output.dense_shape.get_shape().as_list(), [3])
            ...
            sp_output = sparse_ops.sparse_reshape(sp_input, [2, 3, 5])
            self.assertListEqual(
                sp_output.indices.get_shape().as_list(), [5, 3])
            self.assertListEqual(
                sp_output.dense_shape.get_shape().as_list(), [3])
            ...
            self.assertListEqual(
                sp_output.indices.get_shape().as_list(), [5, None])
            self.assertListEqual(
                sp_output.dense_shape.get_shape().as_list(), [None])
...
class EmptySparseTensorReshapeTest(test.TestCase, parameterized.TestCase):
    ...
    def _MakeAndReshapeTensor(self, tensor_class, original_shape, target_shape):
        if tensor_class == "sparse":
            ...
            dense_output = self.evaluate(
                array_ops.reshape(dense_input, target_shape))
    ...
    def testImpliedReshapeEmpty4DTensor(self, tensor_class):
        self.assertAllEqual(
            self._MakeAndReshapeTensor(
                tensor_class, [2, 4, 0, 6], [-1, 4, 6, 2]),
            [0, 4, 6, 2])
