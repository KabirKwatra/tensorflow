...
class SparseReshapeTest(test.TestCase):
    ...
    def testFeedDownRank(self):
        with self.session(use_gpu=False) as sess:
            ...
            self.assertAllEqual(
                output_val.indices,
                np.array([[0, 1], [1, 0], [1, 2], [3, 3], [4, 1], [4, 3],
                          [5, 2]]),
            )
            self.assertAllEqual(output_val.values, input_val.values)
    ...
    def testFeedMismatchedSizesWithInferredDim(self):
        with self.session(use_gpu=False) as sess:
            ...
            with self.assertRaisesOpError(
                    "requested shape requires a multiple"):
                sess.run(sp_output, {sp_input: input_val})
...
class EmptySparseTensorReshapeTest(test.TestCase, parameterized.TestCase):
    ...
    def _MakeAndReshapeTensor(self, tensor_class, original_shape,
                              target_shape):
    ...
    def testImpliedReshapeEmpty1DTensor(self, tensor_class):
        ...
        self.assertAllEqual(
            self._MakeAndReshapeTensor(tensor_class, [0], [-1, 1, 2]),
            [0, 1, 2])
    ...
    def testImpliedReshapeEmpty2DTensor(self, tensor_class):
        ...
        self.assertAllEqual(
            self._MakeAndReshapeTensor(tensor_class, [1, 0], [-1, 2, 3]),
            [0, 2, 3])
    ...
    def testImpliedReshapeEmpty4DTensor(self, tensor_class):
        self.assertAllEqual(
            self._MakeAndReshapeTensor(tensor_class, [2, 4, 0, 6],
                                       [-1, 4, 6, 2]),
            [0, 4, 6, 2],
        )
