...
class LinearOperatorAdjointTest(
    ...
    def test_matmul_adjoint_operator(self):
        ...
        self.assertAllClose(
            ...
            self.evaluate(
                full_matrix1.matmul(full_matrix2,
    ...
    def test_matvec(self):
        ...
        self.assertAllClose(matrix.T.dot(x),
                            self.evaluate(operator.H.matvec(x)))
    ...
    def test_solve_adjoint_operator(self):
        ...
        self.assertAllClose(
            self.evaluate(
                linalg.triangular_solve(matrix1.T, matrix2, lower=False)),
            self.evaluate(
    ...
    def test_solvevec(self):
        ...
        x = np.array([1.0, 2.0])
        operator = linalg.LinearOperatorFullMatrix(matrix)
        self.assertAllClose(inv_matrix.dot(x),
                            self.evaluate(operator.solvevec(x)))
        self.assertAllClose(inv_matrix.T.dot(x),
                            self.evaluate(operator.H.solvevec(x)))
