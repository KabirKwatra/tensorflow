...
class LinearOperatorAdjointTest(
    ...
    def operator_and_matrix(self,
        ...
        if use_placeholder:
            lin_op_matrix = array_ops.placeholder_with_default(
                matrix, shape=None)
    ...
    def test_matvec(self):
        ...
        self.assertAllClose(matrix.T.dot(
            x), self.evaluate(operator.H.matvec(x)))
    ...
    def test_solvevec(self):
        ...
        x = np.array([1., 2.])
        operator = linalg.LinearOperatorFullMatrix(matrix)
        self.assertAllClose(inv_matrix.dot(
            x), self.evaluate(operator.solvevec(x)))
        self.assertAllClose(
