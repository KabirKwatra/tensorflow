...
class BaseFFTOpsTest(test.TestCase):
    def _compare(self,
        ...
        self._compare_backward(x, rank, fft_length, use_placeholder, rtol,
                               atol)
    ...
    def _compare_forward(self,
                         ...
                         rtol=1e-4,
                         atol=1e-4):
        if test.is_built_with_rocm() and x.dtype in (np.complex64,
                                                     np.complex128):
...
class FFTOpsTest(BaseFFTOpsTest, parameterized.TestCase):
    def _tf_fft(self, x, rank, fft_length=None, feed_dict=None):
        ...
        with self.cached_session(use_gpu=True) as sess:
            return sess.run(self._tf_fft_for_rank(rank)(x),
                            feed_dict=feed_dict)

    def _tf_ifft(self, x, rank, fft_length=None, feed_dict=None):
        # fft_length unused for complex FFTs.
        with self.cached_session(use_gpu=True) as sess:
            return sess.run(self._tf_ifft_for_rank(rank)(x),
                            feed_dict=feed_dict)
    ...
    def test_random(self, rank, extra_dims, np_type):
        ...
        self._compare(gen((4, ) * dims).astype(np_type),
                      rank,
                      rtol=tol,
                      atol=tol)
    ...
    def test_grad_simple(self, rank, extra_dims, np_type):
        ...
        self._check_grad_complex(self._tf_fft_for_rank(rank),
                                 re,
                                 im,
                                 rtol=tol,
                                 atol=tol)
        self._check_grad_complex(self._tf_ifft_for_rank(rank),
                                 re,
                                 im,
                                 rtol=tol,
                                 atol=tol)
...
class RFFTOpsTest(BaseFFTOpsTest, parameterized.TestCase):
    def _tf_fft(self, x, rank, fft_length=None, feed_dict=None):
        with self.cached_session(use_gpu=True) as sess:
            return sess.run(self._tf_fft_for_rank(rank)(x, fft_length),
                            feed_dict=feed_dict)

    def _tf_ifft(self, x, rank, fft_length=None, feed_dict=None):
        with self.cached_session(use_gpu=True) as sess:
            return sess.run(self._tf_ifft_for_rank(rank)(x, fft_length),
                            feed_dict=feed_dict)
    ...
    @parameterized.parameters(
        itertools.product(VALID_FFT_RANKS, range(3), (5, 6),
    ...
    def test_basic(self, rank, extra_dims, size, np_rtype):
        ...
        self._compare_backward(c2r.astype(np_ctype),
                               rank, (size, ) * rank,
                               rtol=tol,
                               atol=tol)

    @parameterized.parameters(
        itertools.product((1, ), range(3), (64, 128),
                          (np.float32, np.float64)))
    ...
    def test_random(self, rank, extra_dims, size, np_rtype):
        ...
        inner_dim = size // 2 + 1
        self._compare_forward(
            gen_real((size, ) * dims).astype(np_rtype),
            rank,
            (size, ) * rank,
            rtol=tol,
            atol=tol,
    ...
    @parameterized.parameters(
        itertools.product(VALID_FFT_RANKS, range(2), (5, 6),
                          (np.float32, np.float64)))
    def test_grad_simple(self, rank, extra_dims, size, np_rtype):
        ...
        self._check_grad_real(self._tf_fft_for_rank(rank),
                              re,
                              rtol=tol,
                              atol=tol)
    ...
    def test_negative_axes(self):
        with self.session():
            ...
            self.assertAllEqual(fft_ops.ifftshift(shifted, axes=(0, -1)),
                                freqs)
            self.assertAllEqual(fft_ops.fftshift(freqs, axes=-1),
