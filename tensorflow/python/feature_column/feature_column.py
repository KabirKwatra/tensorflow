    ...
    def _get_logits():  # pylint: disable=missing-docstring
        ...
        for column in sorted(feature_columns, key=lambda x: x.name):
            ...
            with variable_scope.variable_scope(
                ...
                output_tensor = array_ops.reshape(tensor,
                                                  shape=(batch_size,
                                                         num_elements))
...
class _FCLinearWrapper(base.Layer):
    ...
    def __init__(self,
                 ...
                 name=None,
                 **kwargs):
        super(_FCLinearWrapper, self).__init__(trainable=trainable,
                                               name=name,
                                               **kwargs)
    ...
    def build(self, _):
        if isinstance(self._feature_column, _CategoricalColumn):
            ...
            num_elements = (
                self._feature_column._variable_shape.num_elements())  # pylint: disable=protected-access
            weight = self.add_variable(
                name="weights",
...
class _BiasLayer(base.Layer):
    ...
    def __init__(self,
                 ...
                 name=None,
                 **kwargs):
        super(_BiasLayer, self).__init__(trainable=trainable,
                                         name=name,
                                         **kwargs)
        self._units = units
        self._weight_collections = weight_collections
            ...
            for column in self._feature_columns:
                if not isinstance(column, (_DenseColumn, _CategoricalColumn)):
                    raise ValueError(
                        ...
                        "_DenseColumn or _CategoricalColumn. Given: {}".format(
                            column))
            weighted_sums = []
            ordered_columns = []
            builder = _LazyBuilder(features)
            for layer in sorted(self._column_layers.values(),
                                key=lambda x: x.name):
...
def _bucketized_column(source_column, boundaries):
    ...
    if not boundaries or not (isinstance(boundaries, list)
                              or isinstance(boundaries, tuple)):
...
def _categorical_column_with_identity(key, num_buckets, default_value=None):
    ...
    fc_utils.assert_key_is_string(key)
    return _IdentityCategoricalColumn(key=key,
        ...
        if isinstance(key, _HashedCategoricalColumn):
            raise ValueError(
                ...
                "Instead, use the feature name as a string. Given: {}".format(
                    key))
    return _CrossedColumn(keys=tuple(keys),
...
class _EmbeddingColumnLayer(base.Layer):
    ...
    def __init__(self,
        ...
        """
        super(_EmbeddingColumnLayer, self).__init__(trainable=trainable,
                                                    name=name,
                                                    **kwargs)
    ...
    def build(self, _):
        ...
        if self._weight_collections and not context.executing_eagerly():
            _add_to_collections(self._embedding_weight_var,
                                self._weight_collections)
        self.built = True
