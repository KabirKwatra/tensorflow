...
class _StateManagerImpl(StateManager):
    ...
    def create_variable(
        ...
        if isinstance(var, variables.PartitionedVariable):
            for v in var:
                part_name = (name + "/" +
                             str(v._get_save_slice_info().var_offset[0]))  # pylint: disable=protected-access
                self._layer._track_trackable(v, feature_column.name + "/" +
                                             part_name)  # pylint: disable=protected-access
        else:
            if isinstance(var, trackable.Trackable):
                self._layer._track_trackable(var,
                                             feature_column.name + "/" + name)  # pylint: disable=protected-access
    ...
    def add_resource(self, feature_column, resource_name, resource):
        ...
        if self._layer is not None and isinstance(resource,
                                                  trackable.Trackable):
            ...
            if feature_column.name not in self._layer._resources:
                self._layer._resources[
                    feature_column.name] = data_structures.Mapping()
            if resource_name not in self._layer._resources[
                    feature_column.name]:
                self._layer._resources[
                    feature_column.name][resource_name] = resource
    ...
    def get_resource(self, feature_column, resource_name):
        if (feature_column not in self._cols_to_resources_map or resource_name
                not in self._cols_to_resources_map[feature_column]):
...
class _BaseFeaturesLayer(Layer):
    ...
    def __init__(self,
                 ...
                 partitioner=None,
                 **kwargs):
        super(_BaseFeaturesLayer, self).__init__(name=name,
                                                 trainable=trainable,
                                                 **kwargs)
    ...
    def _process_dense_tensor(self, column, tensor):
        ...
        target_shape = self._target_shape(array_ops.shape(tensor),
                                          num_elements)
    ...
    def _verify_and_concat_tensors(self, output_tensors):
        ...
        _verify_static_batch_size_equality(output_tensors,
    ...
    def get_config(self):
        ...
        from tensorflow.python.feature_column import (
            serialization, )  # pylint: disable=g-import-not-at-top
    ...
    def from_config(cls, config, custom_objects=None):
        ...
        config_cp[
            "feature_columns"] = serialization.deserialize_feature_columns(
...
class _LinearModelLayer(Layer):
    ...
    def __init__(self,
                 ...
                 name=None,
                 **kwargs):
        super(_LinearModelLayer, self).__init__(name=name,
                                                trainable=trainable,
                                                **kwargs)
        ...
        for column in self._feature_columns:
            if not isinstance(column, (DenseColumn, CategoricalColumn)):
                raise ValueError(
                    ...
                    "DenseColumn or CategoricalColumn. Given: {}".format(
                        column))
    ...
    def call(self, features):
        if not isinstance(features, dict):
            raise ValueError(
                "We expected a dictionary here. Instead we got: {}".format(
                    features))
        with ops.name_scope(self.name):
            ...
            _verify_static_batch_size_equality(weighted_sums,
                                               self._feature_columns)
            predictions_no_bias = math_ops.add_n(weighted_sums,
                                                 name="weighted_sum_no_bias")
            predictions = nn_ops.bias_add(predictions_no_bias,
                                          self.bias,
                                          name="weighted_sum")
            return predictions
...
class LinearModel(keras_training.Model):
    ...
    def __init__(self,
        ...
        self.layer = _LinearModelLayer(feature_columns,
                                       units,
                                       sparse_combiner,
                                       trainable,
                                       name=self.name,
...
def _transform_features_v2(features, feature_columns, state_manager):
    ...
    with ops.name_scope(None,
                        default_name="transform_features",
                        values=features.values()):
        transformation_cache = FeatureTransformationCache(features)
        for column in feature_columns:
            with ops.name_scope(
                    None,
                    default_name=_sanitize_column_name_for_variable_scope(
                        column.name)):
...
def embedding_column(
    ...
    if initializer is None:
        initializer = initializers.truncated_normal(mean=0.0,
                                                    stddev=1 /
                                                    math.sqrt(dimension))
...
def shared_embedding_columns(
    ...
    if not shared_embedding_collection_name:
        shared_embedding_collection_name = "_".join(c.name
...
def shared_embedding_columns_v2(
    ...
    if not isinstance(c0, CategoricalColumn):
        raise ValueError(
            ...
            "Given: {}, of type: {}".format(c0, type(c0)))
    while isinstance(c0,
                     (WeightedCategoricalColumn, SequenceCategoricalColumn)):
        c0 = c0.categorical_column
    for c in sorted_columns[1:]:
        while isinstance(
                c, (WeightedCategoricalColumn, SequenceCategoricalColumn)):
            c = c.categorical_column
        if not isinstance(c, type(c0)):
            raise ValueError(
                "To use shared_embedding_column, all categorical_columns must have "
    ...
    for column in categorical_columns:
        result.append(
            column_creator(categorical_column=column,
                           combiner=combiner,
                           max_norm=max_norm))
...
def numeric_column(key,
    ...
    default_value = fc_utils.check_default_value(shape, default_value, dtype,
                                                 key)
...
def categorical_column_with_hash_bucket(key,
    ...
    if hash_bucket_size is None:
        raise ValueError("hash_bucket_size must be set. "
                         "key: {}".format(key))
...
def categorical_column_with_vocabulary_file_v2(
    ...
    if num_oov_buckets:
        if default_value is not None:
            raise ValueError(
                "Can't specify both num_oov_buckets and default_value in {}.".
                format(key))
...
def categorical_column_with_vocabulary_list(key,
    ...
    if dtype is None:
        ...
        raise ValueError(
            "dtype {} and vocabulary dtype {} do not match, column_name: {}".
            format(dtype, vocabulary_dtype, key))
    fc_utils.assert_string_or_int(dtype, prefix="column_name: {}".format(key))
    fc_utils.assert_key_is_string(key)
...
def categorical_column_with_identity(key, num_buckets, default_value=None):
    ...
    fc_utils.assert_key_is_string(key)
    return IdentityCategoricalColumn(key=key,
    ...
    * SparseTensor referred by second key:
    ...
    for key in keys:
        if not isinstance(key, six.string_types) and not isinstance(
                key, (CategoricalColumn, fc_old._CategoricalColumn)):  # pylint: disable=protected-access
            raise ValueError(
        ...
        if isinstance(
            ...
            raise ValueError(
                ...
                "Instead, use the feature name as a string. Given: {}".format(
                    key))
    return CrossedColumn(keys=tuple(keys),
...
class FeatureTransformationCache(object):
    ...
    def get(self, key, state_manager, training=None):
        ...
        # argument.
        try:
            transformed = column.transform_feature(self,
                                                   state_manager,
                                                   training=training)
        except TypeError:
    ...
    def _get_raw_feature_as_tensor(self, key):
        ...
        with ops.control_dependencies([
                check_ops.assert_positive(
                    ...
                    message="Feature (key: {}) cannot have rank 0. Given: {}".
                    format(key, feature_tensor),
                )
        ]):
...
def _to_sparse_input_and_drop_ignore_values(input_tensor, ignore_value=None):
    ...
    with ops.name_scope(None, "to_sparse_input", (
        ...
        ignore_value = math_ops.cast(ignore_value,
                                     input_tensor.dtype,
                                     name="ignore_value")
        indices = array_ops.where_v2(math_ops.not_equal(
            input_tensor, ignore_value),
                                     name="indices")
...
class BucketizedColumn(
    ...
    @property
    def _is_v2_column(self):
        return (isinstance(self.source_column, FeatureColumn)
                and self.source_column._is_v2_column)  # pylint: disable=protected-access
    ...
    def transform_feature(self, transformation_cache, state_manager):
        """Returns bucketized categorical `source_column` tensor."""
        source_tensor = transformation_cache.get(self.source_column,
                                                 state_manager)
        return math_ops._bucketize(  # pylint: disable=protected-access
            source_tensor,
            boundaries=self.boundaries)
    ...
    def _get_sparse_tensors_for_input_tensor(self, input_tensor):
        ...
        sparse_tensor = sparse_tensor_lib.SparseTensor(indices=indices,
...
class EmbeddingColumn(
    ...
    @property
    def _is_v2_column(self):
        return (isinstance(self.categorical_column, FeatureColumn)
                and self.categorical_column._is_v2_column)  # pylint: disable=protected-access
    ...
    def _get_dense_tensor_internal_helper(self, sparse_tensors,
        ...
        if (not self.use_safe_embedding_lookup and sparse_id_rank is not None
    ...
    def _old_get_dense_tensor_internal(self, sparse_tensors,
        ...
        if key not in self._embedding_weights:
            ...
            if self._ckpt_to_load_from is not None:
                ...
                checkpoint_utils.init_from_checkpoint(
                    ...
                    {self._tensor_name_in_ckpt: to_restore})
    ...
    def _get_dense_tensor_internal(self, transformation_cache, state_manager):
        ...
        with ops.name_scope(None, default_name=self.name):
            ...
            sparse_id_rank = tensor_shape.dimension_value(
                sparse_ids.dense_shape.get_shape()[0])
            embedding_lookup_sparse = embedding_ops.safe_embedding_lookup_sparse
            if (not self.use_safe_embedding_lookup
                    and sparse_id_rank is not None and sparse_id_rank <= 2):
                embedding_lookup_sparse = embedding_ops.embedding_lookup_sparse
            # Return embedding lookup result.
            return embedding_lookup_sparse(
                embedding_weights,
        ...
        if isinstance(self.categorical_column, SequenceCategoricalColumn):
            raise ValueError(
                ...
                "Given (type {}): {}".format(self.name,
                                             type(self.categorical_column),
                                             self.categorical_column))
        return self._get_dense_tensor_internal(transformation_cache,
                                               state_manager)
...
class CrossedColumn(
            ...
            if isinstance(key, six.string_types):
                feature_tensors.append(
                    transformation_cache.get(key, state_manager))
                            ...
                            (fc_old._CategoricalColumn, CategoricalColumn)):  # pylint: disable=protected-access
    ...
    def get_config(self):
        ...
        config = dict(zip(self._fields, self))
        config["keys"] = tuple(
            [serialize_feature_column(fc) for fc in self.keys])
        return config
...
def _prune_invalid_weights(sparse_ids, sparse_weights):
    ...
    if sparse_weights is not None:
        ...
        sparse_weights = sparse_ops.sparse_retain(sparse_weights,
                                                  is_weights_valid)
    return sparse_ids, sparse_weights
...
class IndicatorColumn(
    ...
    def _transform_id_weight_pair(self, id_weight_pair, size):
        ...
        if weight_tensor is not None:
            weighted_column = sparse_ops.sparse_merge(sp_ids=id_tensor,
        ...
        dense_id_tensor = sparse_ops.sparse_tensor_to_dense(id_tensor,
                                                            default_value=-1)

        # One hot must be float for tf.concat reasons since all other inputs to
        # input_layer are float32.
        one_hot_id_tensor = array_ops.one_hot(dense_id_tensor,
                                              depth=size,
                                              on_value=1.0,
                                              off_value=0.0)
    ...
    def variable_shape(self):
        ...
        if isinstance(self.categorical_column, FeatureColumn):
            return tensor_shape.TensorShape(
                [1, self.categorical_column.num_buckets])
        else:
            return tensor_shape.TensorShape(
                [1, self.categorical_column._num_buckets])  # pylint: disable=protected-access
...
class SequenceCategoricalColumn(
    ...
    def _get_sparse_tensors_helper(self, sparse_tensors):
        ...
        if weight_tensor is not None:
            weight_tensor = sparse_ops.sparse_reshape(weight_tensor,
                                                      target_shape)
