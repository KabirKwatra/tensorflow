...
from tensorflow.python.framework import (composite_tensor, constant_op, dtypes,
                                         errors, ops, tensor_shape,
                                         tensor_spec, tensor_util, type_spec)
from tensorflow.python.ops import array_ops
from tensorflow.python.ops import control_flow_util as util
from tensorflow.python.ops import (gen_array_ops, gen_control_flow_ops,
                                   gen_logging_ops, gen_math_ops, math_ops,
                                   tensor_array_ops)
from tensorflow.python.ops.gen_control_flow_ops import *
# pylint: enable=wildcard-import
from tensorflow.python.platform import tf_logging as logging
from tensorflow.python.util import compat, deprecation, nest, tf_should_use
...
def cond(
    ...
    **WARNING**: Any Tensors or Operations created outside of `true_fn` and
    ...
    if (
        util.EnableControlFlowV2(ops.get_default_graph())
...
def tuple(tensors, name=None, control_inputs=None):  # pylint: disable=redefined-builtin
    ...
    with ops.name_scope(name, "tuple", tensors) as name:
        tensors = [
            t
            if (isinstance(t, ops.Operation) or tensor_util.is_tensor(t) or t is None)
...
def _indexed_case_verify_and_canonicalize_args(branch_fns, default, branch_index):
    ...
    if not isinstance(branch_index, ops.Tensor):
        raise TypeError("branch_index must a Tensor, got {}".format(type(branch_index)))
    if not branch_index.dtype.is_integer:
        raise TypeError(
            "branch_index must an integer Tensor, got {}".format(branch_index.dtype)
        )
