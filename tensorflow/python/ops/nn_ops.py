...
def _non_atrous_convolution(
    ...
    with ops.name_scope(name, "non_atrous_convolution", [input, filter]) as scope:
        input = ops.convert_to_tensor(
            input, name="input")  # pylint: disable=redefined-builtin
        input_shape = input.get_shape()
        filter = ops.convert_to_tensor(
            filter, name="filter")  # pylint: disable=redefined-builtin
...
class _WithSpaceToBatch(object):
    ...
    def __init__(self,
        ...
        if padding == "SAME":
            ...
            filter_shape = ops.convert_to_tensor(
...
def conv1d(
    ...
    with ops.name_scope(name, "conv1d", [value, filters]) as name:
        ...
        dilations = [1] + \
            _get_sequence(dilations, 1, channel_index, "dilations")
...
def atrous_conv2d_transpose(value,
    ...
    with ops.name_scope(name, "atrous_conv2d_transpose",
        ...
        input_sizes = [
            rate * rate *
            output_shape[0], (in_height + pad_bottom_extra) // rate,
...
def _flatten_outer_dims(logits):
    ...
    output = array_ops.reshape(
        logits, array_ops.concat([[-1], last_dim_size], 0))
    ...
    **Note that to avoid confusion, it is required to pass only named arguments to
    ...
    with ops.name_scope(name, "SparseSoftmaxCrossEntropyWithLogits",
        ...
        with ops.control_dependencies(shape_checks):
            ...
            precise_logits = array_ops.reshape(
                precise_logits, [-1, num_classes])
