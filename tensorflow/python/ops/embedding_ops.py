...
from tensorflow.python.ops import \
    data_flow_grad  # pylint: disable=unused-import
...
def _clip(params, ids, max_norm):
    ...
    return clip_ops.clip_by_norm(
        params,
        max_norm,
        axes=(list(range(ids_rank, params_rank)) if ids_static
              and params_static else math_ops.range(ids_rank, params_rank)),
    )
    ...
    with ops.name_scope(name, "embedding_lookup_sparse",
        ...
        if not ignore_weights:
            ...
            if combiner == "sum":
                ...
                embeddings = math_ops.divide(embeddings,
                                             weight_sum_sqrt,
                                             name=name)
            else:
