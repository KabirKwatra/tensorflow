...
import six
from tensorflow.core.framework import graph_pb2, summary_pb2
from tensorflow.core.protobuf import config_pb2
from tensorflow.python.eager import context
from tensorflow.python.eager import profiler as _profiler
from tensorflow.python.framework import (constant_op, dtypes, ops, smart_cond,
                                         tensor_util)
from tensorflow.python.ops import (array_ops, control_flow_ops,
                                   gen_summary_ops, math_ops,
                                   resource_variable_ops, summary_op_util)
...
def summary_writer_function(name, tensor, function, family=None):
    ...
    with ops.device("cpu:0"):
        op = smart_cond.smart_cond(should_record_summaries(), record, _nothing, name="")
        if not context.executing_eagerly():
            ops.add_to_collection(
                ops.GraphKeys._SUMMARY_COLLECTION, op
            )  # pylint: disable=protected-access
...
def trace_export(name, step=None, profiler_outdir=None):
    ...
    with _current_trace_context_lock:
        ...
        if profiler_outdir is None and isinstance(
            _summary_state.writer, ResourceSummaryWriter
        ):
        ...
        if profiler and profiler_outdir is None:
            raise ValueError(
                "Must set profiler_outdir or " "enable summary writer with logdir."
            )
