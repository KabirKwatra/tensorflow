        ...
        if ragged_tensor.is_ragged(input_tensor):
            ...
            if input_tensor.shape.ndims == 2:
                ...
                ragged_input_tensor = ragged_tensor.RaggedTensor.from_row_splits(
                    ...
                    array_ops.stack([
                        0,
                        array_ops.shape(input_tensor, out_type=dtypes.int32)[0]
                    ]),
                    validate=False,
                )
    ...
    if input_ndims == 0:
        ...
        if with_offsets:
            offsets = ragged_tensor.RaggedTensor.from_row_splits(
                flat_result.char_to_byte_starts,
                flat_result.row_splits,
                validate=False)
            if input_ndims > 1:
...
def strings_split_v1(
    ...
    """
    input = deprecation.deprecated_argument_lookup("input", input, "source",
                                                   source)
    with ops.name_scope(name, "StringSplit", [input]):
        input = ragged_tensor.convert_to_tensor_or_ragged_tensor(
            input, dtype=dtypes.string, name="input")
        ...
        if result_type == "SparseTensor":
            if input.shape.rank == 1:
                return string_ops.string_split_v2(input,
                                                  sep=sep,
                                                  maxsplit=maxsplit)
            else:
                return string_split_v2(input, sep=sep,
                                       maxsplit=maxsplit).to_sparse()
        elif result_type == "RaggedTensor":
...
def ngrams(
    ...
    with ops.name_scope(name, "StringNGrams", [data]):
        if pad_values is None:
            ...
            if not isinstance(
                    pad_values[0],
                    util_compat.bytes_or_text_types) or not isinstance(
                        pad_values[1], util_compat.bytes_or_text_types):
                raise TypeError(
        ...
        if padding_width is not None and pad_values is None:
            raise ValueError(
                "pad_values must be provided if padding_width is set.")
        ...
        if isinstance(data, ops.Tensor):
            dense_shape = array_ops.concat([array_ops.shape(data)[:-1], [-1]],
                                           axis=0)
            to_tensor = True
        ...
        if not isinstance(data, ragged_tensor.RaggedTensor):
            if data.shape.ndims is None:
                ...
                rt = ragged_tensor.RaggedTensor.from_row_starts(data, [0],
                                                                validate=False)
                return ngrams(
                    rt,
                    ngram_width,
        ...
        if data.ragged_rank > 1:
            ...
            return (array_ops.reshape(output.flat_values, dense_shape)
                    if to_tensor else output)
        ...
        for width in ngram_widths:
            if width < 1:
                raise ValueError(
                    "All ngram_widths must be greater than 0. Got %s" %
                    ngram_width)
