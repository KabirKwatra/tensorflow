...
class RaggedMapOpTest(test_util.TensorFlowTestCase,
    ...
    @parameterized.parameters([
        ...
        # [d1, (d2)] -> [d1, 2]
        dict(
            fn=lambda x: array_ops.stack(
                [mo.reduce_mean(x), mo.reduce_sum(x)]),
    ...
    def testRaggedMap(
        ...
        elems = ragged_factory_ops.constant(
            elems, elems_dtype, elems_ragged_rank)
    ...
    def testMismatchRaggedRank(self):
        ...
        def fn(x): return ragged_math_ops.reduce_sum(x, axis=0)
    ...
    def testMismatchRaggedRank2(self):
        elems = ragged_factory_ops.constant([[1, 2, 3], [4, 5], [6, 7]])
        def fn(x): return ragged_tensor.RaggedTensor.from_row_starts(x, [0])
        with self.assertRaisesRegexp(
                ValueError, r'(?s)Expected `fn` to return.*But it returned.*'):
            _ = ragged_map_ops.map_fn(
                fn,
