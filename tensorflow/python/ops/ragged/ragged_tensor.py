    ...
    *ragged-rank*.  A `RaggedTensor`'s ragged-rank is fixed at graph creation
    ...
    def from_value_rowids(cls,
        ...
        with ops.name_scope(name, "RaggedFromValueRowIds",
            ...
            return cls._from_row_partition(values,
                                           row_partition,
                                           validate=validate)
            ...
            if values._row_partition.dtype != row_partition.dtype:
                if not ragged_config.auto_cast_partition_dtype():
                    ...
                    raise ValueError(
                        ...
                        (name, row_partition.dtype,
                         values._row_partition.dtype))
    ...
    def bounding_shape(self, axis=None, name=None, out_type=None):
        ...
        with ops.name_scope(name, "RaggedBoundingBox", [self, axis]):
            ...
            ragged_dimensions = [splits_shape[0] - 1] + [
                math_ops.maximum(math_ops.reduce_max(splits[1:] - splits[:-1]),
                                 0) for splits in nested_splits
            ]
    ...
    def with_values(self, new_values):
        ...
        return RaggedTensor(values=new_values,
                ...
                if partition._uniform_row_length is not None:
                    ...
                    if old_row_length is not None:
                        if size == old_row_length:
                            ...
                            raise ValueError(
                                "Inconsistent size for axis %s: %s vs %s" %
                                ((i + 1), old_row_length, size))
                partition._uniform_row_length = ops.convert_to_tensor(
                    size, dtype)
                if partition._nrows is None:
                    partition._nrows = array_ops.size(
                        partition._row_splits) - 1

        # Inner dimensions
        flat_shape = tensor_shape.as_shape([None] +
                                           shape[self.ragged_rank + 1:])
        ...
        with ops.name_scope(name, "RaggedFromTensor",
            ...
            if ragged_rank > 1:
                if tensor.shape.is_fully_defined():
                    ...
                    new_shape = [dim_size[ragged_rank - 1]
        ...
        with ops.name_scope(name, "RaggedToTensor",
            ...
            if ragged_shape.rank is not None and not isinstance(
                ...
                if shape.rank is None:
                    ...
                    output_shape = [
                        s1 if s1 is not None else s2 for (
                            s1,
                            s2) in zip(shape.as_list(), ragged_shape.as_list())
                    ]
...
def _get_row_partition_type_tensor_pairs(rt_input):
    ...
    tail = [
        _get_row_partition_type_tensor_pairs_tail(x) for x in partitions[1:]
    ]
