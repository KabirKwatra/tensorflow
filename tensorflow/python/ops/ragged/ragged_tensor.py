    ...
    *ragged-rank*.  A `RaggedTensor`'s ragged-rank is fixed at graph creation
    ...
    def __init__(self, values, row_partition, internal=False):
        ...
        if not isinstance(row_partition, RowPartition):
            raise TypeError(
                "row_partition must be a RowPartition, got %r" % row_partition
            )
    ...
    def with_values(self, new_values):
        ...
        if (
            ...
            and self.row_partition.row_splits.dtype != new_values.row_splits.dtype
        ):
            if not ragged_config.auto_cast_partition_dtype():
                raise ValueError(
...
def _get_row_partition_type_tensor_pairs(rt_input):
    ...
    if (
        partitions[0]._cached_value_rowids is not None
    ):  # pylint: disable=protected-access
        return [
...
def _get_nested_row_partitions(rt_input):
    ...
    if isinstance(rt_input.values, RaggedTensor):
        return [rt_input.row_partition] + _get_nested_row_partitions(rt_input.values)
