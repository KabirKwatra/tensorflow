    ...
    *ragged-rank*.  A `RaggedTensor`'s ragged-rank is fixed at graph creation
    ...
    def from_nested_row_lengths(cls,
        ...
        with ops.name_scope(name, "RaggedFromNestedRowlengths",
            ...
            for lengths in reversed(nested_row_lengths):
                result = cls.from_row_lengths(
                    result, lengths, validate=validate)
            return result
        ...
        if isinstance(values, RaggedTensor):
            if isinstance(partition, ops.Tensor):
                if partition.dtype not in (dtypes.int32, dtypes.int64):
                    raise ValueError(
                        "%s must have dtype int32 or int64" % name)
        ...
        with ops.name_scope(name, "RaggedRowLengths", [self]):
            ...
            if axis == 0:
                ...
                shape = array_ops.shape(
                    self.values, out_type=self._row_splits.dtype)
    ...
    def bounding_shape(self, axis=None, name=None, out_type=None):
        ...
        with ops.name_scope(name, "RaggedBoundingBox", [self, axis]):
            ...
            flat_values_shape = array_ops.shape(
                rt_flat_values, out_type=out_type)

            ragged_dimensions = array_ops.stack([splits_shape[0] - 1] + [
                math_ops.maximum(math_ops.reduce_max(
                    splits[1:] - splits[:-1]), 0)
            ...
            bbox = array_ops.concat(
                [ragged_dimensions, inner_dimensions], axis=0)
    ...
    def merge_dims(self, outer_axis, inner_axis):
        ...
        """
        outer_axis = ragged_util.get_positive_axis(
            outer_axis, self.shape.ndims)
        inner_axis = ragged_util.get_positive_axis(
            inner_axis, self.shape.ndims)
...
# =============================================================================
    ...
    def from_tensor(cls,
        ...
        with ops.name_scope(name, "RaggedFromTensor", [tensor, lengths, padding]):
            ...
            if ragged_rank > 1:
                ...
                for axis in range(ragged_rank - 1, 0, -1):
                    ...
                    if dim_len is None:
                        ...
                        dim_len = constant_op.constant(
                            dim_len, row_splits_dtype)
            ...
            if padding is not None:
                ...
                has_default = control_flow_ops.cond(
                    ...
                    lambda: math_ops.reduce_all(
                        has_default_value, axis=reduce_axis),
                ...
                has_nondefault = math_ops.cast(
                    has_nondefault, row_splits_dtype)
                length_for_nondefault_value = (
                    has_nondefault * array_ops.expand_dims(
                        math_ops.range(1, ncols + 1), 0))
                lengths = math_ops.reduce_max(
                    length_for_nondefault_value, axis=1)
            ...
            const_nrows = tensor_shape.dimension_at_index(
                tensor.shape, 0).value
            const_ncols = tensor_shape.dimension_at_index(
    ...
    def to_tensor(self, default_value=None, name=None, shape=None):
        ...
        with ops.name_scope(name, "RaggedToTensor", [self, default_value, shape]):
            ...
            shape_tensor = _shape_as_tensor(
                shape, row_partition_tensors[0].dtype)
        ...
        with ops.name_scope(name, "RaggedFromSparse", [st_input]):
            ...
            with ops.control_dependencies(
                ...
                segment_ids = math_ops.cast(
                    st_input.indices[:, 0], row_splits_dtype)
                num_segments = math_ops.cast(
                    st_input.dense_shape[0], row_splits_dtype)
    ...
    def numpy(self):
        ...
        rows = [values[splits[i]:splits[i + 1]]
...
class RaggedTensorSpec(type_spec.BatchableTypeSpec):
    ...
    def _component_specs(self):
        ...
        inner_splits_spec = tensor_spec.TensorSpec(
            [None], self._row_splits_dtype)
    ...
    def _to_tensor_list(self, value):
        ragged_rank = value.ragged_rank if isinstance(
            value, RaggedTensor) else 0
...
def _merge_dims(value, outer_axis, inner_axis):
    ...
    if not isinstance(value, RaggedTensor):
        if value.shape.is_fully_defined():
            ...
            new_shape = old_shape[:outer_axis] + \
                [-1] + old_shape[inner_axis + 1:]
        else:
    ...
    for axis in range(outer_axis, inner_axis):
        if isinstance(new_values, RaggedTensor):
            ...
            if new_values.shape.is_fully_defined():
                ...
                new_shape = array_ops.concat(
                    [[-1], old_shape[shape_split:]], axis=0)
