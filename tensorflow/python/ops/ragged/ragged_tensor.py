    ...
    *ragged-rank*.  A `RaggedTensor`'s ragged-rank is fixed at graph creation
        ...
        with ops.name_scope(name, "RaggedRowLengths", [self]):
            ...
            if axis == 0:
                ...
                shape = array_ops.shape(
                    self.values, out_type=self._row_partition.dtype)
    ...
    def _set_shape(self, shape):
        ...
        * If a dimension of `shape` has a known rank, and is encoded as one
        ...
        # Inner dimensions
        flat_shape = tensor_shape.as_shape(
            [None] + shape[self.ragged_rank + 1:])
    ...
    def _from_compatible_tensor_list(self, tensor_list):
        ...
        if self._shape.ndims is not None:
            if isinstance(result, RaggedTensor):
                ...
                result._set_shape(
                    self._shape)  # pylint: disable=protected-access
            else:
...
def _get_row_partition_type_tensor_pairs(rt_input):
    ...
    tail = [_get_row_partition_type_tensor_pairs_tail(
        x) for x in partitions[1:]]
