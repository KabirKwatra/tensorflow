...
class RaggedGatherOpTest(test_util.TensorFlowTestCase, parameterized.TestCase):
    ...
    def testOutOfBoundsError(self):
        ...
        with self.assertRaisesRegexp(errors.InvalidArgumentError,
            ...
            self.evaluate(
                ragged_gather_ops.gather(tensor_params, ragged_indices))
        with self.assertRaisesRegexp(errors.InvalidArgumentError,
                                     r"indices\[2\] = 2 is not in \[0, 2\)"):
            self.evaluate(
                ragged_gather_ops.gather(ragged_params, tensor_indices))
        with self.assertRaisesRegexp(errors.InvalidArgumentError,
    ...
    @parameterized.parameters([
        ...
        dict(params_shape=[2, 3, 5, 6],
             indices_shape=[2, 3, 5, 7],
             batch_dims=3),
        # Batched gather with axis (axis > batch_dims > 0)
        dict(params_shape=[2, 3, 6], indices_shape=[2], axis=2, batch_dims=1),
        dict(params_shape=[2, 3, 6],
             indices_shape=[2, 4],
             axis=2,
             batch_dims=1),
        dict(params_shape=[3, 1, 6, 7],
             indices_shape=[3, 4],
             axis=3,
             batch_dims=1),
        dict(params_shape=[3, 2, 6, 7],
             indices_shape=[3, 4],
             axis=3,
             batch_dims=1),
        dict(params_shape=[2, 3, 6, 7],
             indices_shape=[2, 3],
             axis=3,
             batch_dims=2),
    ])
