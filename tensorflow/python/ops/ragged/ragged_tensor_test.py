...
class RaggedTensorTest(test_util.TensorFlowTestCase,
    ...
    def testClassDocStringExamples(self):
        ...
        rt1 = RaggedTensor.from_row_splits(
            values, row_splits=[0, 4, 4, 7, 8, 8])
        rt2 = RaggedTensor.from_row_lengths(
            values, row_lengths=[4, 0, 3, 1, 0])
    ...
    def testRaggedTensorConstructionErrors(self):
        ...
        with self.assertRaisesRegexp(TypeError,
            ...
            RaggedTensor(values=values, row_splits=[
                         0, 2, 2, 5, 6, 7], internal=True)
    ...
    def testFromValueRowIdsWithDerivedNRows(self):
        ...
        values = constant_op.constant(['a', 'b', 'c', 'd', 'e', 'f', 'g'])
        value_rowids = constant_op.constant(
            [0, 0, 2, 2, 2, 3, 4], dtypes.int64)

        rt = RaggedTensor.from_value_rowids(
            values, value_rowids, validate=False)
    ...
    def testFromUniformRowLengthWithPlaceholders(self):
        ph_values = array_ops.placeholder_with_default(
            [1, 2, 3, 4, 5, 6], [None])
        ...
        rt3 = RaggedTensor.from_uniform_row_length(
            [1, 2, 3, 4, 5, 6], ph_rowlen)
    ...
    def testFromNestedValueRowIdsWithExplicitNRowsMismatch(self):
        ...
        with self.assertRaisesRegexp(
            ...
            RaggedTensor.from_nested_value_rowids(
                values, nested_value_rowids, nrows)
    ...
    def testRaggedTensorAccessors_3d_with_ragged_rank_1(self):
        ...
        for rt in [rt1, rt2, rt3]:
            ...
            self.assertAllEqual(rt.nested_value_rowids()[
                                0], [0, 0, 2, 2, 2, 3, 4])
    ...
    def testRaggedTensorAccessors_3d_with_ragged_rank_2(self):
        ...
        rt2 = RaggedTensor.from_nested_value_rowids(
            values, nested_value_rowids)
    ...
    @parameterized.parameters(
        ...
        (SLICE_BUILDER[:, ::2], [row[::2]
                                 for row in EXAMPLE_RAGGED_TENSOR_2D]),
        (SLICE_BUILDER[:, ::-1], [row[::-1]
                                  for row in EXAMPLE_RAGGED_TENSOR_2D]),
        (SLICE_BUILDER[:, ::-2], [row[::-2]
                                  for row in EXAMPLE_RAGGED_TENSOR_2D]),
        (SLICE_BUILDER[:, ::-3], [row[::-3]
                                  for row in EXAMPLE_RAGGED_TENSOR_2D]),
        (SLICE_BUILDER[:, 2::-1],
    ...
    def testRaggedTensorGetItemWithPlaceholderShapes(self, slice_spec, expected):
        ...
        rt = RaggedTensor.from_row_splits(
            EXAMPLE_RAGGED_TENSOR_2D_VALUES, splits)
        self.assertAllEqual(rt, EXAMPLE_RAGGED_TENSOR_2D)
        self._TestGetItem(rt, slice_spec, expected)
    ...
    def testSessionPartialRunFeed(self):
        ...
        with self.test_session() as session:
            ...
            res1 = session.partial_run(
                handle, r1, feed_dict={a: a_val, b: b_val})
...
# =============================================================================
    ...
    def testRaggedToVariant(self, ragged_constant, ragged_rank):
        rt = ragged_factory_ops.constant(
            ragged_constant, ragged_rank=ragged_rank)
        et = rt._to_variant()
...
class RaggedTensorSpecTest(test_util.TensorFlowTestCase,
    ...
    def testIsCompatibleWith(self):
        ...
        self.assertTrue(spec4.is_compatible_with(
