    ...
    def __new__(cls,
        ...
        if value_key is not None:
            if not isinstance(value_key, str):
                raise ValueError(
                    "value_key must be a string; got %r" % value_key)
            if not value_key:
...
class _ParseOpParams(object):
    ...
    def from_features(cls, features, types):
        ...
        if features:
            ...
            for key in sorted(features.keys()):
                ...
                params._add_feature(
                    key, feature)  # pylint: disable=protected-access
        return params
    ...
    def _add_ragged_key(self, key, value_type, split_type):
        ...
        if key in self.ragged_keys:
            original_value_type = self.ragged_value_types[self.ragged_keys.index(
                key)]
            original_split_type = self.ragged_split_types[self.ragged_keys.index(
                key)]
    ...
    for key in sorted(features.keys()):
        ...
        if isinstance(feature, SparseFeature):
            ...
            if isinstance(feature.index_key, str):
                ...
                sp_ids = [tensor_dict[index_key]
                          for index_key in feature.index_key]
...
def _add_ragged_partition(values, partition, tensor_dict, row_splits_dtype,
    ...
    if isinstance(partition, RaggedFeature.UniformRowLength):
        if isinstance(values, ragged_tensor.RaggedTensor):
            length = ops.convert_to_tensor(
                partition.length, dtype=row_splits_dtype)
        ...
        partition_t = math_ops.cast(
            tensor_dict[partition.key], row_splits_dtype)
...
def _add_batched_ragged_partition(rt, partition, tensor_dict, feature_key,
    ...
    if isinstance(partition, RaggedFeature.UniformRowLength):
        if rt.ragged_rank > 1:
            length = ops.convert_to_tensor(
                partition.length, rt.row_splits.dtype)
            return ragged_tensor.RaggedTensor.from_row_splits(
