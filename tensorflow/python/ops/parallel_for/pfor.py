...
flags.DEFINE_bool("op_conversion_fallback_to_while_loop", True,
...
class WhileOp(object):
    ...
    def __init__(self, exit_node, pfor_ops, fallback_to_while_loop,
                 pfor_config):
        """Initializer.
    ...
    def _init_pfor(self, parent_pfor, indices, cond_stacked, inputs,
        ...
        for enter in self._direct_enters:
            ...
            assert not stacked and not is_sparse_stacked, (enter,
                                                           converted_enter)
    ...
    def _create_init_values(self, pfor_input):
        ...
        with ops.name_scope("while_init"):
            ...
            for i, enter in enumerate(self._enters):
                ...
                if i < num_outputs:
                    maybe_stacked_cache[
                        self._exit_switches[i].outputs[1]] = stacked
            ...
            for i, inp in enumerate(inputs):
                ...
                if i < num_outputs:
                    ...
                    output_tas.append(
                        tensor_array_ops.TensorArray(inp.dtype, loop_len))
            ...
            init_values = [True, pfor_input.pfor.all_indices
                           ] + inputs + output_tas
    ...
    def _process_cond_unstacked(self, conditions, indices, inputs, output_tas):
        ...
        for i, out_ta in enumerate(output_tas):
            ...
            new_output_tas.append(
                control_flow_ops.cond(not_all_done, lambda: out_ta, lambda:
                                      out_ta.write(0, inp)))
        # pylint: enable=cell-var-from-loop
        return not_all_done, indices, inputs, new_output_tas

    def _process_cond_stacked(self, conditions, indices, inputs,
                              inputs_stacked, output_tas):
        num_outputs = len(self._outputs)
    ...
    def _process_body(
        ...
        def true_fn(control_inputs, body_pfor, body_output, stacked):
            ...
            for x in control_inputs:
                for t in x.outputs:
                    converted_control_inp.append(
                        body_pfor._convert_helper(t).t)
            if stacked:
                # Note convert always does the stacking.
                output = body_pfor.convert(body_output)
            else:
                output, convert_stacked, _ = body_pfor._convert_helper(
                    body_output)
    ...
    def __call__(self, pfor_input):
        ...
        def body(not_all_done, indices, *args):
            ...
            with ops.name_scope("while_cond"):
                ...
                conditions, cond_stacked, _ = cond_pfor._convert_helper(
                    self._condition)
            ...
            if not cond_stacked:
                (
                    ...
                    new_inputs,
                    new_output_tas,
                ) = self._process_cond_unstacked(conditions, indices, inputs,
                                                 output_tas)
            else:
                (
                    not_all_done,
                    ...
                    new_inputs,
                    new_output_tas,
                ) = self._process_cond_stacked(conditions, indices, inputs,
                                               inputs_stacked, output_tas)
    ...
    def expanddim_inputs_for_broadcast(self):
        ...
        for i, inp in enumerate(self._inputs):
            if inp.is_stacked:
                ...
                new_shape = array_ops.concat([shape[:1], ones, shape[1:]],
                                             axis=0)
                self._inputs[i] = wrap(array_ops.reshape(inp.t, new_shape),
                                       True)
...
class RegisterPForWithArgs(RegisterPFor):
    ...
    def __call__(self, converter):
        def _f(pfor_input):
            return converter(pfor_input, self.op_type, *self._args,
                             **self._kw_args)
...
def _create_op(op_type, inputs, op_dtypes, attrs=None):
    ...
    op = ops.get_default_graph().create_op(op_type,
                                           inputs,
                                           op_dtypes,
                                           attrs=attrs,
                                           compute_device=True)
    flat_attrs = nest.flatten([(str(a), op.get_attr(str(a))) for a in attrs])
    execute.record_gradient(op_type, op.inputs, tuple(flat_attrs),
                            op.outputs[:])
    return op
...
def _fallback_converter(pfor_input):
    ...
    def while_body(i, *ta_list):
        ...
        op_outputs = _create_op(pfor_input.op_type,
                                inputs,
                                output_dtypes,
    ...
    def reduce(self, fn, *args):
        ...
        for arg in args:
            if not isinstance(arg, ops.Tensor):
                raise ValueError("Got a non-Tensor argument %s in reduce" %
                                 arg)
        ...
        with ops.control_dependencies(args):
            for output in concrete_function.outputs:
                if not isinstance(output, ops.Tensor):
                    raise ValueError(
                        "Got a non-Tensor output %s while running reduce" %
                        output)
                ...
                if output.shape.is_fully_defined():
                    dummy = array_ops.zeros(output.shape.as_list(),
                                            dtype=output.dtype)
                    pl_outputs.append(
                        array_ops.placeholder_with_default(dummy,
                                                           shape=output.shape))
                else:
                    # TODO(agarwal): support case when under XLA and output.shape is not
                    # fully defined.
                    pl_outputs.append(
                        array_ops.placeholder(output.dtype,
                                              shape=output.shape))
...
class PFor(object):
    ...
    def __init__(
        ...
        self.all_indices = (math_ops.range(loop_len)
                            if all_indices is None else all_indices)
    ...
    def _convert_sparse(self, y):
        ...
        """
        outputs = [
            self._convert_helper(t)
            for t in (y.indices, y.values, y.dense_shape)
        ]
    ...
    def _restack_sparse_tensor_logically(self, indices, values, shape):
        ...
        def fn(args):
            res = gen_sparse_ops.serialize_sparse(args[0],
                                                  args[1],
                                                  args[2],
        ...
        result = map_fn.map_fn(fn, [indices, values, shape],
                               dtype=dtypes.variant)
        return sparse_ops.deserialize_sparse(result,
                                             dtype=values.dtype,
                                             rank=sparse_tensor_rank)
    ...
    def _add_conversion(self, old_output, new_output):
        ...
        assert isinstance(new_output,
                          (WrappedTensor, ops.Operation)), new_output
    ...
    def _convert_helper(self, op_or_tensor):
        ...
        while stack:
            ...
            if is_inside_loop:
                ...
                converted_inputs = [
                    self._conversion_map[inp] for inp in y_op.inputs
                ]
                some_input_converted = any(
                    self._was_converted(x) for x in y_op.inputs)
                some_input_stacked = any(x.is_stacked
                                         for x in converted_inputs)
            ...
            with ops.control_dependencies(
                    control_dependencies), ops.name_scope(
                        y_op.name +
                        "/pfor/"), ops.get_default_graph()._original_op(y_op):
                ...
                # None of the inputs and control inputs were converted.
                      ...
                      (not is_stateful and not some_input_converted
                    ...
                    if hasattr(y_op, "pfor_converter"):
                        ...
                        converter = _pfor_converter_registry.get(
                            y_op.type, None)
                    ...
                    try:
                        try:
                            ...
                            if self._fallback_to_while_loop:
                                ...
                                six.reraise(ValueError, ValueError(str(e)),
                                            sys.exc_info()[2])
                    ...
                    assert isinstance(
                        new_outputs, (list, tuple, ops.Operation)), new_outputs
                ...
                if y is y_op:
                    ...
                    for old_output, new_output in zip(y_op.outputs,
                                                      new_outputs):
                        ...
                        if not new_output.is_sparse_stacked:
                            if new_output.is_stacked:
                                ...
                                if loop_len is None:
                                    batch_dim = tensor_shape.TensorShape(
                                        [None])
                                else:
                                    batch_dim = tensor_shape.TensorShape(
                                        loop_len)
                                output_shape = batch_dim.concatenate(
                                    output_shape)
...
def _convert_fused_batch_norm_grad(pfor_input):
    ...
    x, x_reverse_order, x_reverse_shape = _channel_flatten_input(
        x, data_format)
    inputs = [y_backprop, x] + [
        array_ops.reshape(pfor_input.stacked_input(i), [-1])
...
def _convert_conv2d_backprop_filter(pfor_input):
    ...
    if inputs_stacked:
        ...
        new_filter_shape = array_ops.concat([old_filter_sizes[:3], n, [-1]],
                                            axis=0)
...
def _convert_identity_n(pfor_input):
    ...
    return [
        wrap(out, inp.is_stacked)
        for out, inp in zip(outputs, pfor_input.inputs)
    ]
...
def _convert_fill(pfor_input):
    ...
    new_shape = array_ops.concat(
        [[-1],
         array_ops.ones([array_ops.size(dims)], dtype=dtypes.int32)],
        axis=0)
    value = array_ops.reshape(value, new_shape)
    # Compute the new output shape
    new_dims = array_ops.concat([pfor_input.pfor.loop_len_vector, dims],
                                axis=0)
    # Broadcast
...
def _convert_broadcast_to(pfor_input):
    ...
    t_expanded_shape = array_ops.concat([t_shape[:1], ones, t_shape[1:]],
                                        axis=0)

    return wrap(
        array_ops.broadcast_to(array_ops.reshape(t, t_expanded_shape),
                               new_shape), True)
...
def _convert_searchsorted(pfor_input, _, op_func):
    ...
    return wrap(_unflatten_first_dim(output, pfor_input.pfor.loop_len_vector),
                True)
...
def _convert_matrix_band_part(pfor_input):
    ...
    return wrap(
        array_ops.matrix_band_part(t, num_lower=num_lower,
                                   num_upper=num_upper), True)
...
def _convert_one_hot(pfor_input):
    ...
    if axis >= 0:
        axis += 1
    return wrap(array_ops.one_hot(indices, depth, on_value, off_value, axis),
                True)
...
def _convert_pack(pfor_input):
    ...
    if axis >= 0:
        axis += 1
    return wrap(array_ops.stack([x.t for x in pfor_input.inputs], axis=axis),
                True)
...
def _convert_unpack(pfor_input):
    ...
    return [
        wrap(x, True) for x in array_ops.unstack(value, axis=axis, num=num)
    ]
...
def _convert_split(pfor_input):
    ...
    return [
        wrap(x, True) for x in array_ops.split(t, num_split, axis=split_dim)
    ]
        ...
        if batch_dims > 0:
            ...
            order = array_ops.concat(
                [
                    ...
                    math_ops.range(batch_dims + 1, array_ops.rank(indices)),
                ],
                axis=0,
            )
...
def _convert_matmul(pfor_input):
    ...
    if a_stacked and b_stacked:
        output = wrap(math_ops.matmul(a, b, adjoint_a=tr_a, adjoint_b=tr_b),
                      True)
        return output
    elif a_stacked:
        if tr_a:
        ...
        if tr_b:
            ...
            perm = control_flow_ops.cond(math_ops.equal(
                min_dim, 1), lambda: [0, 1, 2], lambda: [1, 0, 2])
...
def _flatten_array_with_offset(ids, offset_delta, num_rows):
    ...
    offsets = math_ops.range(start=0,
                             limit=n * offset_delta,
                             delta=offset_delta,
                             dtype=ids.dtype)
...
def _convert_sparse_segment(pfor_input, _, op_func):
    ...
    if data_stacked:
        indices = _flatten_array_with_offset(indices,
                                             array_ops.shape(data)[1], n)
...
def _convert_shape_n(pfor_input):
    ...
    shapes = [
        ...
        if stacked else array_ops.shape(x, out_type=out_type)
...
def _convert_select(pfor_input):
    ...
    out = control_flow_ops.cond(
        cond_rank >
        1, lambda: _unflatten_first_dim(outputs[0], n), lambda: outputs[0])
...
def _convert_random(pfor_input, op_type, *args, **kw_args):
    ...
    del kw_args
    inputs = [
        pfor_input.unstacked_input(i) for i in range(pfor_input.num_inputs)
    ]
    # inputs[0] is "shape"
...
def _convert_random_with_param(pfor_input):
    ...
    if param_stacked:
        ...
        shape = array_ops.concat([pfor_input.pfor.loop_len_vector, shape],
                                 axis=0)
...
def _convert_multinomial(pfor_input):
    ...
    if logits_stacked:
        ...
        samples = gen_random_ops.multinomial(logits,
                                             num_samples * n,
                                             seed=seed,
                                             seed2=seed2,
                                             output_dtype=output_dtype)
...
def _convert_einsum(pfor_input, op_type):
    ...
    if chosen_symbol is None:
        raise ValueError(
            "Could not figure out what symbol to use for new axis.")
    ...
    if op_type == "XlaEinsum":
        result = xla.einsum(equation=new_equation,
                            a=first_input,
                            b=second_input)
    else:
        assert op_type == "Einsum"
        result = special_math_ops.einsum(new_equation, first_input,
                                         second_input)
...
def _convert_matrix_triangular_solve(pfor_input):
    ...
    adjoint = pfor_input.get_attr("adjoint")
    output = linalg_ops.matrix_triangular_solve(matrix,
                                                rhs,
                                                lower=lower,
                                                adjoint=adjoint)
    return wrap(output, True)
...
def _convert_tensor_array_read_v3(pfor_input):
    ...
    if is_inside_pfor:
        ...
        if index_stacked:
            ...
            value = data_flow_ops.tensor_array_gather_v3(handle,
                                                         index,
                                                         flow,
                                                         dtype=dtype)
            return wrap(value, True)
        value = data_flow_ops.tensor_array_read_v3(handle,
                                                   index,
                                                   flow,
                                                   dtype=dtype)
...
def _convert_tensor_array_write_v3(pfor_input):
    ...
    if is_inside:
        if index_stacked:
            raise ValueError("Need indices for %s to be loop invariant" %
                             handle)
        if not flow_stacked and not value_stacked:
            flow_out = data_flow_ops.tensor_array_write_v3(
                handle, index, value, flow)
        ...
        if not index_stacked:
            raise ValueError("Need indices for %s to be not loop invariant" %
                             handle)
...
def _convert_stack_push_v2(pfor_input):
    ...
    if not _stack_handle_inside_pfor(pfor_input.op.inputs[0], pfor_input):
        raise ValueError(
            "StackPushV2 not allowed on stacks created outside pfor")
...
def _convert_function_call(func, converter, inputs):
    ...
    outputs = []
    for call_output, output_tensor in zip(call_outputs,
                                          func._func_graph_outputs):
...
def _convert_partitioned_call(pfor_input):
    ...
    ), "Could not find FuncGraph object for %s. Got func %s" % (func_name,
                                                                func)
...
def _convert_stateless_if(pfor_input):
    ...
    if cond_stacked:
        ...
        outputs = control_flow_ops.cond(
            cond,
            lambda: _outputs_for_branch(then_branch.name, None, pfor_input,
                                        inputs),
            lambda: _outputs_for_branch(else_branch.name, None, pfor_input,
                                        inputs),
        )
