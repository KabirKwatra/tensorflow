...
class ArrayTest(PForTestCase):
        ...
        def loop_fn(i):
            ...
            for y in [x, x2, x_i]:
                for axis in [0, 2, -1]:
                    ...
                    outputs.append(array_ops.gather(
                        y, [[2, i], [i, 1]], axis=axis))

                outputs.append(array_ops.gather(
                    y, [0, 1, 2], axis=1, batch_dims=1))
                outputs.append(array_ops.gather(
                    y, [i, 1, 2], axis=2, batch_dims=1))
    ...
    def test_gather_nd(self):
        ...
        def loop_fn(i):
            ...
            outputs.append(array_ops.gather_nd(
                x_i, [[i], [i], [i]], batch_dims=1))
            return outputs
    ...
    def test_tile_loop_dependent(self):
        ...
        with self.assertRaisesRegexp(ValueError, "expected to be loop invariant"):
            pfor_control_flow_ops.pfor(
                loop_fn, 2, fallback_to_while_loop=False)
    ...
    def test_diag_part(self):
        for x in (random_ops.random_uniform([3, 2, 2]),
            ...
            # pylint: disable=cell-var-from-loop
            def loop_fn(i):
                inp = array_ops.gather(
                    x, i)  # pylint: disable=redefined-builtin
            ...
            for align in ["RIGHT_LEFT", "LEFT_RIGHT"]:
                results.extend([
                    array_ops.matrix_set_diag(
                        matrix_i, band_i, k=k, align=align),
                    array_ops.matrix_set_diag(
