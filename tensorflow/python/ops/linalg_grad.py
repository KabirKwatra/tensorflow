...
def _QrGrad(op, dq, dr):
    ...
    if q.dtype.is_complex:
        raise NotImplementedError(
            "QrGrad not implemented for dtype: %s" % q.dtype)
...
def _MatrixSolveLsGrad(op, grad):
    ...
    def _Overdetermined(op, grad):
        ...
        grad_a = -math_ops.matmul(a, zx_sym) + \
            math_ops.matmul(b, z, adjoint_b=True)
...
def _SelfAdjointEigV2Grad(op, grad_e, grad_v):
    ...
    with ops.control_dependencies([grad_e, grad_v]):
        ...
        grad_a = array_ops.matrix_band_part(
            grad_a + _linalg.adjoint(grad_a), -1, 0)
...
def _SvdGrad(op, grad_s, grad_u, grad_v):
    ...
    with ops.control_dependencies([grad_s, grad_u, grad_v]):
        ...
        term1 = math_ops.matmul(u, math_ops.matmul(
            term1_nouv, v1, adjoint_b=True))
        ...
        if a.dtype.is_complex:
            eye = _linalg.eye(
                s_shape[-1], batch_shape=s_shape[:-1], dtype=a.dtype)
            l = eye * v_gv
...
def _TransposeTridiagonalMatrix(diags):
    ...
    if diags.shape.is_fully_defined():
        ...
        zeros = array_ops.zeros(
            list(diags.shape[:-2]) + [1], dtype=diags.dtype)
