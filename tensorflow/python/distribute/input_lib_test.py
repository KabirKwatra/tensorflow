...
class DistributedIteratorTestBase(test.TestCase):
    ...
    def _wrap_dataset(
        ...
        if isinstance(dataset,
                      (dataset_ops.Dataset, dataset_ops.DatasetV1Adapter)):
...
class DistributedIteratorSingleWorkerTest(DistributedIteratorTestBase,
    ...
    def testTupleDataset(
        ...
        expected_values = [[(i, i**2), (i + 1, (i + 1)**2)]
    ...
    def testUnevenDatasetBatches(self, input_type, api_type, iteration_type,
        ...
        if tf2.enabled():
            ...
            def dataset_fn(_):
                return dataset_ops.DatasetV2.range(9).batch(  # pylint: disable=g-long-lambda
                    2,
            ...
            def dataset_fn(_):
                return dataset_ops.Dataset.range(9).batch(  # pylint: disable=g-long-lambda
                    2,
                    drop_remainder=drop_remainder)
    ...
    def testBatchSplitting(
        ...
        updated_batch_size = (batch_size //
                              split_batch_by if split_batch_by else batch_size)
        expected_values = [[
...
class DistributedIteratorTensorTypeTest(DistributedIteratorTestBase,
    ...
    def testRaggedSparse(self, distribution, input_type, drop_remainder,
        ...
        self.assertAllEqual(
            ...
            [[0.0, 0.0, 0.0], [5.0, 0.0, 0.0], [6.0, 6.0, 0.0],
             [7.0, 7.0, 7.0]],
        )
        ...
        if not drop_remainder or (defun_type == "tf_function"
                                  and input_type == "input_fn"):
...
class DistributedIteratorMultiWorkerTest(
    ...
    def testAutoshardingOption(self, input_type, api_type, iteration_type,
        ...
        if tf2.enabled():
            ...
            dataset_fn = lambda _: dataset_ops.Dataset.range(4).with_options(
                ds_option)
        dataset_or_input_fn = self._create_dataset_or_input_fn(
            input_type, dataset_fn)
    ...
    def testUnevenDatasetBatches(self, input_type, api_type, iteration_type):
        ...
        with context.graph_mode(), strategy.scope(), self.cached_session(
            ...
            if input_type == "dataset":
                # Autosharded
                expected_values = [[[0, 1], [4, 5], [2, 3], [6, 7]],
                                   [[8], [], [], []]]
            else:
                expected_values = [
                    [[0, 1], [2, 3], [0, 1], [2, 3]],
