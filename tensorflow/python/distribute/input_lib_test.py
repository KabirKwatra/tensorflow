...
from tensorflow.python.framework import errors, sparse_tensor
...
class DistributedIteratorTestBase(test.TestCase):
    ...
    def _test_input_iteration(
        ...
        if iteration_type == "get_next":
            ...
            def evaluate(x):
                return sess.run(x) if sess else self.evaluate(x)
...
class DistributedIteratorSingleWorkerTest(
    ...
    def testMultiDeviceIterInitialize(self, distribution):
        ...
        def dataset_fn(_):
            return dataset_ops.DatasetV1.range(10)
    ...
    def testDatasetV2IterError(self, distribution):
        ...
        with self.assertRaisesRegexp(
            RuntimeError, "or when eager execution is enabled"
        ):
            iter(dist_dataset)
    ...
    def testUnevenDatasetBatches(
        ...
        if tf2.enabled():
            ...
            def dataset_fn(_):
                return dataset_ops.DatasetV2.range(
                    9
    ...
    def testBatchSplitting(
        ...
        if tf2.enabled():
            ...
            def dataset_fn(_):
                return dataset_ops.DatasetV2.range(100).batch(batch_size)

        else:

            def dataset_fn(_):
                return dataset_ops.Dataset.range(100).batch(batch_size)
...
class DistributedIteratorMultiWorkerTest(
    ...
    def testAutoshardingOption(
        self, input_type, api_type, iteration_type, auto_shard_policy
    ):
        ds_option = dataset_ops.Options()
        ds_option.experimental_distribute.auto_shard_policy = auto_shard_policy
        if tf2.enabled():
            dataset_fn = lambda _: dataset_ops.DatasetV2.range(4).with_options(
                ds_option
            )
        else:
    ...
    def testOneDevicePerWorker(
        self, input_type, api_type, iteration_type, enable_get_next_as_optional
    ):
        if tf2.enabled():

            def dataset_fn(_):
                return dataset_ops.DatasetV2.range(4)

        else:

            def dataset_fn(_):
                return dataset_ops.Dataset.range(4)
    ...
    def testTwoDevicesPerWorker(
        ...
        strategy = mirrored_strategy.MirroredStrategy(
            devices=(
                self._cpu_and_one_gpu_devices()[0][1]
    ...
    def testUnevenDatasetBatches(self, input_type, api_type, iteration_type):
        ...
        if tf2.enabled():
            ...
            def dataset_fn(_):
                return dataset_ops.DatasetV2.range(9).batch(2)
    ...
    def testUnevenDatasetBatchesBetweenGraph(
        self, input_type, api_type, iteration_type, strategy_cls
    ):
        ...
        def _worker_fn(task_type, task_id, num_gpus):
            ...
            with context.graph_mode(), strategy.scope(), self.cached_session(
                target="grpc://" + self._cluster_spec[task_type][task_id]
            ) as sess:
                if tf2.enabled():

                    def dataset_fn(_):
                        return dataset_ops.DatasetV2.range(5).batch(2)

                else:

                    def dataset_fn(_):
                        return dataset_ops.Dataset.range(5).batch(2)
                ...
                if (
                    input_type == "dataset"
                    and strategy_cls
                    is collective_all_reduce_strategy.CollectiveAllReduceStrategy
                ):
                    # Autosharded
                    if task_id == 0:
                ...
                self._test_input_iteration(
                    ...
                    [
                        (
                            "/job:%s/task:%d" % (task_type, task_id),
    ...
    def testInputSignatureForPerReplicaValues(self, distribution, input_type):
        def dataset_fn(ctx):
            ...
            return dataset_ops.DatasetV2.from_tensor_slices(
                np.ones([10, 12]).astype(np.float32)
            ).batch(4)
