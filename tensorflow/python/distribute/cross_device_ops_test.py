...
class MultiWorkerCrossDeviceOpsTest(
    multi_worker_test_base.MultiWorkerTestBase, CrossDeviceOpsTestBase
):

    worker_devices = ["/job:worker/replica:0/task:0", "/job:worker/replica:0/task:1"]
    multi_worker_allreduce_combinations = combinations.combine(
        cross_device_ops=[
            ...
            combinations.NamedObject(
                ...
                cross_device_ops_lib.MultiWorkerAllReduce(
                    worker_devices, 2, [("pscpu/pscpu", 2, 100), ("xring", 2, -1)], 0
                ),
            ),
        ],
        devices=[
