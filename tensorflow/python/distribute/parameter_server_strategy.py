...
class ParameterServerStrategy(distribute_lib.Strategy):
    ...
    def __init__(self, cluster_resolver=None):
        ...
        if not cluster_resolver.cluster_spec():
            raise ValueError(
                "Cluster spec must be non-empty in `cluster_resolver`.")
        ...
        distribute_lib.distribution_strategy_replica_gauge.get_cell(
            "num_ps").set(len(self.extended.parameter_devices))
    ...
    def run(self, fn, args=(), kwargs=None, options=None):
        self._raise_pss_error_if_eager()
        super(ParameterServerStrategy, self).run(fn,
                                                 args=args,
                                                 kwargs=kwargs,
                                                 options=options)
...
class ParameterServerStrategyExtended(distribute_lib.StrategyExtendedV1):
    ...
    def _initialize_strategy(self,
        ...
        if cluster_resolver and cluster_resolver.cluster_spec():
            ...
            self._initialize_local(compute_devices,
                                   parameter_device,
                                   cluster_resolver=cluster_resolver)
    ...
    def _initialize_multi_worker(self, cluster_resolver):
        ...
        self._input_workers = input_lib.InputWorkers([(worker_device,
                                                       compute_devices)])
            ...
            if isinstance(x, values.Mirrored):
                if len(x._devices) == 1:  # pylint: disable=protected-access
                    ...
                    raise ValueError(
                        ...
                        "specify a single value or a Mirrored with a single value."
                        % x)
