...
class ParameterServerStrategy(distribute_lib.Strategy):
    ...
    def experimental_distribute_dataset(self, dataset):
        ...
        super(ParameterServerStrategy, self).experimental_distribute_dataset(
            dataset=dataset
        )
    ...
    def experimental_distribute_datasets_from_function(self, dataset_fn):
        self._raise_pss_error_if_eager()
        super(
            ParameterServerStrategy, self
        ).experimental_distribute_datasets_from_function(dataset_fn=dataset_fn)
...
class ParameterServerStrategyExtended(distribute_lib.StrategyExtendedV1):
    ...
    def _initialize_multi_worker(self, cluster_resolver):
        ...
        self._input_workers = input_lib.InputWorkers([(worker_device, compute_devices)])
    ...
    def _experimental_distribute_datasets_from_function(self, dataset_fn):
        ...
        return input_lib.get_distributed_datasets_from_function(
            dataset_fn, self._input_workers, [input_context], self._container_strategy()
        )
    ...
    def value_container(self, val):
        if hasattr(val, "_aggregating_container") and not isinstance(
            val, values.AggregatingVariable
        ):
