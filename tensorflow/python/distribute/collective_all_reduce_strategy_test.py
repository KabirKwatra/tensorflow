...
class CollectiveAllReduceStrategyTestBase(
    ...
    def _test_minimize_loss_graph(self, task_type, task_id, num_gpus):
        ...
        with ops.Graph().as_default(), self.cached_session(
                config=config, target=master_target) as sess, d.scope():
            l = core.Dense(1,
                           use_bias=False,
    ...
    def _test_complex_model(self, task_type, task_id, num_gpus):
        ...
        with ops.Graph().as_default(), self.cached_session(
                config=config, target=master_target) as sess:
            with d.scope():
    ...
    def _test_mixed_precision(self, task_type, task_id, num_gpus):
        ...
        with ops.Graph().as_default(), self.cached_session(
            ...
            loss_multiplier_for_first_worker = variables.Variable(
                1.0, dtype="float16", trainable=False)
            with d.scope():
                model = keras.Sequential([
                ...
                def model_fn():
                    ...
                    if (task_type == "worker"
                            and task_id == 0) or task_type is task_id is None:
    ...
    def _test_variable_initialization(self, task_type, task_id, num_gpus):
        ...
        with ops.Graph().as_default(), self.cached_session(
                config=config,
                target=master_target) as sess, distribution.scope():
            ...
            reduced_x = distribution.reduce(reduce_util.ReduceOp.MEAN,
                                            x,
                                            axis=None)
            ...
            self.assertTrue(
                ...
                msg=("x_value = %r, reduced_x_value = %r" %
                     (x_value, reduced_x_value)),
            )
...
class DistributedCollectiveAllReduceStrategyTest(
    ...
    def testUpdateConfigProto(self):
        strategy, _, _ = self._get_test_object(task_type="worker",
                                               task_id=1,
                                               num_gpus=2)
    ...
    def testEnableCollectiveOps(self):
        ...
        with test.mock.patch.object(
                context.context(), "enable_collective_ops",
                mock_enable_collective_ops), test.mock.patch.object(
...
class LocalCollectiveAllReduceStrategy(
    ...
    def testMakeInputFnIterator(self, required_gpus, use_dataset):
        if use_dataset:
            ...
            def fn():
                return dataset_ops.Dataset.range(5 * required_gpus)
    ...
    @combinations.generate(
        combinations.combine(mode=["graph"], required_gpus=2))
    ...
    def testNumpyDataset(self, required_gpus):
        ...
        self._test_numpy_dataset(strategy,
                                 session=self.cached_session(config=config,
                                                             target=target))
