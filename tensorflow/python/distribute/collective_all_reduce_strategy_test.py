...
class CollectiveAllReduceStrategyTestBase(multi_worker_test_base.MultiWorkerTestBase):
    ...
    def _test_minimize_loss_graph(self, task_type, task_id, num_gpus):
        ...
        with ops.Graph().as_default(), self.cached_session(
            ...
            l = core.Dense(
                1, use_bias=False, name="gpu_%d" % d.extended._num_gpus_per_worker
            )
                ...
                model = keras.Sequential(
                    [
                        mp_test_util.MultiplyLayer(
                            assert_type=dtypes.float16, input_shape=(1,)
                        ),
                    ]
                )
...
class DistributedCollectiveAllReduceStrategyTest(
    ...
    def testMixedPrecision(self, required_gpus):
        ...
        with policy.policy_scope("mixed_float16"):
            self._run_between_graph_clients(
                self._test_mixed_precision, self._cluster_spec, num_gpus=required_gpus
            )
    ...
    def testEnableCollectiveOps(self):
        ...
        with test.mock.patch.object(
            context.context(), "enable_collective_ops", mock_enable_collective_ops
        ), test.mock.patch.object(
            context.context(), "configure_collective_ops", mock_configure_collective_ops
        ):
            strategy, _, _ = self._get_test_object(
                task_type="worker", task_id=1, num_gpus=2
            )
...
class LocalCollectiveAllReduceStrategy(
    ...
    @combinations.generate(combinations.combine(mode=["graph"], required_gpus=[2, 4]))
    def testComplexModel(self, required_gpus):
