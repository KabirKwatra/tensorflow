...
class MinimizeLossStepTest(test.TestCase, parameterized.TestCase):
    ...
    def testTrainNetwork(self, distribution, optimizer_fn, use_callable_loss):
        with distribution.scope():
            ...
            error = abs(numpy.add(numpy.squeeze(
                weights), numpy.squeeze(biases)) - 1)
    ...
    def testRunStepsWithOutputContext(self, distribution, optimizer_fn, is_tpu):
        with distribution.scope():
            ...
            def run_step():
                def initial_loss(): return constant_op.constant(1e7)
            ...
            loss_is_not_increasing = all(
                y <= x for x, y in zip(losses, losses[1:]))
