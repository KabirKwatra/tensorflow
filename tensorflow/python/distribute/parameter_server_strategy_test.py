...
class ParameterServerStrategyTestBase(
    ...
    def _test_device_assignment_distributed(self, task_type, task_id, num_gpus):
        ...
        with ops.Graph().as_default(), \
            ...
            def model_fn():
                ...
                c = a + b
                self.assertEqual(a.device, worker_device +
                                 '/' + last_part_device)
                self.assertEqual(b.device, worker_device +
                                 '/' + last_part_device)
                self.assertEqual(c.device, worker_device +
                                 '/' + last_part_device)
                ...
                with ops.control_dependencies([z_add]):
                    f = z + c
                self.assertEqual(f.device, worker_device +
                                 '/' + last_part_device)
    ...
    def _test_device_assignment_local(self,
        ...
        with ops.Graph().as_default(), \
            ...
            def model_fn():
                ...
                self.assertEqual(x_add.device, x.device)
                self.assertEqual(
                    e.device, device_util.canonicalize('/device:GPU:2'))
                ...
                with ops.device('/CPU:1'):
                    g = e + 1.0
                self.assertEqual(
                    g.device, device_util.canonicalize('/device:CPU:1'))
    ...
    def _test_simple_increment(self, task_type, task_id, num_gpus):
        ...
        with ops.Graph().as_default(), \
            ...
            self.assertEqual(x_val, 10.0 + 1.0 *
                             num_workers * d.num_replicas_in_sync)
            self.assertEqual(y_val, 20.0 + 1.0 *
                             num_workers * d.num_replicas_in_sync)
    ...
    def _test_minimize_loss_graph(self, task_type, task_id, num_gpus):
        ...
        if task_type:
            # Multi-worker
            assert hasattr(
                d.extended, '_cluster_spec') and d.extended._cluster_spec
            num_workers = len(d.extended._cluster_spec.as_dict().get(WORKER))
            if CHIEF in d.extended._cluster_spec.as_dict():
                num_workers += 1
        else:
...
class ParameterServerStrategyTest(
    ...
    def testMakeInputFnIteratorDistributed(self, required_gpus, use_dataset):
        if use_dataset:
            def fn(): return dataset_ops.Dataset.range(100)
        else:
            def fn():
