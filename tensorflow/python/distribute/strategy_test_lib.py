...
def _events_from_logdir(test_case, logdir):
    ...
    records = list(tf_record.tf_record_iterator(os.path.join(logdir,
                                                             files[0])))
    result = []
    for r in records:
...
class DistributionTestBase(test.TestCase):
    ...
    def _test_minimize_loss_eager(self, d):
        with d.scope():
            ...
            def loss(x):
                y = array_ops.reshape(l(x), []) - constant_op.constant(1.0)
                return y * y
    ...
    def _test_minimize_loss_graph(self,
        ...
        ), self.cached_session(config=config) as sess, d.scope():
            l = core.Dense(1, use_bias=False)
    ...
    def _test_call_and_merge_exceptions(self, dist):
        with dist.scope():
            ...
            with self.assertRaises(_TestException):
                dist.extended.call_for_each_replica(
    ...
    def _input_fn_to_test_input_context(
        ...
        def _input_fn(input_context):
            ...
            if expected_input_pipeline_id is not None:
                ...
                self.assertEqual(worker_id_counter[0],
                                 input_context.input_pipeline_id)
    ...
    def _test_input_fn_iterable(self,
                                strategy,
                                input_fn,
                                expected_values,
                                ignore_order=False):
        ...
        iterable = strategy.experimental_distribute_datasets_from_function(
            input_fn)
        if context.executing_eagerly():
            ...
            with self.assertRaises(StopIteration):
                self.evaluate(
                    strategy.experimental_local_results(next(iterator)))
    ...
    def _test_global_step_update(self, strategy):
        with strategy.scope():
            ...
            train_ops, value = strategy.extended.call_for_each_replica(
                model_fn)
            ...
            self.assertEqual((1, ) * len(global_step_tensors),
                             global_step_values)
...
class OneDeviceDistributionTestBase(test.TestCase):
    ...
    def _test_run(self, strategy):
        out1 = strategy.run(lambda: constant_op.constant(4.0))
    ...
    def _test_all_reduce_sum(self, strategy):
        self._test_collective_comms(strategy,
                                    _all_sum,
                                    inputs=(4.0, [42.0, 43.0]),
                                    expected=(4.0, [42.0, 43.0]))

    def _test_all_reduce_sum_gradients(self, strategy):
        self._test_collective_comms_gradients(strategy,
                                              _all_sum,
                                              inputs=[4.0],
                                              expected_grads=[4.0])

    def _test_all_reduce_sum_gradient_tape(self, strategy):
        self._test_collective_comms_gradient_tape(strategy,
                                                  _all_sum,
                                                  inputs=[4.0],
                                                  expected_grads=[4.0])
    ...
    def _test_collective_comms(self, strategy, comm_fn, inputs, expected):
        inputs = strategy.make_input_fn_iterator(lambda _: dataset_ops.Dataset.
                                                 from_tensors(inputs))
...
class TwoDeviceDistributionTestBase(test.TestCase):
    ...
    def _test_run(self, strategy):
        out1 = strategy.run(lambda: ds_context.get_replica_context().
...
class RemoteSingleWorkerMirroredStrategyBase(DistributionTestBase):
    ...
    def _testNumReplicasInSync(self, distribution):
        self.assertEqual(self._get_num_gpus(),
