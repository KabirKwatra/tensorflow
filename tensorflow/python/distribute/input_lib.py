...
def get_distributed_datasets_from_function(dataset_fn, input_workers,
    ...
    if tf2.enabled():
        ...
        return DistributedDatasetsFromFunctionV1(dataset_fn, input_workers,
                                                 input_contexts, strategy)
...
class InputWorkers(object):
    ...
    def __init__(self, worker_device_pairs):
        ...
        self._input_worker_devices = tuple(
            d for d, _ in self._worker_device_pairs)
    ...
    for i, worker in enumerate(iterator._input_workers.worker_devices):  # pylint: disable=protected-access
        ...
        with ops.device(worker):
            worker_has_value, next_element = iterator._iterators[
                i].get_next_as_list(new_name)  # pylint: disable=protected-access
...
def _get_static_shape(iterators):
    ...
    for iterator in iterators:
        if not isinstance(
                iterator,
            (_SingleWorkerOwnedDatasetIterator, _SingleWorkerDatasetIterator)):
            continue
...
class DistributedIteratorBase(object):
    ...
    def __init__(self, input_workers, iterators, strategy):
        ...
        if not input_workers.worker_devices:
            raise ValueError(
                "Should have at least one worker for input iterator.")
    ...
    def get_next(self, name=None):
        ...
        for i, replica_data in enumerate(nest.flatten(out_of_range_replicas)):
            for target, source in zip(
                    nest.flatten(flattened_replicas[i],
            ...
            if sparse_tensor.is_sparse(
                    replica_data) and replica_data.get_shape():
...
class DistributedIteratorSpec(type_spec.TypeSpec):
    ...
    def __init__(self, input_workers, element_spec, strategy):
        ...
        if isinstance(input_workers, tuple):
            raise NotImplementedError(
                "DistributedIteratorSpec does not have support "
                "for deserialization.")
        else:
            self._input_workers = input_workers
    ...
    def _is_compatible(a, b):
        ...
        if isinstance(a, tuple):
            return len(a) == len(b) and all(
                DistributedIteratorSpec._is_compatible(x, y)
                for (x, y) in zip(a, b))
        if isinstance(a, dict):
            return (len(a) == len(b) and sorted(a.keys()) == sorted(b.keys())
                    and all(
                        DistributedIteratorSpec._is_compatible(a[k], b[k])
                        for k in a.keys()))
        if isinstance(
                a,
            (type_spec.TypeSpec, tensor_shape.TensorShape, dtypes.DType)):
    ...
    def from_value(value):
        # pylint: disable=protected-access
        return DistributedIteratorSpec(value._input_workers,
                                       value._element_spec, value._strategy)
...
class _IterableInput(object):
    ...
    def reduce(self, initial_state, reduce_fn):
        ...
        has_data, data, final_state = control_flow_ops.while_loop(
            cond,
            loop_body, [has_data, data, initial_state],
...
class DistributedDataset(_IterableInput):
    ...
    def __init__(self,
        ...
        if split_batch_by:
            try:
                ...
                with ops.colocate_with(dataset._variant_tensor):
                    dataset = distribute._RebatchDataset(
                        dataset, split_batch_by)
        ...
        self._element_spec = _create_distributed_tensor_spec(
            self._strategy, dataset.element_spec)  # pylint: disable=protected-access
    ...
    def __iter__(self):
        ...
        worker_iterators = _create_iterators_per_worker(
            self._cloned_datasets, self._input_workers,
            enable_legacy_iterators)
        if enable_legacy_iterators:
            iterator = DistributedIteratorV1(self._input_workers,
                                             worker_iterators, self._strategy)
        else:
            iterator = DistributedIterator(self._input_workers,
                                           worker_iterators, self._strategy)
    ...
    def __init__(self, dataset_fn, input_workers, input_contexts, strategy):
        ...
        Args:
          ...
          strategy: a `tf.distribute.Strategy` object, used to run all-reduce to
            handle last partial batch.
        """
        super(DistributedDatasetsFromFunction,
        ...
        for i, ctx in enumerate(input_contexts):
            ...
            with ops.device(worker):
                ...
                if isinstance(result, dataset_ops.DatasetV2):
                    ...
                    iterator = _SingleWorkerCallableIterator(
                        result, worker, devices)
                else:
                    raise ValueError(
    ...
    def create_dummy_tensor(spec):
        ...
        dims = (
            [dim if dim is not None else 0
             for dim in feature_shape.as_list()] if feature_shape else [])
        if dims and (isinstance(spec, ragged_tensor.RaggedTensorSpec)
        ...
        if isinstance(spec, ragged_tensor.RaggedTensorSpec):
            ...
            dummy_tensor = ragged_tensor.RaggedTensor.from_nested_row_splits(
                dummy_tensor, (row_splits, ) * spec._ragged_rank,
                validate=False)
            # pylint: enable=protected-access
        return dummy_tensor
    ...
    def __init__(
        ...
        if dataset is None:
            ...
            super(_SingleWorkerOwnedDatasetIterator,
                  self).__init__(dataset, worker, devices)

    def _make_iterator(self):
        """Make appropriate iterator on the dataset."""
        if not self._worker:
            raise ValueError(
                "Worked device must be specified when creating an "
                "owned iterator.")
...
def _get_batched_dataset(d):
    ...
    if isinstance(d, (dataset_ops.BatchDataset, batching._MapAndBatchDataset)):
        return d
            ...
            d, (dataset_ops.PrefetchDataset, dataset_ops._OptionsDataset)):
...
def _get_dataset_attributes(dataset):
    ...
    batch_size, drop_remainder = _get_batched_dataset_attributes(
        batched_dataset)
    ...
    def set_non_tensor_output(self, name, output):
        ...
        if distribution_strategy_context.in_cross_replica_context():
            ...
            def merge_fn(distribution, value):
                ...
                self._non_tensor_outputs[
                    name] = distribution.experimental_local_results(value)
