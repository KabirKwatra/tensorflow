...
def _call_concrete_function(function, inputs):
    ...
    result = function._call_flat(
        tensor_inputs, function._captured_inputs)  # pylint: disable=protected-access
...
def _concrete_function_callable_with(function, inputs, allow_conversion):
    ...
    for arg, expected in zip(flatten_inputs, nest.flatten(expected_structure)):
        if isinstance(expected, tensor_spec.TensorSpec):
            if allow_conversion:
                arg = _try_convert_to_tensor_spec(
                    arg, dtype_hint=expected.dtype)
...
def recreate_function(saved_function, concrete_functions):
    ...
    for cf in concrete_function_objects:
        cf._set_function_spec(
            function_spec)  # pylint: disable=protected-access
...
def load_function_def_library(library, load_shared_name_suffix=None):
    ...
    library_function_names = set(
        fdef.signature.name for fdef in library.function)
    functions = {}
    renamed_functions = {}
...
def _fix_fdef(orig_fdef, functions, shared_name_suffix):
    ...
    for node_def in fdef.node_def:
        fix_node_def(node_def, functions,
                     shared_name_suffix, fdef.signature.name)
