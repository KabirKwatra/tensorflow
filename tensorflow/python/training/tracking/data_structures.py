...
def _should_wrap_tuple(t):
    ...
    for element in t:
        if isinstance(element, NoDependency):
            # We should remove the NoDependency object from the tuple.
            return True
...
class List(TrackableDataStructure, collections_abc.Sequence):
    ...
    def append(self, value):
        ...
        value = self._track_value(value,
                                  self._name_element(len(self._storage)))
...
class ListWrapper(
    ...
    def _has_mutation_or_trackable(self):
        ...
        if self._non_append_mutation:
            return True
        return any(
            isinstance(element, base.Trackable) for element in self._storage)
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(key, slice):
            ...
            if isinstance(self._storage[key], base.Trackable):
                self._non_append_mutation = True
            self._storage[key] = self._track_value(value,
                                                   self._name_element(key))
    ...
    def insert(self, index, obj):
        self._check_external_modification()
        if self._has_mutation_or_trackable() or isinstance(
                obj, base.Trackable):
            self._non_append_mutation = True
    ...
    def _track_value(self, value, name):
        ...
        try:
            value = super(ListWrapper, self)._track_value(value=value,
                                                          name=name)
        except ValueError:
    ...
    def _list_functions_for_serialization(self, unused_functions):
        return {
            str(key): value
            for key, value in enumerate(self) if _is_function(value)
        }
...
class _DictWrapper(TrackableDataStructure, wrapt.ObjectProxy):
    ...
    def __getattribute__(self, name):
        if hasattr(type(self), name) and isinstance(getattr(type(self), name),
                                                    property):
    ...
    def _checkpoint_dependencies(self):
        ...
        # Any reason for dirtiness should have an exception.
    ...
    def _track_value(self, value, name):
        ...
        try:
            ...
            value = super(_DictWrapper, self)._track_value(value=value,
                                                           name=name)
            ...
            # Even if this value isn't trackable, we need to make sure
            # NoDependency objects get unwrapped.
            return sticky_attribute_assignment(trackable=self,
                                               value=value,
                                               name=name)
...
class _TupleWrapper(TrackableDataStructure, wrapt.ObjectProxy):
    ...
    def __init__(self, original_wrapped_tuple=()):
        ...
        if is_namedtuple:
            ...
            # dictionaries.
            for name, should_depend, element in zip(fields, add_dependency,
                                                    substituted_wrapped_tuple):
                if should_depend:
    ...
    def _track_value(self, value, name):
        """Allows storage of non-trackable objects."""
        try:
            value = super(_TupleWrapper, self)._track_value(value=value,
                                                            name=name)
        except ValueError:
            # Even if this value isn't trackable, we need to make sure
            # NoDependency objects get unwrapped.
