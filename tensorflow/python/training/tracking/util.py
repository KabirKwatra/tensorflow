    ...
    def node_names(self):
        ...
        for node_id, node in enumerate(self._object_graph_proto.nodes):
            for slot_reference in node.slot_variables:
                node_names[
                    slot_reference.
                    slot_variable_node_id] = "{}'s state '{}' for {}".format(
                        node_names[node_id],
                ...
                self.slot_restorations.setdefault(
                    slot_reference.original_variable_node_id, []).append(
                        base._SlotVariableRestoration(  # pylint: disable=protected-access
                            ...
                            slot_variable_id=slot_reference.
...
class _NameBasedRestoreCoordinator(object):
    ...
    def globally_named_object_attributes(self, trackable):
        ...
        for (
                attribute_name,
                saveable_factory,
        ) in (trackable._gather_saveables_for_checkpoint().items()):  # pylint: disable=protected-access
            if callable(saveable_factory):
                try:
                    ...
                    if not saveable_factory("").optional_restore:
                        self.unused_attributes.setdefault(
                            trackable, []).append(attribute_name)
                    continue
            else:
                saveable = saveable_factory
...
def _default_getter(name,
    ...
    with ops.init_scope():
        if initializer is None:
            ...
            )._get_default_initializer(  # pylint: disable=protected-access
                name=name,
                shape=shape_object,
                dtype=dtype)
        else:
        ...
        if initializing_from_value:
            ...
            def initial_value():
                return initializer(shape_object.as_list(),
                                   dtype=dtype,
                                   partition_info=partition_info)

        return variables.VariableV1(initial_value=initial_value,
                                    name=name,
...
def capture_dependencies(template):
    ...
    def _trackable_custom_creator(next_creator,
        ...
        def _call_next_creator_renaming_initializer(initializer,
                                                    **inner_kwargs):
            # Ignored; this is the scope-stripped name which
            inner_kwargs.pop("name")
            # we don't want to propagate.
            return next_creator(initial_value=initializer,
                                name=name,
                                **inner_kwargs)
...
class CheckpointLoadStatus(_LoadStatus):
    ...
    def assert_consumed(self):
        ...
        for node_id, node in enumerate(
                self._checkpoint.object_graph_proto.nodes):
        ...
        if self._checkpoint.unused_attributes:
            ...
            for node_id, attribute in six.iteritems(
                ...
                unused_attribute_messages.append("{} ({}): {}".format(
    ...
    def assert_existing_objects_matched(self):
        ...
        for node_id, node in enumerate(
            ...
            if (trackable is not None
                ...
                raise AssertionError(
                    "Object not assigned a value from checkpoint: %s" %
                    (node, ))
    ...
    def initialize_or_restore(self, session=None):
        ...
        initializers_for_non_restored_variables = [
            c.initializer for c in all_objects if hasattr(c, "initializer")
            and c not in already_initialized_objects and (
                getattr(c, "_update_uid", self._checkpoint.restore_uid -
                        1) < self._checkpoint.restore_uid)
        ]
...
class InitializationOnlyStatus(_LoadStatus):
    ...
    def initialize_or_restore(self, session=None):
        ...
        initializers = [
            c.initializer for c in trackable_objects
            if hasattr(c, "initializer") and c.initializer is not None and (
                getattr(c, "_update_uid", self._restore_uid -
                        1) < self._restore_uid)
        ]
...
class NameBasedSaverStatus(_LoadStatus):
    ...
    @deprecation.deprecated(date=None,
                            instructions=_DEPRECATED_RESTORE_INSTRUCTIONS)
    ...
    def assert_consumed(self):
        ...
        if unused_attributes:
            ...
            raise AssertionError(
                ...
                format("".join(unused_attribute_strings)))
...
class _SessionWithFeedDictAdditions(session_lib.SessionInterface):
    ...
    def run(self, fetches, feed_dict=None, **kwargs):
        ...
        return self._wrapped_session.run(fetches=fetches,
                                         feed_dict=feed_dict,
                                         **kwargs)
...
class TrackableSaver(object):
        ...
        if (self._last_save_object_graph != graph_proto
            ...
            with ops.device("/cpu:0"):
                with ops.control_dependencies([save_op]):
                    self._cached_save_operation = array_ops.identity(
                        file_prefix)
    ...
    def save(self, file_prefix, checkpoint_number=None, session=None):
        ...
        if use_session:
            ...
            with ops.device("/cpu:0"):
                file_prefix_tensor = constant_op.constant(file_prefix,
                                                          dtype=dtypes.string)
            object_graph_tensor = None
            ...
            if not graph_building:
                for existing_trackable in self._graph_view.list_objects():
                    ...
                    existing_trackable._name_based_attribute_restore(
                        restore_coordinator)
                    # pylint: enable=protected-access
            return NameBasedSaverStatus(restore_coordinator,
                                        graph_view=self._graph_view)

        if graph_building:
            if self._file_prefix_placeholder is None:
                with ops.device("/cpu:0"):
                    self._file_prefix_placeholder = constant_op.constant(
                        "model")
...
def saver_with_op_caching(obj):
    ...
    return TrackableSaver(
        graph_view_lib.ObjectGraphView(weakref.ref(obj),
...
class CheckpointV1(tracking.AutoTrackable):
    ...
    def __init__(self, **kwargs):
        ...
        for k, v in sorted(kwargs.items(), key=lambda item: item[0]):
            setattr(self, k, v)
            if not isinstance(getattr(self, k),
                              (base.Trackable, def_function.Function)):
                raise ValueError((
