...
class BaseSession(SessionInterface):
    ...
    def __init__(self, target='', graph=None, config=None):
        ...
        if graph is None:
            ...
            if not isinstance(graph, ops.Graph):
                raise TypeError(
                    'graph must be a tf.Graph, but got %s' % type(graph))
            self._graph = graph
        ...
        if target is not None:
            try:
                ...
                raise TypeError(
                    'target must be a string, but got %s' % type(target))
        else:
            self._target = None
        ...
        opts = tf_session.TF_NewSessionOptions(
            target=self._target, config=config)
        try:
        ...
        for feed in feeds:
            for subfeed in _feed_fn(feed):
                try:
                    ...
                    e.message = (
                        'Cannot interpret feed_list key as Tensor: ' + e.message)
                    e.args = (e.message,)
                    raise e
            ...
            for feed, feed_val in feed_dict.items():
                for subfeed, subfeed_val in _feed_fn(feed, feed_val):
                    ...
                    if not self.graph.is_feedable(subfeed_t):
                        raise ValueError(
                            'Tensor %s may not be fed.' % subfeed_t)

                    feed_dict_tensor[subfeed_t.ref()] = np_val
                    feed_map[compat.as_bytes(subfeed_t.name)] = (
                        subfeed_t, subfeed_val)
    ...
    def _do_run(self, handle, target_list, fetch_list, feed_dict, options,
        ...
        """
        # pylint: disable=protected-access
        feeds = dict((t.deref()._as_tf_output(), v)
