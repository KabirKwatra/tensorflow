...
class BackendVariableTest(test.TestCase):
    ...
    def test_random_uniform_variable(self):
        x = keras.backend.random_uniform_variable(
            (30, 20), low=1, high=2, seed=0)
        val = keras.backend.eval(x)
...
class BackendNNOpsTest(test.TestCase, parameterized.TestCase):
    ...
    def test_pool2d(self):
        ...
        with self.assertRaises(ValueError):
            y = keras.backend.pool2d(
                x, (2, 2), strides=(2, 2), pool_mode='other')
            ...
            for kernel_size in [1, 2]:
                for stride in [1, 2]:
                    ...
                    kernel_cl = np.reshape(
                        np.transpose(kernel, list(range(dim)) +
                                     [dim + 1, dim, dim + 2]),
                        kernel_shape
                    )
    ...
    def test_rnn(self):
        ...
        def rnn_step_fn():
            ...
            def step_function(x, states):
                ...
                output = keras.backend.dot(
                    x, w_i) + keras.backend.dot(prev_output, w_o)
    ...
    def test_rnn_output_num_dim_larger_than_2_masking(self):
        ...
        def step_function(inputs, states):
            outputs = keras.backend.tile(
                keras.backend.expand_dims(inputs), [1, 1, 2])
...
class BackendCrossEntropyLossesTest(test.TestCase):
    ...
    def test_categorical_crossentropy_loss_with_unknown_rank_tensor(self):
        ...
        o = keras.backend.categorical_crossentropy(
            t, p, from_logits=True, axis=0)
        f = keras.backend.function([t, p], o)
            ...
            inputs = np.asarray(
                [[[0.633766, 0.221185, 0.0917319, 0.0129757, 0.0142857, 0.0260553], [
                    ...
                    [0.0663296, 0.643849, 0.280111,
                        0.00283995, 0.0035545, 0.00331533],
                    [0.458235, 0.396634, 0.123377, 0.00648837, 0.00903441, 0.00623107]]
                ],
                dtype=np.float32)
...
class ControlOpsTests(test.TestCase):
    ...
    def test_function_switch_basics(self):
        ...
        tensor = keras.backend.switch(
            keras.backend.greater(x, y), xpowy, ypowx)
    ...
    def test_unequal_rank(self):
        ...
        with self.assertRaisesRegexp(ValueError,
            ...
            keras.backend.switch(keras.backend.equal(
                x, x), false_func, true_func)
