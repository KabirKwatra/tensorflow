...
class RNNCell(base_layer.Layer):
    ...
    def __init__(self, trainable=True, name=None, dtype=None, **kwargs):
        super(RNNCell, self).__init__(trainable=trainable,
                                      name=name,
                                      dtype=dtype,
                                      **kwargs)
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        if scope is not None:
            with vs.variable_scope(
                ...
                return super(RNNCell, self).__call__(inputs,
                                                     state,
                                                     scope=scope)
        else:
    ...
    def _rnn_get_variable(self, getter, *args, **kwargs):
        ...
        if trainable and all(variable is not v
                             for v in self._trainable_weights):
            self._trainable_weights.append(variable)
        elif not trainable and all(variable is not v
                                   for v in self._non_trainable_weights):
    ...
    def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
        if inputs is not None:
            ...
            if batch_size is not None:
                ...
                if inputs.shape.dims[0].value != static_batch_size:
                    raise ValueError(
                        ...
                        "input param. Input tensor batch: {}, batch_size: {}".
                        format(inputs.shape.dims[0].value, batch_size))
            ...
            batch_size = inputs.shape.dims[0].value or array_ops.shape(
                inputs)[0]
            dtype = inputs.dtype
...
class LayerRNNCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None, *args, **kwargs):
        ...
        return base_layer.Layer.__call__(self,
                                         inputs,
                                         state,
                                         scope=scope,
                                         *args,
...
class BasicRNNCell(LayerRNNCell):
    ...
    @deprecated(
        None,
        "This class is equivalent as tf.keras.layers.SimpleRNNCell,"
...
class GRUCell(LayerRNNCell):
    ...
    @deprecated(
        None,
        "This class is equivalent as tf.keras.layers.GRUCell,"
        " and will be replaced by that in Tensorflow 2.0.",
    )
    def __init__(self,
                 num_units,
    ...
    def call(self, inputs, state):
        ...
        candidate = math_ops.matmul(array_ops.concat([inputs, r_state], 1),
...
class BasicLSTMCell(LayerRNNCell):
    ...
    @deprecated(
        None,
        "This class is equivalent as tf.keras.layers.LSTMCell,"
        " and will be replaced by that in Tensorflow 2.0.",
    )
    def __init__(self,
                 num_units,
...
class LSTMCell(LayerRNNCell):
    ...
    def call(self, inputs, state):
        ...
        # i = input_gate, j = new_input, f = forget_gate, o = output_gate
        lstm_matrix = math_ops.matmul(array_ops.concat([inputs, m_prev], 1),
                                      self._kernel)
        ...
        if self._use_peepholes:
            c = sigmoid(f + self._forget_bias +
                        self._w_f_diag * c_prev) * c_prev + sigmoid(
...
class _RNNCellWrapperV1(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        Returns:
          ...
          - New state: A tensor or tuple of tensors with new wrapped cell's state.
        """
        return self._call_wrapped_cell(inputs,
                                       state,
                                       cell_call_fn=self.cell.__call__,
                                       scope=scope)
...
class MultiRNNCell(RNNCell):
    ...
    def __init__(self, cells, state_is_tuple=True):
        ...
        if not nest.is_sequence(cells):
            raise TypeError("cells must be a list or tuple, but saw: %s." %
                            cells)
