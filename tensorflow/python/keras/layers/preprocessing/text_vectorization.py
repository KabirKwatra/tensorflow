...
@keras_export("keras.layers.experimental.preprocessing.TextVectorization",
              v1=[])
class TextVectorization(CombinerPreprocessingLayer):
    ...
    def __init__(self,
        ...
        if output_mode == INT and not (isinstance(output_sequence_length, int)
    ...
    def compute_output_signature(self, input_spec):
        ...
        output_dtype = K.floatx(
        ) if self._output_mode == TFIDF else dtypes.int64
    ...
    def adapt(self, data, reset_state=True):
        ...
        if not reset_state:
            raise ValueError(
                "TextVectorization does not support streaming adapts.")
        ...
        if isinstance(data, np.ndarray):
            ...
            if not isinstance(shape, tensor_shape.TensorShape):
                raise ValueError(
                    ...
                    "tensor value.")
            if shape.rank == 1:
                data = data.map(lambda tensor: array_ops.expand_dims(
                    tensor, -1))
            self.build(dataset_ops.get_legacy_output_shapes(data))
            preprocessed_inputs = data.map(self._preprocess)
        else:
            raise ValueError(
                "adapt() requires a Dataset or a Numpy array as input, got {}".
                format(type(data)))
...
class _TextVectorizationCombiner(Combiner):
        ...
        for accumulator in accumulators[1:]:
            ...
            if self._compute_idf:
                for token, value in accumulator.per_doc_count_dict.items():
                    ...
                    base_accumulator.per_doc_count_dict[token][
                        "count"] += value["count"]
    ...
    def extract(self, accumulator):
        ...
        if self._compute_idf:
            ...
            idf = self._inverse_document_frequency(doc_counts,
                                                   num_documents[0])
