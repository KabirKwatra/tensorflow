...
class CenterCropTest(keras_parameterized.TestCase):
    ...
    def test_invalid_center_crop(self, expected_height, expected_width):
        with self.assertRaisesRegexp(errors.InvalidArgumentError, r"assertion failed"):
            self._run_test(expected_height, expected_width)
...
class RandomFlipTest(keras_parameterized.TestCase):
    def _run_test(
        self, flip_horizontal, flip_vertical, expected_output=None, mock_random=None
    ):
        np.random.seed(1337)
        num_samples = 2
        orig_height = 5
...
class RandomTranslationTest(keras_parameterized.TestCase):
    ...
    def test_random_translation_reflect(self):
        if compat.forward_compatible(2020, 3, 25):
            ...
            self._run_random_translation_with_mock(
                expected_output, mock_offset, height_factor, width_factor, "reflect"
            )
    ...
    def test_random_translation_wrap(self):
        if compat.forward_compatible(2020, 3, 25):
            ...
            # pyformat: enable
            self._run_random_translation_with_mock(
                expected_output, mock_offset, height_factor, width_factor, "wrap"
            )
    ...
    def test_random_translation_constant(self):
        if compat.forward_compatible(2020, 3, 25):
            ...
            # pyformat: enable
            self._run_random_translation_with_mock(
                expected_output, mock_offset, height_factor, width_factor, "constant"
            )
    ...
    def test_random_translation_negative_lower(self):
        ...
        with test.mock.patch.object(
            ...
            with self.cached_session(use_gpu=True):
                ...
                layer_2 = image_preprocessing.RandomTranslation((0.2, 0.3), 0.4)
