...
class BatchNormalizationBase(Layer):
    ...
    def _get_trainable_var(self):
        if self._trainable_var is None:
            self._trainable_var = K.freezable_variable(self._trainable,
                                                       name=self.name +
                                                       "_trainable")
    ...
    def _fused_batch_norm(self, inputs, training):
        ...
        use_fused_avg_updates = (compat.forward_compatible(2020, 3, 6)
        ...
        def _maybe_add_or_remove_bessels_correction(variance, remove=True):
            ...
            if remove:
                ...
                factor = sample_size / (sample_size -
                                        math_ops.cast(1.0, variance.dtype))
    ...
    def _get_training_value(self, training=None):
        ...
        if self._USE_V2_BEHAVIOR:
            ...
            if base_layer_utils.is_in_keras_graph():
                training = math_ops.logical_and(training,
                                                self._get_trainable_var())
        ...
        if (training_value == False):  # pylint: disable=singleton-comparison,g-explicit-bool-comparison
            ...
            if self.adjustment:
                ...
                adj_scale = tf_utils.smart_cond(
                    training, lambda: adj_scale, lambda: array_ops.ones_like(
                        adj_scale))
                adj_bias = tf_utils.smart_cond(
                    training, lambda: adj_bias, lambda: array_ops.zeros_like(
                        adj_bias))
            ...
            mean = tf_utils.smart_cond(
                training, lambda: mean, lambda: ops.convert_to_tensor_v2(
                    moving_mean))
...
class LayerNormalization(Layer):
    ...
    def build(self, input_shape):
        ndims = len(input_shape)
        if ndims is None:
            raise ValueError("Input shape %s has undefined rank." %
                             input_shape)
    ...
    def call(self, inputs):
        ...
        def _broadcast(v):
            if v is not None and len(
                    v.shape) != ndims and self.axis != [ndims - 1]:
                return array_ops.reshape(v, broadcast_shape)
            return v
