...
class BatchNormalizationBase(Layer):
    ...
    def _raise_if_fused_cannot_be_used(self):
        ...
        if self.renorm:
            raise ValueError(
                "Passing both fused=True and renorm=True is " "unsupported"
            )
        ...
        if len(axis) > 1 or axis[0] not in (-3, -1, 1, 3):
            raise ValueError(
                "Passing fused=True is only supported when axis is 1 " "or 3"
            )
        if self.virtual_batch_size is not None:
            raise ValueError(
                "Passing fused=True is unsupported when "
                "virtual_batch_size is specified."
            )
        if self.adjustment is not None:
            raise ValueError(
                "Passing fused=True is unsupported when " "adjustment is specified."
            )
    ...
    def build(self, input_shape):
        ...
        for x in axis_to_dim:
            if axis_to_dim[x] is None:
                raise ValueError(
                    "Input has undefined `axis` dimension. Input shape: ", input_shape
                )
    ...
    def _renorm_correction_and_moments(self, mean, variance, training, inputs_size):
        ...
        d = tf_utils.smart_cond(training, lambda: d, lambda: array_ops.zeros_like(d))
    ...
    def _moments(self, inputs, reduction_axes, keep_dims):
        ...
        if self._support_zero_size_input():
            ...
            variance = array_ops.where(
                input_batch_size > 0, variance, K.zeros_like(variance)
            )
...
@keras_export(
    v1=["keras.layers.BatchNormalization"]
