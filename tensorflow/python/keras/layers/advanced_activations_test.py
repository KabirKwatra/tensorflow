...
class AdvancedActivationsTest(keras_parameterized.TestCase):
    def test_leaky_relu(self):
        for alpha in [0.0, 0.5, -1.0]:
            testing_utils.layer_test(
                keras.layers.LeakyReLU, kwargs={"alpha": alpha}, input_shape=(2, 3, 4)
            )

    def test_prelu(self):
        testing_utils.layer_test(keras.layers.PReLU, kwargs={}, input_shape=(2, 3, 4))

    def test_prelu_share(self):
        testing_utils.layer_test(
            keras.layers.PReLU, kwargs={"shared_axes": 1}, input_shape=(2, 3, 4)
        )

    def test_elu(self):
        for alpha in [0.0, 0.5, -1.0]:
            testing_utils.layer_test(
                keras.layers.ELU, kwargs={"alpha": alpha}, input_shape=(2, 3, 4)
            )

    def test_thresholded_relu(self):
        testing_utils.layer_test(
            keras.layers.ThresholdedReLU, kwargs={"theta": 0.5}, input_shape=(2, 3, 4)
        )

    def test_softmax(self):
        testing_utils.layer_test(
            keras.layers.Softmax, kwargs={"axis": 1}, input_shape=(2, 3, 4)
        )

    def test_relu(self):
        testing_utils.layer_test(
            keras.layers.ReLU, kwargs={"max_value": 10}, input_shape=(2, 3, 4)
        )
    ...
    def test_relu_with_invalid_arg(self):
        ...
        with self.assertRaisesRegexp(
            ValueError, "negative_slope of Relu layer cannot be negative value: -2"
        ):
