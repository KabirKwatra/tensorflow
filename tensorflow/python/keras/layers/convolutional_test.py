...
class Conv1DTest(keras_parameterized.TestCase):
    ...
    def test_conv1d_constraints(self):
        def k_constraint(x): return x
        def b_constraint(x): return x
                ...
                for length_row in [2]:
                    for length_col in [2, 3]:
                        ...
                        if context.executing_eagerly():
                            ...
                            np_output = keras.backend.eval(output)
                        if data_format == 'channels_first':
                            assert np_output.shape[2] == length_row * \
                                input_num_row
                            assert np_output.shape[3] == length_col * \
                                input_num_col
                        else:  # tf
                            assert np_output.shape[1] == length_row * \
                                input_num_row
                            assert np_output.shape[2] == length_col * \
                                input_num_col

                        # compare with numpy
                        if data_format == 'channels_first':
                            expected_out = np.repeat(
                                inputs, length_row, axis=2)
                            expected_out = np.repeat(
                                expected_out, length_col, axis=3)
                        else:  # tf
                            expected_out = np.repeat(
                                inputs, length_row, axis=1)
                            expected_out = np.repeat(
