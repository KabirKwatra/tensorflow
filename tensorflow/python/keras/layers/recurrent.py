...
class StackedRNNCells(Layer):
    ...
    def build(self, input_shape):
        ...
        for cell in self.cells:
            ...
            input_shape = tuple(
                [input_shape[0]] + tensor_shape.as_shape(output_dim).as_list()
            )
        self.built = True
    ...
    def get_config(self):
        ...
        for cell in self.cells:
            cells.append(
                {"class_name": cell.__class__.__name__, "config": cell.get_config()}
            )
        config = {"cells": cells}
...
class RNN(Layer):
    ...
    def _validate_state_spec(cell_state_sizes, init_state_specs):
        ...
        for i in range(len(flat_cell_state_size)):
            if not tensor_shape.TensorShape(
                ...
                flat_state_spec[i].shape[1:]
            ).is_compatible_with(tensor_shape.TensorShape(flat_cell_state_size[i])):
                raise validation_error
    ...
    def __call__(self, inputs, initial_state=None, constants=None, **kwargs):
        ...
        is_keras_tensor = (
            K.is_keras_tensor(flat_additional_inputs[0])
    ...
    def reset_states(self, states=None):
        ...
        if nest.flatten(self.states)[0] is None:
            ...
            for state, size in zip(
                ...
                K.set_value(
                    state,
                    np.zeros([batch_size] + tensor_shape.as_shape(size).as_list()),
                )
        else:
...
class PeepholeLSTMCell(LSTMCell):
    ...
    def _compute_carry_and_output(self, x, h_tm1, c_tm1):
        ...
        i = self.recurrent_activation(
            x_i
            + K.dot(h_tm1_i, self.recurrent_kernel[:, : self.units])
        ...
        o = self.recurrent_activation(
            x_o
            + K.dot(h_tm1_o, self.recurrent_kernel[:, self.units * 3 :])
