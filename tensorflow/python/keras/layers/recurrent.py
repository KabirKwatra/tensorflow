...
class RNN(Layer):
    ...
    def compute_output_shape(self, input_shape):
        ...
        def _get_output_shape(flat_output_size):
            ...
            if self.return_sequences:
                if self.time_major:
                    output_shape = tensor_shape.as_shape(
    ...
    def build(self, input_shape):
        ...
        def get_input_spec(shape):
            ...
            batch_index, time_step_index = (
                1, 0) if self.time_major else (0, 1)
        ...
        if not nest.is_sequence(input_shape):
            ...
            step_input_shape = nest.map_structure(
    ...
    def call(self,
        ...
        is_tf_rnn_cell = getattr(
            self.cell, '_is_tf_rnn_cell', None) is not None
        # Use the __call__ function for callable objects, eg layers, so that it
        # will have the proper name scopes for the ops, etc.
        ...
        if constants:
            ...
            def step(inputs, states):
                ...
                states = states[:-
                                self._num_constants]  # pylint: disable=invalid-unary-operand-type
        ...
        if self.return_sequences:
            output = K.maybe_convert_to_ragged(
    ...
    def from_config(cls, config, custom_objects=None):
        from tensorflow.python.keras.layers import deserialize as deserialize_layer  # pylint: disable=g-import-not-at-top
        cell = deserialize_layer(config.pop(
            'cell'), custom_objects=custom_objects)
...
class PeepholeLSTMCell(LSTMCell):
    ...
    def _compute_carry_and_output_fused(self, z, c_tm1):
        ...
        c = f * c_tm1 + i * self.activation(z2)
        o = self.recurrent_activation(
            z3 + self.output_gate_peephole_weights * c)
        return c, o
