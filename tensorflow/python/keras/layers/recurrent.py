...
class RNN(Layer):
    ...
    def __init__(self,
        ...
        if not "call" in dir(cell):
            raise ValueError(
                "`cell` should have a `call` method. "
    ...
    def compute_output_shape(self, input_shape):
        ...
        def _get_output_shape(flat_output_size):
            ...
            if self.return_sequences:
                if self.time_major:
                    output_shape = tensor_shape.as_shape([time_step, batch] +
                                                         output_dim)
                else:
                    output_shape = tensor_shape.as_shape([batch, time_step] +
                                                         output_dim)
            else:
    ...
    def get_initial_state(self, inputs):
        ...
        if get_initial_state_fn:
            ...
            init_state = _generate_zero_filled_state(batch_size,
    ...
    def __call__(self, inputs, initial_state=None, constants=None, **kwargs):
        ...
        is_keras_tensor = (K.is_keras_tensor(flat_additional_inputs[0])
                           if flat_additional_inputs else True)
        ...
        if is_keras_tensor:
            ...
            if self.built:
                ...
                full_input_spec = (generic_utils.to_list(
                    nest.map_structure(lambda _: None, inputs)) +
                                   additional_specs)
    ...
    def call(self,
        ...
        if self.stateful:
            updates = [
                ...
                for self_state, state in zip(nest.flatten(self.states),
                                             nest.flatten(states))
            ]
    ...
    def _process_inputs(self, inputs, initial_state, constants):
        ...
        if isinstance(inputs,
                      collections.Sequence) and not isinstance(inputs, tuple):
    ...
    def _validate_args_if_ragged(self, is_ragged_input, mask):
        ...
        if mask is not None:
            raise ValueError(
                "The mask that was passed in was " + str(mask) +
        ...
        if nest.flatten(self.states)[0] is None:
            ...
            def create_state_variable(state):
                return K.zeros([batch_size] +
                               tensor_shape.as_shape(state).as_list())

            self.states = nest.map_structure(create_state_variable,
                                             self.cell.state_size)
...
class AbstractRNNCell(Layer):
    ...
    def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
        return _generate_zero_filled_state_for_cell(self, inputs, batch_size,
                                                    dtype)
...
class DropoutRNNCellMixin(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # is used every time.
        self._dropout_mask_cache = K.ContextValueCache(
    ...
    def _create_dropout_mask(self, inputs, training, count=1):
        return _generate_dropout_mask(array_ops.ones_like(inputs),
                                      self.dropout,
                                      training=training,
                                      count=count)
    ...
    def get_recurrent_dropout_mask_for_cell(self, inputs, training, count=1):
        ...
        if self.recurrent_dropout == 0:
            return None
        init_kwargs = dict(inputs=inputs, training=training, count=count)
        return self._recurrent_dropout_mask_cache.setdefault(
            kwargs=init_kwargs)
...
class GRUCell(DropoutRNNCellMixin, Layer):
    ...
    def call(self, inputs, states, training=None):
        h_tm1 = states[0] if nest.is_sequence(
...
class LSTMCell(DropoutRNNCellMixin, Layer):
            ...
            if self.unit_forget_bias:
                ...
                def bias_initializer(_, *args, **kwargs):
                    return K.concatenate([
                        ...
                        initializers.get("ones")((self.units, ), *args,
                                                 **kwargs),
    ...
    def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
        return list(
            _generate_zero_filled_state_for_cell(self, inputs, batch_size,
                                                 dtype))
...
class PeepholeLSTMCell(LSTMCell):
    ...
    def _compute_carry_and_output(self, x, h_tm1, c_tm1):
        ...
        f = self.recurrent_activation(
            x_f +
            K.dot(h_tm1_f, self.recurrent_kernel[:, self.units:self.units *
                                                 2]) +
    ...
    def _compute_carry_and_output_fused(self, z, c_tm1):
        ...
        i = self.recurrent_activation(z0 +
                                      self.input_gate_peephole_weights * c_tm1)
        f = self.recurrent_activation(z1 + self.forget_gate_peephole_weights *
                                      c_tm1)
        c = f * c_tm1 + i * self.activation(z2)
        o = self.recurrent_activation(z3 +
                                      self.output_gate_peephole_weights * c)
