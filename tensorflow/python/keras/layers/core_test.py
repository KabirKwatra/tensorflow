...
class DropoutLayersTest(keras_parameterized.TestCase):
    def test_dropout(self):
        testing_utils.layer_test(keras.layers.Dropout,
                                 kwargs={"rate": 0.5},
                                 input_shape=(3, 2))
...
class LambdaLayerTest(keras_parameterized.TestCase):
    def test_lambda(self):
        ...
        # test with lambda
        ld = keras.layers.Lambda(lambda x: keras.backend.concatenate(
            [math_ops.square(x), x]))
        config = ld.get_config()
...
class CoreLayersTest(keras_parameterized.TestCase):
    ...
    def test_reshape(self):
        testing_utils.layer_test(keras.layers.Reshape,
    ...
    def test_flatten(self):
        testing_utils.layer_test(keras.layers.Flatten,
                                 kwargs={},
                                 input_shape=(3, 2, 4))
        ...
        target_outputs = np.reshape(np.transpose(inputs, (0, 2, 3, 1)),
                                    (-1, 5 * 5 * 3))
    ...
    def test_dense(self):
        testing_utils.layer_test(keras.layers.Dense,
                                 kwargs={"units": 3},
                                 input_shape=(3, 2))
