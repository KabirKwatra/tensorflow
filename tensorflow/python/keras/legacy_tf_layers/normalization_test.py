...
class BNTest(test.TestCase):
            ...
            for train2_use_gpu in [True, False]:
                for infer_use_gpu in [True, False]:
                    ...
                    for test_var, ref_var in zip(test_vars, ref_vars):
                        self.assertAllClose(
                            test_var, ref_var, rtol=1.e-3, atol=1.e-3)
                    self.assertAllClose(test_loss, ref_loss,
            ...
            with self.session(use_gpu=True) as sess:
                ...
                for _ in range(100):
                    ...
                    self.assertAlmostEqual(
                        np.mean(normed_np_output), 0., places=1)
                    self.assertAlmostEqual(
                        np.std(normed_np_output), 1., places=1)
    ...
    def testConstraints(self):
        def g_constraint(x): return x / math_ops.reduce_sum(x)
        ...
        with self.session(use_gpu=True) as sess:
            ...
            for _ in range(5):
                ...
                moving_means = moving_means * \
                    momentum + avg_means * (1. - momentum)
                moving_vars = moving_vars * momentum + \
                    avg_variances * (1. - momentum)
