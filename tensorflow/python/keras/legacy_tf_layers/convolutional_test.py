...
class ConvTest(test.TestCase):
    ...
    def testConv2DFloat16(self):
        ...
        images = random_ops.random_uniform(
            (5, height, width, 4), dtype='float16')
    ...
    def testConstraints(self):
        # Conv1D
        def k_constraint(x): return x / math_ops.reduce_sum(x)
        def b_constraint(x): return x / math_ops.reduce_max(x)
...
class SeparableConv1DTest(test.TestCase):
    ...
    def testConstraints(self):
        def d_constraint(x): return x / math_ops.reduce_sum(x)
        def p_constraint(x): return x / math_ops.reduce_sum(x)
        def b_constraint(x): return x / math_ops.reduce_max(x)
...
class Conv3DTransposeTest(test.TestCase):
    ...
    def testCreateConv3DTranspose(self):
        ...
        layer = conv_layers.Conv3DTranspose(
            4, [3, 3, 3], activation=nn_ops.relu)
        output = layer.apply(volumes)
    ...
    def testConv3DTransposePaddingSame(self):
        depth, height, width = 5, 7, 9
        volumes = random_ops.random_uniform(
            (5, depth, height, width, 64), seed=1)
        layer = conv_layers.Conv3DTranspose(
