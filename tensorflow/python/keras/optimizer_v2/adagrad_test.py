        ...
        with ops.Graph().as_default():
            for dtype in _DATA_TYPES:
                ...
                grad_repeated_index = ops.IndexedSlices(
                    constant_op.constant([0.1, 0.1], shape=[
                                         2, 1], dtype=dtype),
        ...
        with ops.Graph().as_default():
            for dtype in _DATA_TYPES:
                var_repeated = resource_variable_ops.ResourceVariable([1.0, 2.0],
                                                                      dtype=dtype)
                def loss_repeated(): return math_ops.reduce_sum(  # pylint: disable=g-long-lambda
                ...
                var_aggregated = resource_variable_ops.ResourceVariable([1.0, 2.0],
                                                                        dtype=dtype)
                def loss_aggregated(): return 2 * math_ops.reduce_sum(  # pylint: disable=g-long-lambda
