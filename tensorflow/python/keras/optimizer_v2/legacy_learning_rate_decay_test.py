...
class LRDecayTest(test_util.TensorFlowTestCase):
    ...
    def testStaircase(self):
        if context.executing_eagerly():
            ...
            self.evaluate(variables.global_variables_initializer())
            decayed_lr = learning_rate_decay.exponential_decay(0.1,
                                                               step,
                                                               3,
                                                               0.96,
                                                               staircase=True)
...
class LinearDecayTest(test_util.TensorFlowTestCase):
    ...
    def testBeyondEndWithCycle(self):
        ...
        lr = 0.05
        end_lr = 0.001
        decayed_lr = learning_rate_decay.polynomial_decay(lr,
                                                          step,
                                                          10,
                                                          end_lr,
                                                          cycle=True)
...
class CosineDecayRestartsTest(test_util.TensorFlowTestCase):
    def np_cosine_decay_restarts(self,
                                 step,
                                 decay_steps,
                                 t_mul=2.0,
                                 m_mul=1.0,
    ...
    def testAlpha(self):
        ...
        for step in range(0, 1500, 250):
            ...
            expected = self.np_cosine_decay_restarts(step,
                                                     num_training_steps,
                                                     alpha=alpha)
            self.assertAllClose(self.evaluate(decayed_lr), expected, 1e-6)
