...
class GradientDescentOptimizerTest(test.TestCase, parameterized.TestCase):
    ...
    def testMinimizeResourceVariable(self):
        for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
            ...
            def loss():
                return (math_ops.matmul(var0, x) + var1)  # pylint: disable=cell-var-from-loop
    ...
    def testGradWrtRef(self):
        ...
        with ops.Graph().as_default():
            for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
                ...
                def loss():
                    return vars_[0] + vars_[1]  # pylint: disable=cell-var-from-loop
    ...
    def testNesterovMomentum(self):
        ...
        with ops.Graph().as_default():
            for dtype in [dtypes.float32, dtypes.float64]:
                ...
                def loss():
                    return (5 * var0 * var0 + 3 * var1)  # pylint: disable=cell-var-from-loop
    ...
    def testSparse(self):
        ...
        with ops.Graph().as_default():
            for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
                ...
                grads1 = ops.IndexedSlices(
                    constant_op.constant([[0.01, 0.01], [0.01, 0.01]],
                                         dtype=dtype),
