...
class LossScaleOptimizerTest(test.TestCase, parameterized.TestCase):
    ...
    def testGetUnscaledSparseGradients(self):
        ...
        self.assertAllEqual([[2.0, 1.0], [4.0, 2.5]],
        ...
        with self.test_session(), strategy.scope():
            ...
            if save_with_ls:
                loss_scale = loss_scale_module.DynamicLossScale(
                    ...
                    increment_period=2.0,
                    multiplier=2.0)
                opt = loss_scale_optimizer.LossScaleOptimizer(opt, loss_scale)

            def run_fn():
                return opt.minimize(lambda: var / replicas + 1.0,
                                    var_list=[var])
    ...
    def testGetConfig(self):
        ...
        loss_scale = loss_scale_module.DynamicLossScale(initial_loss_scale=2.0,
                                                        increment_period=3.0,
                                                        multiplier=4.0)
        opt = loss_scale_optimizer.LossScaleOptimizer(opt, loss_scale)
    ...
    def testUnsupportedStrategy(self):
        ...
        expected_error = (
            ...
            "MirroredStrategy")
        with strategy.scope(), self.assertRaisesRegexp(ValueError,
                                                       expected_error):
