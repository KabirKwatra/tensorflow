...
class PolicyTest(test.TestCase, parameterized.TestCase):
    ...
    def test_repr(self):
        for policy in ("float32", "int8", "mixed_bfloat16", "_infer"):
            self.assertEqual(
                repr(mp_policy.Policy(policy)),
                '<Policy "%s", loss_scale=None>' % policy,
            )
        self.assertEqual(
            repr(mp_policy.Policy("float16", loss_scale=2)),
    ...
    def test_loss_scale(self):
        ...
        self.assertIsInstance(policy.loss_scale,
    ...
    def test_loss_scale_warning(self):
        with test.mock.patch.object(tf_logging, "warn") as mock_warn:
            mp_policy.Policy("float32", loss_scale=2.0)
            self.assertEqual(
        ...
        for policy_name in "float16", "mixed_float16":
            with test.mock.patch.object(tf_logging, "warn") as mock_warn:
                mp_policy.Policy(policy_name, loss_scale=2.0)
    ...
    def test_device_compatibility_warning(self):
        with context.eager_mode():
            ...
            with test.mock.patch.object(
                    tf_logging, "warn") as mock_warn, test.mock.patch.object(
                        tf_logging, "info") as mock_info:
    ...
    def test_config(self):
        for policy in (
                ...
                mp_policy.Policy("mixed_float16", loss_scale=2.0),
                mp_policy.Policy("mixed_float16", loss_scale=None),
                mp_policy.Policy("mixed_bfloat16", loss_scale=2.0),
