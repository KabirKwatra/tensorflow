...
def _get_device_attrs(device_type,
                      device_name=None,
                      cc_major=None,
                      cc_minor=None):
    if device_type == "CPU":
    ...
    physical_device_desc = "device: 0, name: %s, pci bus id: 0:0:0.0" % device_name
    if cc_major:
        physical_device_desc += ", compute capability: %d.%d" % (cc_major,
                                                                 cc_minor)
...
class DeviceCompatibilityCheckTest(test.TestCase):
    ...
    def test_unsupported(self):
        device_attrs_list = [_get_device_attrs("GPU", "GPU 1", 6, 0)]
        regex = re.compile(
        ...
        device_attrs_list = [_get_device_attrs("GPU")]
        regex = re.compile(
            r".*compatibility check \(mixed_float16\): WARNING\n"
            r"Your GPU may run slowly with dtype policy mixed_float16.*\n"
