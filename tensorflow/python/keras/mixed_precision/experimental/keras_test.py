...
class KerasLayerTest(keras_parameterized.TestCase):
    ...
    def test_error_passing_policy_string_to_layer(self):
        with self.assertRaisesRegexp(
            TypeError, "Cannot convert value 'mixed_float16' to a " "TensorFlow DType"
        ):
    ...
    def test_config(self, strategy_fn):
        ...
        with strategy_fn().scope():
            ...
            layer = mp_test_util.MultiplyLayer(
                dtype=policy.Policy("float64", loss_scale=2.0)
            )
            config = layer.get_config()
            self.assertEqual(
                config["dtype"],
            ...
            layer = mp_test_util.MultiplyLayer(
                dtype=policy.Policy("_infer", loss_scale=2.0)
            )
            config = layer.get_config()
            self.assertEqual(
                config["dtype"],
...
class KerasModelTest(keras_parameterized.TestCase):
    ...
    def test_model(
        ...
        regularizer = mp_test_util.IdentityRegularizer() if use_regularizer else None
        with strategy_fn().scope():
    ...
    def test_dynamic_loss_scaling(
        self, strategy_fn, pass_loss_scale_to_policy=False, get_config=False
    ):
        strategy = strategy_fn()
    ...
    def test_loss_scale_optimizer_overrides_policy_loss_scale(self):
        with policy.policy_scope(policy.Policy("float32", loss_scale=10.0)):
            opt = gradient_descent.SGD(1.0)
            opt = loss_scale_optimizer.LossScaleOptimizer(opt, loss_scale=5.0)
    ...
    def test_pass_invalid_optimizer_with_loss_scaling(self):
        with policy.policy_scope(policy.Policy("float32", loss_scale=10.0)):
            ...
            with self.assertRaisesRegexp(ValueError, error_msg):
                model.compile(optimizers.SGD(1.0), "mse")
    ...
    def test_restore_old_saved_model(self):
        saved_model_dir = test.test_src_dir_path(
            "python/keras/mixed_precision/experimental/testdata/" "lso_savedmodel_tf2.2"
        )
        model = save.load_model(saved_model_dir)
        expected_kernel = np.array([[9.229685, 10.901115], [10.370763, 9.757362]])
        self.assertAllClose(backend.eval(model.weights[0]), expected_kernel)
        self.assertIsInstance(model.optimizer, loss_scale_optimizer.LossScaleOptimizer)
