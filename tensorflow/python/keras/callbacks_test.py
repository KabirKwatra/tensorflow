...
class KerasCallbacksTest(keras_parameterized.TestCase):
    ...
    def _run_load_weights_on_restart_test_common_iterations(self):
        ...
        self.assertFalse(os.path.exists(
            filepath.format(epoch=initial_epochs + 1)))
        self.assertEqual(
    ...
    def test_EarlyStopping_reuse(self):
        with self.cached_session():
            ...
            hist = model.fit(data, labels, callbacks=[
                             stopper], verbose=0, epochs=20)
    ...
    def test_LearningRateScheduler(self):
        with self.cached_session():
            ...
            cbks = [keras.callbacks.LearningRateScheduler(
                lambda x: 1. / (1. + x))]
            model.fit(
                x_train,
                y_train,
    ...
    def test_ReduceLROnPlateau_patience(self):
        ...
        for epoch in range(len(losses)):
            ...
            lrs.append(keras.backend.get_value(
                reduce_on_plateau.model.optimizer.lr))
    ...
    def test_CSVLogger(self):
        with self.cached_session():
            ...
            cbks = [keras.callbacks.CSVLogger(
                filepath, separator=sep, append=True)]
            model.fit(
                x_train,
                y_train,
                batch_size=BATCH_SIZE,
...
_ObservedSummary = collections.namedtuple(
    '_ObservedSummary', ('logdir', 'tag'))
...
class TestTensorBoardV2(keras_parameterized.TestCase):
    ...
    def test_custom_summary(self):
        ...
        model.fit(x, y, batch_size=2, validation_data=(
            x, y), callbacks=[tb_cbk])
        summary_file = list_summaries(self.logdir)
        self.assertEqual(
            summary_file.scalars,
            {
...
class MostRecentlyModifiedFileMatchingPatternTest(test.TestCase):
    ...
    def test_get_same_file_if_file_name_equals_pattern(self):
        ...
        self.assertEqual(os.path.join(
            test_dir, os.listdir(test_dir)[0]), file_path)
        self.assertEqual(
            keras.callbacks.ModelCheckpoint(
