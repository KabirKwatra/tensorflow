...
class KerasCallbacksTest(keras_parameterized.TestCase):
    ...
    def test_ModelCheckpoint_with_bad_path_placeholders(self):
        ...
        with self.assertRaisesRegexp(
                KeyError, "Failed to format this callback "
                "filepath.*"):
            model.fit(train_ds, epochs=1, callbacks=[callback])
    ...
    def test_LearningRateScheduler(self):
        with self.cached_session():
            ...
            cbks = [
                keras.callbacks.LearningRateScheduler(lambda x: 1.0 / (1.0 + x)
                                                      )
            ]
            model.fit(
                x_train,
            ...
            cbks = [
                keras.callbacks.LearningRateScheduler(lambda x, lr: lr / 2)
            ]
            model.compile(loss="categorical_crossentropy",
                          optimizer="sgd",
                          metrics=["accuracy"])
    ...
    def test_ReduceLROnPlateau_patience(self):
        ...
        for epoch in range(len(losses)):
            reduce_on_plateau.on_epoch_end(epoch,
                                           logs={"val_loss": losses[epoch]})
            lrs.append(
            ...
            for event in summary_iterator.summary_iterator(path):
                ...
                for value in event.summary.value:
                    ...
                    if container is None:
                        ...
                        # structure.
                        if (event.summary.value[0].metadata.plugin_data.
                                plugin_name == "scalars"):
...
class TestTensorBoardV2(keras_parameterized.TestCase):
        ...
        self.assertEqual(
            self._strip_layer_names(summary_file.images, model_type),
            {
                ...
                _ObservedSummary(logdir=self.train_dir,
                                 tag="kernel_0/image/0"),
                _ObservedSummary(logdir=self.train_dir,
                                 tag="kernel_0/image/1"),
                _ObservedSummary(logdir=self.train_dir,
                                 tag="kernel_0/image/2"),
            },
        )
    ...
    def test_custom_summary(self):
        ...
        def scalar_v2_mock(name, data, step=None):
            ...
            with summary_ops_v2.summary_scope(name,
                                              "scalar_summary",
...
class TestTensorBoardV2NonParameterizedTest(keras_parameterized.TestCase):
    ...
    def test_TensorBoard_autoTrace_profileTwiceGraphMode(self):
        ...
        model.fit(
            ...
            np.zeros((64, 1)),
            callbacks=[
                keras.callbacks.TensorBoard(self.logdir, profile_batch=1)
            ],
        )
        ...
        model.fit(
            ...
            np.zeros((64, 1)),
            callbacks=[
                keras.callbacks.TensorBoard(self.logdir, profile_batch=2)
            ],
        )
...
class MostRecentlyModifiedFileMatchingPatternTest(test.TestCase):
    ...
    def test_get_same_file_if_file_name_equals_pattern(self):
        ...
        with open(file_path, "w") as f:
            f.write("foo bar")
        self.assertEqual(os.path.join(test_dir,
                                      os.listdir(test_dir)[0]), file_path)
        self.assertEqual(
            keras.callbacks.ModelCheckpoint(None).
