...
class TestWholeModelSaving(test.TestCase, parameterized.TestCase):
    ...
    def test_sequential_model_saving_2(self):
        ...
        with ops.Graph().as_default(), self.cached_session():
            ...
            model.compile(loss=custom_loss,
                          optimizer=CustomOp(), metrics=['acc'])
    ...
    def test_saving_without_compilation(self):
        ...
        keras.models.save_model(model, saved_model_dir,
        ...
        with self.cached_session():
            ...
            for i in range(4):
                f = keras.layers.Dense(
                    2, name='nested_model_dense_%d' % (i,))(f)
            # This layer name will make the `weights_name`
            # HDF5 attribute blow out of proportion.
            f = keras.layers.Dense(
                2, name='nested_model_output' + ('x' * (2**14)))(f)
    ...
    def test_functional_model_with_custom_loss_and_metric(self):
        def _make_model():
            ...
            model.add_metric(custom_loss, aggregation='mean',
                             name='custom_loss')
            return model
        ...
        self.assertAllClose(evaluation_results,
                            loaded_model_eval_results, 1e-9)
...
class TestWeightSavingAndLoadingTFFormat(test.TestCase):
    ...
    def test_keras_optimizer_warning(self):
        ...
        with graph.as_default(), self.session(graph):
            ...
            model.add(keras.layers.Dense(3))
            model.compile(
                loss='mse', optimizer=optimizers.Adam(), metrics=['acc'])
            if not ops.executing_eagerly_outside_functions():
                model._make_train_function()
            temp_dir = self.get_temp_dir()
    ...
    def test_tensorflow_format_overwrite(self):
        with self.cached_session() as session:
            ...
            x = constant_op.constant(
                np.random.random((3, 2)), dtype=dtypes.float32)
            ...
            model.save_weights(
                prefix, save_format='tensorflow', overwrite=True)
