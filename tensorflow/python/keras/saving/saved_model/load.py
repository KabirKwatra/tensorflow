...
def _is_graph_network(layer):
    ...
    if isinstance(layer, RevivedNetwork):
        ...
        return layer._is_graph_network or isinstance(layer,
...
class KerasObjectLoader(tf_load.Loader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._all_nodes_recreated_from_config = object_identity.ObjectIdentityWeakSet(
        )
    ...
    def _recreate(self, proto, node_id):
        ...
        if node_id in self._nodes_recreated_from_config:
            ...
            obj, setter = super(KerasObjectLoader,
                                self)._recreate(proto, node_id)
        return obj, setter
    ...
    def _add_children_recreated_from_config(self, obj, proto, node_id):
        ...
        for (obj_child, child_id) in children:
            ...
            if (child_id in self._nodes_recreated_from_config
                    and self._nodes_recreated_from_config[child_id][0] is
                    not obj_child):
            ...
            self._nodes_recreated_from_config[child_id] = (
                obj_child,
                self._config_node_setter(setter),
            )
    ...
    def _load_layers(self):
        ...
        for node_id, proto in enumerate(self._proto.nodes):
            if (proto.WhichOneof("kind") != "user_object"
                    or proto.user_object.identifier not in
                    KERAS_OBJECT_IDENTIFIERS):
                continue
        ...
        if node_id in self._nodes_recreated_from_config:
            ...
            if _is_graph_network(node) and generic_utils.validate_config(
                ...
                self.model_layer_dependencies[node_id] = (
                    node,
                    self._get_child_layer_node_ids(node_id, node.name),
                )
            return node, setter

        # Detect whether this object can be revived from the config. If not, then
        # revive from the SavedModel instead.
        obj, setter = self._revive_from_config(proto.identifier, metadata,
                                               node_id)
        if obj is None:
    ...
    def _revive_from_config(self, identifier, metadata, node_id):
        ...
        if identifier == "_tf_keras_metric":
            ...
            obj = self._revive_graph_network(
                metadata, node_id) or self._revive_layer_from_config(
                    metadata, node_id)
        ...
        self._add_children_recreated_from_config(obj,
                                                 self._proto.nodes[node_id],
                                                 node_id)
        return obj, setter
    ...
    def _try_build_layer(self, obj, node_id, build_input_shape):
        ...
        if build_input_shape is None:
            build_input_shape = self._infer_inputs(node_id,
    ...
    def _reconstruct_all_models(self):
        ...
        if all_initialized_models != set(self.model_layer_dependencies.keys()):
            ...
            raise ValueError(
                ...
                "models could not be initialized: {}".format(
                    uninitialized_model_names))
    ...
    def _reconstruct_model(self, model_id, model, layers):
        ...
        if isinstance(model, models_lib.Sequential):
            ...
            if not model.inputs:
                first_layer = self._get_child_layer_node_ids(
                    model_id, model.name)[0]
                input_specs = self._infer_inputs(first_layer)
                input_shapes = self._infer_inputs(first_layer,
                                                  convert_to_shapes=True)
            ...
            (inputs, outputs,
             created_layers) = network_lib.reconstruct_from_config(
                 config,
    ...
    def _get_child_layer_node_ids(self, node_id, name):
        ...
        return [
            node.node_id for node in self._proto.nodes[layer_list].children
        ]

    def _search_for_child_node(self,
                               parent_id,
                               path_to_child,
                               debugging_name=None,
                               raise_error=True):
        ...
        for child in self._proto.nodes[parent_id].children:
            if child.local_name == path_to_child[0]:
                return self._search_for_child_node(child.node_id,
                                                   path_to_child[1:],
        ...
        if raise_error:
            raise ValueError(
                ...
                "Most likely this object was serialized incorrectly.".format(
                    debugging_name or path_to_child[0], path_to_child[0]))
        else:
            return None
    ...
    def _infer_inputs(self, layer_node_id, convert_to_shapes=False):
        ...
        concrete_functions = self._proto.nodes[
            call_fn_id].function.concrete_functions
...
def _finalize_saved_model_layers(layers):
    ...
    for layer in layers:
        layer.built = True
        if hasattr(_get_keras_attr(layer),
...
def revive_custom_object(identifier, metadata):
    ...
    if parent_classes is not None:
        ...
        raise ValueError(
            ...
            "the `custom_objects` arg when calling `load_model()`.".format(
                identifier))
