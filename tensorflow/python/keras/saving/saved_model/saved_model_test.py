...
class TestModelSavingAndLoadingV2(keras_parameterized.TestCase):
    ...
    def test_revived_sequential(self):
        ...
        model.add(keras.layers.Dense(
            2, kernel_regularizer=regularizers.get('l2')))
    ...
    def testBatchNormUpdates(self):
        ...
        loaded = keras_load.load(saved_model_dir)
        self.evaluate(variables.variables_initializer(loaded.variables))
        input_arr = array_ops.constant(
            [[11], [12], [13]], dtype=dtypes.float32)
        input_arr2 = array_ops.constant(
            [[14], [15], [16]], dtype=dtypes.float32)
        ...
        self.assertAllClose(self.evaluate(
            loaded.layers[-1].moving_mean), [0.12])
    ...
    def testSaveWithSignatures(self):
        ...
        model.add(keras.layers.Dense(
            4, kernel_regularizer=regularizers.get('l2')))
        ...
        self.assertAllClose(model.layers[0](
            input_arr), outputs['layer_1_outputs'])
    ...
    def testReviveFunctionalModel(self):
        ...
        model = keras.models.Model(
            [input1, input2], outputs, name='save_model')
        ...
        self.assertEqual('CustomAdd', type(
            loaded.get_layer('custom')).__name__)
...
class TestLayerCallTracing(test.TestCase):
    ...
    def test_training_arg_replacement(self):
        ...
        def assert_num_traces(layer_cls, training_keyword):
            ...
            self.assertLen(
                fn._list_all_concrete_functions_for_serialization(), 2)

            fn(np.ones((2, 4)), training=False)
            self.assertLen(
                fn._list_all_concrete_functions_for_serialization(), 4)
