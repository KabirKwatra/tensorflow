...
def _convert_rnn_weights(layer, weights):
    ...
    if target_class in ["LSTM", "CuDNNLSTM"] and len(weights) == 3:
        ...
        def convert_lstm_weights(weights, from_cudnn=True):
            ...
            kernels = transform_kernels(weights[0],
                                        transpose_input(from_cudnn), n_gates)
    ...
    if target_class in ["GRU", "CuDNNGRU"] and len(weights) == 3:
        ...
        def convert_gru_weights(weights, from_cudnn=True):
            ...
            recurrent_kernels = transform_kernels(weights[1], lambda k: k.T,
                                                  n_gates)
            biases = np.array(weights[2]).reshape((2,
                                                   -1) if from_cudnn else -1)
            return [kernels, recurrent_kernels, biases]
...
def save_optimizer_weights_to_hdf5_group(hdf5_group, optimizer):
    ...
    if symbolic_weights:
        ...
        save_attributes_to_hdf5_group(weights_group, "weight_names",
                                      weight_names)
        weight_values = K.batch_get_value(symbolic_weights)
        for name, val in zip(weight_names, weight_values):
            param_dset = weights_group.create_dataset(name,
                                                      val.shape,
                                                      dtype=val.dtype)
            if not val.shape:
                # scalar
...
def _legacy_weights(layer):
    ...
    if any([not isinstance(w, variables_module.Variable) for w in weights]):
        raise NotImplementedError(
            ...
            "or layer {} with weights {}".format(layer.__class__.__name__,
                                                 weights))
    return weights
