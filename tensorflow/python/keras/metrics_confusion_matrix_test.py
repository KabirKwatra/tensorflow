...
class PrecisionTest(test.TestCase, parameterized.TestCase):
    def test_config(self):
        p_obj = metrics.Precision(name="my_precision",
                                  thresholds=[0.4, 0.9],
                                  top_k=15,
                                  class_id=12)
    ...
    def test_value_is_idempotent(self):
        ...
        for _ in range(10):
            self.assertArrayNear(initial_precision,
                                 self.evaluate(p_obj.result()), 1e-3)
    ...
    def test_weighted_with_threshold(self):
        ...
        expected_precision = weighted_tp / weighted_positives
        self.assertArrayNear([expected_precision, 0], self.evaluate(result),
                             1e-3)
    ...
    def test_multiple_updates(self):
        ...
        weighted_positives = ((0 + 3.0) + (4.0 + 0.0)) + ((0 + 3.0) +
                                                          (4.0 + 0.0))
        expected_precision = weighted_tp / weighted_positives
        self.assertArrayNear([expected_precision, 0],
                             self.evaluate(p_obj.result()), 1e-3)
...
class RecallTest(test.TestCase, parameterized.TestCase):
    ...
    def test_value_is_idempotent(self):
        ...
        for _ in range(10):
            self.assertArrayNear(initial_recall, self.evaluate(r_obj.result()),
                                 1e-3)
    ...
    def test_multiple_updates(self):
        ...
        weighted_positives = ((0 + 3.0) + (4.0 + 0.0)) + ((0 + 3.0) +
                                                          (4.0 + 0.0))
        expected_recall = weighted_tp / weighted_positives
        self.assertArrayNear([expected_recall, 0],
                             self.evaluate(r_obj.result()), 1e-3)
...
class SensitivityAtSpecificityTest(test.TestCase, parameterized.TestCase):
    ...
    def test_value_is_idempotent(self):
        ...
        for _ in range(10):
            self.assertAlmostEqual(initial_sensitivity,
                                   self.evaluate(s_obj.result()), 1e-3)
...
class SpecificityAtSensitivityTest(test.TestCase, parameterized.TestCase):
    ...
    def test_value_is_idempotent(self):
        ...
        for _ in range(10):
            self.assertAlmostEqual(initial_specificity,
                                   self.evaluate(s_obj.result()), 1e-3)
...
class PrecisionAtRecallTest(test.TestCase, parameterized.TestCase):
    ...
    def test_value_is_idempotent(self):
        ...
        for _ in range(10):
            self.assertAlmostEqual(initial_precision,
                                   self.evaluate(s_obj.result()), 1e-3)
    ...
    def test_weighted(self, label_dtype):
        ...
        self.evaluate(variables.variables_initializer(s_obj.variables))
        result = s_obj(y_true, y_pred, sample_weight=weights)
        self.assertAlmostEqual(2.0 / 3.0, self.evaluate(result))
...
class RecallAtPrecisionTest(test.TestCase, parameterized.TestCase):
    def test_config(self):
        s_obj = metrics.RecallAtPrecision(0.4,
                                          num_thresholds=100,
                                          name="recall_at_precision_1")
    ...
    def test_value_is_idempotent(self):
        ...
        for _ in range(10):
            self.assertAlmostEqual(initial_recall,
                                   self.evaluate(s_obj.result()), 1e-3)
...
class AUCTest(test.TestCase, parameterized.TestCase):
    def setup(self):
        ...
        self.y_pred = constant_op.constant([0, 0.5, 0.3, 0.9],
                                           dtype=dtypes.float32)
    ...
    def test_value_is_idempotent(self):
        ...
        for _ in range(10):
            self.assertAllClose(initial_auc, self.evaluate(auc_obj.result()),
                                1e-3)
...
class MultiAUCTest(test.TestCase, parameterized.TestCase):
    def setup(self):
        ...
        self.y_pred = constant_op.constant(np.array([[0, 0.5, 0.3, 0.9],
                                                     [0.1, 0.2, 0.3, 0.4]]).T,
                                           dtype=dtypes.float32)
    ...
    def test_value_is_idempotent(self):
        with self.test_session():
            ...
            initial_auc = self.evaluate(auc_obj.result())
            for _ in range(10):
                self.assertAllClose(initial_auc,
                                    self.evaluate(auc_obj.result()), 1e-3)
    ...
    def test_sample_weight_flat(self):
        ...
        auc_obj = metrics.AUC(num_thresholds=self.num_thresholds,
                              multi_label=False)
        self.evaluate(variables.variables_initializer(auc_obj.variables))
        result = auc_obj(self.y_true_good,
                         self.y_pred,
    ...
    def test_manual_thresholds(self):
        with self.test_session():
            ...
            # num_thresholds.
            auc_obj = metrics.AUC(num_thresholds=2,
                                  thresholds=[0.5],
                                  multi_label=True)
            self.assertEqual(auc_obj.num_thresholds, 3)
            self.assertAllClose(auc_obj.thresholds, [0.0, 0.5, 1.0])
    ...
    def test_pr_interpolation_unweighted(self):
        with self.test_session():
            ...
            with self.subTest(name="good"):
                ...
                self.assertAllClose(self.evaluate(good_result),
                                    (0.91667 + 1.0) / 2.0, 1e-1)
            bad_result = auc_obj(self.y_true_bad, self.y_pred)
            with self.subTest(name="bad"):
                # PR AUCs are 0.917 and 0.5 respectively
                self.assertAllClose(self.evaluate(bad_result),
                                    (0.91667 + 0.5) / 2.0, 1e-1)
    ...
    def test_pr_interpolation(self):
        with self.test_session():
            ...
            auc_obj = metrics.AUC(num_thresholds=self.num_thresholds,
                                  ...
                                  multi_label=True)
            self.evaluate(variables.variables_initializer(auc_obj.variables))
            good_result = auc_obj(self.y_true_good,
                                  self.y_pred,
                                  sample_weight=self.sample_weight)
