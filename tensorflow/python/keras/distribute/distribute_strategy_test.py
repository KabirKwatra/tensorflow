...
def simple_multi_inputs_multi_outputs_model():
    ...
    output_c = keras.layers.Dense(
        3, activation='softmax', name='dense_2')(merged)
    output_d = keras.layers.Dense(
        2, activation='softmax', name='dense_3')(merged)
...
class TestDistributionStrategyWithNumpyArrays(test.TestCase,
    ...
    def test_calling_model_with_nested_numpy_arrays(self, distribution):
        with self.cached_session():
            ...
            input_a_np = np.asarray(
                np.random.random((64, 3)), dtype=np.float32)
            input_b_np = np.asarray(
                np.random.random((64, 5)), dtype=np.float32)
...
class TestDistributionStrategyWithDatasets(test.TestCase,
    ...
    def test_fit_with_dictionary_in_the_dataset_b135161171(
        ...
        with self.cached_session():
            with distribution.scope():
                ...
                predict = keras.layers.Conv2D(
                    2, [1, 1], padding='same')(input_img)
    ...
    def test_fit_eval_and_predict_methods_on_dataset_without_steps(
        ...
        with self.cached_session():
            ...
            self.assertAllClose(fit_with_numpy, fit_with_ds,
                                atol=1e-4, rtol=1e-4)
            self.assertAllClose(
...
class TestDistributionStrategyWithKerasModels(test.TestCase,
    ...
    def test_distribution_strategy_with_loss_reduction_types(
        ...
        history = model.fit(dataset, steps_per_epoch=2,
