...
def _initialize_variables(session):
    ...
    if candidate_vars:
        ...
        if uninitialized_vars:
            session.run(variables_module.variables_initializer(
                uninitialized_vars))
...
class GraphExecutionFunction(object):
    ...
    def __call__(self, inputs):
        ...
        if (self._callable_fn is None or feed_arrays != self._feed_arrays or
            ...
            self._make_callable(feed_arrays, feed_symbols,
                                symbol_vals, session)
    ...
    if unroll:
        ...
        if mask is not None:
            ...
            for i in range(time_steps):
                ...
                tiled_mask_t = tuple(_expand_mask(mask_t, s)
...
def switch(condition, then_expression, else_expression):
    ...
    if not cond_ndim:
        ...
        x = control_flow_ops.cond(
            condition, then_expression_fn, else_expression_fn)
    else:
...
def ctc_label_dense_to_sparse(labels, label_lengths):
    ...
    batch_array = array_ops.transpose(
        array_ops.reshape(
            array_ops.tile(math_ops.range(
                0, label_shape[0]), max_num_labels_tns),
    ...
    def _create_session(distribution_strategy):
        ...
        if is_tpu_strategy(distribution_strategy):
            ...
            session = session_module.Session(
                config=session_config, target=master)
        else:
