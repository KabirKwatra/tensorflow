...
class TrainingTest(keras_parameterized.TestCase):
    ...
    def test_fit_and_validate_nested_training_arg(self):
        ...
        class ReturnTraining(keras.layers.Layer):
            def __init__(self, input_shape=None, **kwargs):
                super(ReturnTraining, self).__init__(input_shape=input_shape,
                                                     **kwargs)
    ...
    def _make_sequence_input_functions(self, input_type):
        ...
        if input_type == "dataset":
            dataset = dataset_ops.Dataset.range(
                16).map(lambda _: array_ops.ones(shape=(1, )))

            xy_dataset = dataset_ops.Dataset.zip((dataset, dataset)).batch(4)
            x_dataset = dataset.batch(4)

            def xy_function(use_namedtuple):
                return xy_dataset.map(
                    xy_namedtuple) if use_namedtuple else xy_dataset

            def x_function(use_namedtuple):
                return x_dataset.map(
                    x_namedtuple) if use_namedtuple else x_dataset
    ...
    def test_compile_with_sparse_placeholders(self):
        ...
        weights = variables_lib.Variable(np.ones((10, 1)).astype(np.float32),
                                         name="weights")

        def weights_mult(x):
            return sparse_ops.sparse_tensor_dense_matmul(x, weights)
    ...
    def test_weight_deduplication(self):
        ...
        model.compile(
            loss="mse",
            optimizer=gradient_descent.SGD(0.24),
            run_eagerly=testing_utils.should_run_eagerly(),
        )
    ...
    def test_weight_shared_across_layers(self):
        ...
        class LayerWithWeightSharedLayers(keras.layers.Layer):
            def __init__(self):
                ...
                shared_trainable_var = resource_variable_ops.ResourceVariable(
                    1.0)
    ...
    def test_static_batch_in_input_layer(self):
        ...
        for batch_size, expected_batches in [(None, 2), (4, 16)]:
            ...
            model.compile(keras.optimizer_v2.adam.Adam(0.001),
                          "binary_crossentropy")
            counter = Counter()
    ...
    def test_training_on_sparse_categorical_crossentropy_loss_with_softmax(
        ...
        with context.eager_mode():
            ...
            reference_model = testing_utils.get_small_sequential_mlp(
                16, 2, input_dim=4)
            ...
            reference_model_loss = reference_model.train_on_batch(
                train_x, train_y)

            test_model = testing_utils.get_small_sequential_mlp(16,
                                                                2,
                                                                input_dim=4)
            test_model.compile(
                loss="sparse_categorical_crossentropy",
            ...
            self.assertAlmostEqual(test_model_loss,
    ...
    def test_losses_of_different_dtypes(self):
        ...
        out_1 = keras.layers.Dense(2, dtype="float32",
                                   kernel_regularizer="l2")(inp)
        out_2 = keras.layers.Dense(2, dtype="float16",
                                   kernel_regularizer="l2")(inp)
...
class TestExceptionsAndWarnings(keras_parameterized.TestCase):
    ...
    def test_compile_warning_for_loss_missing_output(self):
        with self.cached_session():
            ...
            out_2 = keras.layers.Dense(3, activation="softmax",
                                       name="dense_2")(out_1)
...
class LossWeightingTest(keras_parameterized.TestCase):
    ...
    def test_sample_weights(self):
        ...
        ref_score = model.evaluate(x_test,
                                   y_test,
                                   verbose=0,
                                   sample_weight=sample_weight)
    ...
    def test_sample_weight_tensor(self):
        ...
        with ops.get_default_graph().as_default():
            ...
            sample_weights = array_ops.constant([[0, 0.4, 1, 1],
                                                 [2, 0.4, 0.3, 1]])
            dataset = dataset_ops.Dataset.from_tensor_slices(sample_weights)
            sample_weights = dataset_ops.make_one_shot_iterator(
                dataset).get_next()
            ...
            with self.cached_session() as sess:
                self.assertAllClose((0.4 + 1 + 1) / 4,
                                    sess.run(model.total_loss,
                                             feed_dict=feeds))
                self.assertAllClose((2 + 0.4 + 0.3 + 1) / 4,
                                    sess.run(model.total_loss,
                                             feed_dict=feeds))
...
class TestDynamicTrainability(keras_parameterized.TestCase):
    ...
    def test_toggle_value(self):
        ...
        dense_0 = keras.layers.Dense(1,
                                     kernel_initializer="ones",
                                     bias_initializer="ones")
        dense_1 = keras.layers.Dense(1,
                                     kernel_initializer="ones",
                                     bias_initializer="ones")
...
class TestTrainingWithDataTensors(keras_parameterized.TestCase):
    ...
    def test_training_and_eval_methods_on_symbolic_tensors_multi_io(self):
        ...
        model.train_on_batch([input_a_tf, input_b_tf],
                             [output_d_tf, output_e_tf])
        ...
        model.test_on_batch([input_a_tf, input_b_tf],
                            [output_d_tf, output_e_tf])
    ...
    def test_model_with_input_feed_tensor(self):
        ...
        with ops.Graph().as_default(), self.cached_session():
            ...
            input_v = keras.backend.variables_module.Variable(input_a_np,
                                                              dtype="float32")
            ...
            out = model.evaluate({"input_b": input_b_np},
                                 [output_a_np, output_b_np],
                                 batch_size=10)
            out = model.evaluate(input_b_np, [output_a_np, output_b_np],
                                 batch_size=10)
...
class TestTrainingWithMetrics(keras_parameterized.TestCase):
    ...
    def test_metrics_names(self):
        ...
        mse_metric = "mse" if context.executing_eagerly(
    ...
    def test_metrics_valid_compile_input_formats(self):
        ...
        out_1 = keras.layers.Dense(1,
                                   kernel_initializer="ones",
                                   name="output_1",
                                   trainable=False)
        out_2 = keras.layers.Dense(1,
                                   kernel_initializer="ones",
                                   name="output_2",
                                   trainable=False)
...
class TestAutoUpdates(keras_parameterized.TestCase):
    ...
    def test_batchnorm_trainable_false(self):
        bn = keras.layers.BatchNormalization()
        model = testing_utils.get_model_from_layers(
            [bn, keras.layers.Dense(1)], input_shape=(10, ))
        bn.trainable = False
        model.compile("sgd",
                      "mse",
                      run_eagerly=testing_utils.should_run_eagerly())
