...
class LossesContainer(object):
    ...
    def _build(self, y_pred):
        ...
        self._loss_weights = map_to_output_names(
            y_pred, self._output_names, self._loss_weights
        )
    ...
    def __call__(self, y_true, y_pred, sample_weight=None, regularization_losses=None):
        ...
        sample_weight = map_to_output_names(y_pred, self._output_names, sample_weight)
...
class MetricsContainer(object):
    ...
    def _build(self, y_pred, y_true):
        ...
        self._weighted_metrics = self._maybe_broadcast(self._weighted_metrics, y_pred)

        # Accept a dict of metrics keyed by output_name when outputs are a flat
        # list.
        self._metrics = map_to_output_names(y_pred, self._output_names, self._metrics)
        self._weighted_metrics = map_to_output_names(
            y_pred, self._output_names, self._weighted_metrics
        )
    ...
    def _maybe_broadcast(self, metrics, y_pred):
        ...
        def _should_broadcast(metrics):
            ...
            if not nest.is_sequence(metrics):
                return True
            # e.g. ['mse'] or ['mse', 'mae'].
            return isinstance(metrics, (list, tuple)) and not any(
                nest.is_sequence(m) for m in metrics
            )
        ...
        if _should_broadcast(metrics):
            ...
            return nest.map_structure(
                lambda _: [_maybe_copy(m) for m in metrics], y_pred
            )
...
def map_to_output_names(y_pred, output_names, struct):
    ...
    """
    outputs_are_flat_list = isinstance(y_pred, (list, tuple)) and not any(
        nest.is_sequence(y_p) for y_p in y_pred
    )
