...
class GenericArrayLikeDataAdapter(TensorLikeDataAdapter):
    ...
    def can_handle(x, y=None):
        ...
        def _is_array_like(v):
            ...
            return (hasattr(v, "__getitem__") and hasattr(v, "shape")
                    and hasattr(v, "dtype") and hasattr(v, "__len__"))

        if not TensorLikeDataAdapter.can_handle(
                x, y) and not CompositeTensorDataAdapter.can_handle(x, y):
    ...
    def slice_inputs(self, indices_dataset, inputs):
        ...
        def grab_batch(indices):
            ...
            def py_method(ind):
                def slice_array(data):
                    return training_utils.slice_arrays(data,
                                                       ind.numpy(),
                                                       contiguous=contiguous)

                return [slice_array(inp) for inp in flat_inputs]

            flat_out = script_ops.eager_py_func(py_method, [indices],
                                                flat_dtypes)
        ...
        dataset = indices_dataset.map(grab_batch,
                                      num_parallel_calls=dataset_ops.AUTOTUNE)
...
class CompositeTensorDataAdapter(DataAdapter):
    ...
    def can_handle(x, y=None):
        ...
        def _is_composite(v):
            ...
            if isinstance(v,
                          composite_tensor.CompositeTensor) and not isinstance(
    ...
    def __init__(self,
        ...
        if self._has_partial_batch:
            self._partial_batch_size = num_samples - (self._size -
                                                      1) * self._batch_size
...
class DatasetAdapter(DataAdapter):
    ...
    def can_handle(x, y=None):
        return isinstance(
            x, (dataset_ops.DatasetV1,
                dataset_ops.DatasetV2)) or _is_distributed_dataset(x)
...
class GeneratorDataAdapter(DataAdapter):
    ...
    def __init__(self,
        ...
        if model is not None and not model.built:
            ...
            model.distribute_strategy.run(lambda x: model(x, training=False),
                                          args=(concrete_x, ))
        ...
        generator_fn = self._handle_multiprocessing(x, workers,
                                                    use_multiprocessing,
                                                    max_queue_size)
    ...
    def _handle_multiprocessing(self, x, workers, use_multiprocessing,
        ...
        if workers > 1 or (workers > 0 and use_multiprocessing):
            ...
            def generator_fn():
                ...
                self._enqueuer.start(workers=workers,
                                     max_queue_size=max_queue_size)
...
def broadcast_sample_weight_modes(target_structure, sample_weight_modes):
    ...
    if isinstance(sample_weight_modes, str):
        if isinstance(target_structure, dict):
            return {
                key: sample_weight_modes
                for key in target_structure.keys()
            }
    ...
    if sample_weight_modes:
        try:
            ...
            target_str = str(
                nest.map_structure(lambda _: "...", target_structure))
            mode_str = str(
                nest.map_structure(lambda _: "...", sample_weight_modes))
...
class DataHandler(object):
    ...
    def steps(self):
        ...
        while self._inferred_steps is None or self._current_step < self._inferred_steps:
            ...
            can_run_full_execution = (self._steps_per_execution_value == 1
                                      or self._inferred_steps is None or
                                      self._inferred_steps - self._current_step
            ...
            if can_run_full_execution:
                ...
                self._steps_per_execution.assign(
                    self._steps_per_execution_value)
...
def train_validation_split(arrays, validation_split, shuffle=True):
    ...
    if unsplitable:
        raise ValueError(
            ...
            "arrays, found following types in the input: {}".format(
                unsplitable))
...
def _is_distributed_dataset(ds):
    ...
    return isinstance(ds, (input_lib.DistributedDataset,
