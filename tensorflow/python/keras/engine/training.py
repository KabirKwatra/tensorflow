...
class Model(network.Network, version_utils.ModelVersionSelector):
    ...
    def make_train_function(self):
        ...
        def train_function(iterator):
            data = next(iterator)
            outputs = self.distribute_strategy.run(self.train_step,
                                                   args=(data, ))
    ...
    def make_test_function(self):
        ...
        def test_function(iterator):
            data = next(iterator)
            outputs = self.distribute_strategy.run(self.test_step,
                                                   args=(data, ))
            outputs = reduce_per_replica(outputs,
                                         self.distribute_strategy,
                                         reduction="first")
    ...
    def make_predict_function(self):
        ...
        def predict_function(iterator):
            data = next(iterator)
            outputs = self.distribute_strategy.run(self.predict_step,
                                                   args=(data, ))
            outputs = reduce_per_replica(outputs,
                                         self.distribute_strategy,
                                         reduction="concat")
