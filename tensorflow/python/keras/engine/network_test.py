...
class NetworkConstructionTest(keras_parameterized.TestCase):
    ...
    def test_multi_input_layer(self):
        with self.cached_session():
            ...
            output_shapes = model.compute_output_shape(
                [(None, 32), (None, 32)])
            ...
            self.assertListEqual([x.shape for x in fn_outputs], [
                                 (10, 64), (10, 5)])
    ...
    def test_recursion(self):
        with ops.Graph().as_default(), self.cached_session():
            ...
            fn = keras.backend.function(
                final_model.inputs, final_model.outputs)
            input_a_np = np.random.random((10, 32))
            input_b_np = np.random.random((10, 32))
            fn_outputs = fn([input_a_np, input_b_np])
            self.assertListEqual([x.shape for x in fn_outputs], [
    ...
    def test_multi_input_multi_output_recursion(self):
        with self.cached_session():
            ...
            fn = keras.backend.function(
                multi_io_model.inputs, multi_io_model.outputs)
            fn_outputs = fn([
    ...
    def test_raw_tf_compatibility(self):
        with ops.Graph().as_default():
            ...
            j_tf = array_ops.placeholder(
                dtype=dtypes.float32, shape=(None, 32))
            k_tf = array_ops.placeholder(
                dtype=dtypes.float32, shape=(None, 32))
    ...
    def test_mask_derived_from_keras_layer(self):
        ...
        outputs = keras.layers.RNN(
            keras.layers.LSTMCell(100))(inputs, mask=mask)
    ...
    def test_call_nested_arg_derived_from_keras_layer(self):
        ...
        model.compile(
            ...
            run_eagerly=testing_utils.should_run_eagerly())
        history = model.fit(
            x=[np.ones((10, 10)), 2 * np.ones((10, 10)),
               3 * np.ones((10, 10))],
            y=15 * np.ones((10, 10)),
            batch_size=2)
    ...
    def test_model_initialization(self):
        ...
        w = model.add_weight(
            'w', [], initializer=keras.initializers.Constant(1))
    ...
    def test_dependency_tracking(self):
        ...
        self.assertEqual(
            model.trackable, model._lookup_dependency('trackable'))
...
class DefaultShapeInferenceBehaviorTest(keras_parameterized.TestCase):
    ...
    def test_sequential_as_downstream_of_masking_layer(self):
        ...
        if not context.executing_eagerly():
            ...
            # issue.
            mask_outputs = [model.layers[1].compute_mask(
...
class NestedNetworkTest(test.TestCase):
    ...
    def test_nested_network_inside_network(self):
        ...
        inner_network = keras.engine.network.Network(
