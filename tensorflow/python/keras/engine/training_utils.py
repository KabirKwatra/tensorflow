...
class SliceAggregator(Aggregator):
    ...
    def finalize(self):
        ...
        for is_finished in self._async_copies:
            timeout = max([0., self._MAX_COPY_SECONDS -
...
class OutputsAggregator(Aggregator):
    ...
    def create(self, batch_outs):
        ...
        self._structure = nest.get_traverse_shallow_structure(
            lambda x: not composite_tensor_utils.is_composite_or_composite_value(
                x),
            batch_outs)
...
def standardize_weights(y,
    ...
    if isinstance(class_weight, dict):
        ...
        if tensor_util.is_tensor(y):
            ...
            class_sample_weight = math_ops.cast(
                class_sample_weight, K.floatx())
            if sample_weight is not None:
...
class RespectCompiledTrainableState(object):
    ...
    def __enter__(self):
        ...
        if self._should_set_trainable:
            self._model._set_trainable_state(
                self._compiled_trainable_state)  # pylint: disable=protected-access
    ...
    def __exit__(self, type_arg, value_arg, traceback_arg):
        ...
        if self._should_set_trainable:
            self._model._set_trainable_state(
                self._current_trainable_state)  # pylint: disable=protected-access
...
def prepare_loss_functions(loss, output_names):
    ...
    if isinstance(loss, collections_abc.Mapping):
        ...
        loss_functions = [get_loss_function(loss)
