...
def check_array_lengths(inputs, targets, weights=None):
    ...
    def is_tensor_or_composite_tensor(x):
        return tensor_util.is_tensor(
            x
        ) or composite_tensor_utils.is_composite_or_composite_value(x)
...
def collect_per_output_metric_info(
    metrics, output_names, output_shapes, loss_fns, is_weighted=False
):
...
def standardize_weights(
    ...
    if sample_weight is not None:
        ...
        if (
            not tensor_util.is_tensor(sample_weight)
...
def has_tensors(ls):
    ...
    if isinstance(ls, (list, tuple)):
        return any(
            tensor_util.is_tensor(v) and not isinstance(v, ragged_tensor.RaggedTensor)
            for v in ls
        )
    ...
    return tensor_util.is_tensor(ls) and not isinstance(ls, ragged_tensor.RaggedTensor)
...
def validate_input_types(inp, orig_inp, allow_dict=True, field_name="inputs"):
    ...
    if isinstance(inp, (list, tuple)):
        if not all(isinstance(v, np.ndarray) or tensor_util.is_tensor(v) for v in inp):
            raise ValueError(
...
def is_eager_dataset_or_iterator(data):
    return context.executing_eagerly() and isinstance(
        data, (dataset_ops.DatasetV1, dataset_ops.DatasetV2, iterator_ops.OwnedIterator)
    )
