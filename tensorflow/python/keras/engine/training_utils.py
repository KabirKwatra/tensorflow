...
class MetricsAggregator(Aggregator):
    ...
    def __init__(self, use_steps, num_samples=None, steps=None):
        super(MetricsAggregator, self).__init__(use_steps=use_steps,
...
class ConcatAggregator(Aggregator):
    ...
    def __init__(self, batch_size):
        self.composite = None
        super(ConcatAggregator, self).__init__(use_steps=True,
                                               num_samples=None,
                                               steps=None,
    ...
    def finalize(self):
        ...
        if len(self.results) == 1:
            ...
            for r in self.results[1:]:
                results = composite_tensor_utils.append_composite_tensor(
                    results, r)
...
class SliceAggregator(Aggregator):
    ...
    def __init__(self, num_samples, batch_size):
        ...
        self._errors = []
        super(SliceAggregator, self).__init__(use_steps=False,
                                              num_samples=num_samples,
                                              steps=None,
                                              batch_size=batch_size)
    ...
    def aggregate(self, batch_element, batch_start, batch_end):
        ...
        if batch_end - batch_start == self.num_samples:
            if self.num_samples != batch_element.shape[0]:
                raise ValueError(
                    "Mismatch between expected batch size and model output batch size. "
                    "Output shape = {}, expected output shape = shape {}".
                    format(batch_element.shape, self.results.shape))
    ...
    def _slice_assign(self, batch_element, batch_start, batch_end,
                      is_finished):
        try:
            self.results[batch_start:batch_end] = batch_element
...
class OutputsAggregator(Aggregator):
    ...
    def create(self, batch_outs):
        ...
        self._structure = nest.get_traverse_shallow_structure(
            lambda x: not composite_tensor_utils.
            is_composite_or_composite_value(x),
            batch_outs,
        )
...
def standardize_single_array(x, expected_shape=None):
    ...
    if isinstance(x, int):
        raise ValueError(
            "Expected an array data type but received an integer: {}".format(
                x))
    ...
    if shapes:
        for i in range(len(names)):
            if shapes[i] is not None:
                ...
                elif composite_tensor_utils.is_composite_or_composite_value(
                        data[i]):
...
def standardize_sample_or_class_weights(x_weight, output_names, weight_type):
    ...
    """
    if x_weight is None or (isinstance(x_weight,
                                       (list, tuple)) and len(x_weight) == 0):  # pylint: disable=g-explicit-length-test
    ...
    if isinstance(x_weight, collections.Mapping):
        ...
        raise TypeError("The model has multiple outputs, so `" + weight_type +
                        ...
                        "` type not understood: " + str(x_weight))
...
def collect_per_output_metric_info(metrics,
    ...
    if isinstance(metrics, list):
        ...
        generic_utils.check_for_unexpected_keys("metrics", metrics,
                                                output_names)
        nested_metrics = []
        for name in output_names:
    ...
    for i, metrics in enumerate(nested_metrics):
        ...
        for metric in metrics:
            ...
            if not isinstance(metric_fn, metrics_module.Metric):
                metric_fn = metrics_module.MeanMetricWrapper(metric_fn,
                                                             name=metric_name)
...
def standardize_weights(y,
    ...
    if isinstance(class_weight, dict):
        ...
        if tensor_util.is_tensor(y):
            ...
            class_sample_weight = math_ops.cast(class_sample_weight,
                                                K.floatx())
            if sample_weight is not None:
...
def has_tensors(ls):
    ...
    if isinstance(ls, (list, tuple)):
        return any(
            tensor_util.is_tensor(v)
            and not isinstance(v, ragged_tensor.RaggedTensor) for v in ls)
    if isinstance(ls, dict):
        return any(
            tensor_util.is_tensor(v)
            and not isinstance(v, ragged_tensor.RaggedTensor)
...
def get_metric_function(metric, output_shape=None, loss_fn=None):
    ...
    is_sparse_categorical_crossentropy = isinstance(
        loss_fn, losses.SparseCategoricalCrossentropy) or (
            isinstance(loss_fn, losses.LossFunctionWrapper)
            and loss_fn.fn == losses.sparse_categorical_crossentropy)

    is_binary_crossentropy = isinstance(
        loss_fn, losses.BinaryCrossentropy) or (
            isinstance(loss_fn, losses.LossFunctionWrapper)
...
class RespectCompiledTrainableState(object):
    ...
    def __enter__(self):
        ...
        for layer, trainable in self._compiled_trainable_state.items():
            if (layer in self._current_trainable_state
...
def validate_dataset_input(x, y, sample_weight, validation_split=None):
    ...
    if validation_split is not None and validation_split != 0.0:
        raise ValueError("`validation_split` argument is not supported when "
...
def validate_input_types(inp, orig_inp, allow_dict=True, field_name="inputs"):
    ...
    if isinstance(inp, (list, tuple)):
        if not all(
                isinstance(v, np.ndarray) or tensor_util.is_tensor(v)
                for v in inp):
            raise ValueError(
...
def check_steps_argument(input_data, steps, steps_name):
    ...
    if (input_data is None or is_x_iterator or has_symbolic_tensors(input_data)
    ...
    if steps is not None:
        ...
        if isinstance(input_data,
                      list_types) or (isinstance(input_data, dict) and any(
...
class ModelInputs(object):
    ...
    def __init__(self, inputs):
        ...
        self._is_single_input = not isinstance(self._inputs,
                                               (list, tuple, dict))
    ...
    def get_symbolic_inputs(self, return_single_as_list=False):
        ...
        for i, (k,
                v) in enumerate(zip(self._input_names,
                                    self._flattened_inputs)):
...
def get_input_shape_and_dtype(layer):
    ...
    def _is_graph_model(layer):
        return (hasattr(layer, "_is_graph_network") and layer._is_graph_network
...
def unpack_validation_data(validation_data, raise_if_ambiguous=True):
    ...
    if isinstance(
        ...
        if raise_if_ambiguous:
            raise ValueError("When passing a `validation_data` argument, "
