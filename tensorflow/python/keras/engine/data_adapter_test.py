...
class TensorLikeDataAdapterTest(DataAdapterTestBase):
        ...
        with context.eager_mode():
            ...
            def _verify_batch(batch):
                ...
                for i in range(1, len(batch)):
                    self.assertEqual(
                        shuffled_batch[i-1] + 1, shuffled_batch[i])
    ...
    def test_partial_batch(
        ...
        self.assertEqual(adapter.partial_batch_size(),
                         partial_batch_size or None)
...
class TestValidationSplit(keras_parameterized.TestCase):
    ...
    def test_validation_split_shuffled(self, use_numpy):
        ...
        self.assertEqual(
            sorted(array_ops.concat(
                [train_x, val_x], axis=0).numpy().tolist()),
            sorted(ops.convert_to_tensor_v2(x).numpy().tolist()))
        self.assertEqual(
            sorted(array_ops.concat(
                [train_y, val_y], axis=0).numpy().tolist()),
            sorted(ops.convert_to_tensor_v2(y).numpy().tolist()))
        self.assertEqual(
            sorted(array_ops.concat(
                [train_sw, val_sw], axis=0).numpy().tolist()),
