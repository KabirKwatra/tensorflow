...
def _build_model(strategy, model, mode, inputs, targets=None):
    if model._compile_distribution:
        dist_utils.clone_model_on_replicas(model,
                                           strategy,
                                           mode,
                                           inputs=inputs,
                                           targets=targets)
...
def experimental_tpu_predict_loop(model,
    ...
    while current_step < target_steps:
        ...
        for i in range(num_model_outputs):
            ...
            single_model_output = batch_outs[output_start_index:
                                             output_end_index]
...
class DistributionMultiWorkerTrainingLoop(training_utils.TrainingLoop):
    ...
    def fit(self, *args, **kwargs):
        return _train_with_multi_worker(self._single_worker_loop.fit)(*args,
                                                                      **kwargs)
