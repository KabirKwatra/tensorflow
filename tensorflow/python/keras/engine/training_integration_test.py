...
_LAYERS_TO_TEST = [
    ...
    (
        ...
        (8, 8, 3),
        (True, True, False),
        collections.OrderedDict([("axis", [3]), ("center", [True, False]),
                                 ("scale", [True, False])]),
        None,
    ),
    (
...
def _gather_test_cases():
    ...
    for layer_type, inp_shape, fuzz_dims, arg_dict, filter_fn in _LAYERS_TO_TEST:
        ...
        for arguments in itertools.product(*arg_combinations):
            ...
            cases.append(
                (name, layer_type, inp_shape, fuzz_dims, layer_kwargs))
    return cases
...
class CoreLayerIntegrationTest(keras_parameterized.TestCase):
    ...
    def test_layer_output_type(self, layer_to_test, input_shape, _,
                               layer_kwargs):
        ...
        for x in [layer_result, model_result]:
            ...
            if isinstance(x, ops.EagerTensor) != context.executing_eagerly():
                expected_type = (ops.EagerTensor if
                                 context.executing_eagerly() else ops.Tensor)
                raise ValueError("Expected type {}, got type {}".format(
    ...
    def _run_fit_eval_predict(self, layer_to_test, input_shape, data_shape,
        ...
        layer = keras.layers.Lambda(lambda x: math_ops.reduce_mean(
            x, keepdims=True))(layer)
        ...
        eval_dataset = dataset_ops.DatasetV2.range(4).map(map_fn).batch(
            batch_size)
    ...
    def test_model_loops(self, layer_to_test, input_shape, fuzz_dims,
                         layer_kwargs):
        self._run_fit_eval_predict(layer_to_test, input_shape, input_shape,
                                   layer_kwargs)
