...
class LegacyRNNTest(tf.test.TestCase):
    ...
    def testRNNWithKerasSimpleRNNCell(self):
        with self.cached_session() as sess:
            ...
            _, outputs, state = sess.run([train_op, outputs, state], {
                inputs: x_train,
                predict: y_train
            })
    ...
    def testKerasAndTFRNNLayerOutputComparison(self):
        ...
        with self.session(graph=tf.Graph()) as sess:
            inputs = tf.placeholder(tf.float32,
                                    shape=(None, timestep, input_shape))
            cell = tf.keras.layers.SimpleRNNCell(output_shape)
            tf_out, tf_state = tf.nn.dynamic_rnn(cell,
                                                 inputs,
                                                 dtype=tf.float32)
            cell.set_weights(weights)
            [tf_out, tf_state] = sess.run([tf_out, tf_state],
                                          {inputs: x_train})
        with self.session(graph=tf.Graph()) as sess:
    ...
    def testRNNCellSerialization(self):
        for cell in [
                tf.nn.rnn_cell.LSTMCell(32, use_peepholes=True,
                                        cell_clip=True),
                tf.nn.rnn_cell.BasicLSTMCell(32, dtype=tf.float32),
                tf.nn.rnn_cell.BasicRNNCell(32,
                                            activation="relu",
                                            dtype=tf.float32),
...
def get_test_data(train_samples, test_samples, input_shape, num_classes):
    ...
    templates = 2 * num_classes * np.random.random((num_classes, ) +
                                                   input_shape)
