...
class KerasOptimizersTest(keras_parameterized.TestCase):
    ...
    def test_adadelta(self):
        with self.cached_session():
            ...
            self._test_optimizer(
                keras.optimizers.Adadelta(decay=1e-3), target=0.4)
    ...
    def test_adam(self):
        with self.cached_session():
            ...
            self._test_optimizer(
                keras.optimizers.Adam(decay=1e-3), target=0.73)
    ...
    def test_tf_optimizer_iterations(self):
        ...
        with self.cached_session():
            ...
            model.compile(
                ...
                run_eagerly=testing_utils.should_run_eagerly())
            keras.backend.track_tf_optimizer(optimizer)
            self.assertEqual(keras.backend.get_value(
    ...
    def test_mixed_precision_loss_scale_optimizer(self):
        ...
        optimizer = MixedPrecisionLossScaleOptimizer(
            AdamOptimizer(), 'dynamic')
        model = keras.models.Sequential()
        model.add(
            keras.layers.Dense(
                2, input_shape=(3,),
