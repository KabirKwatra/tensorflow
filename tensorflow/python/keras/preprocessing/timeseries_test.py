...
class TimeseriesDatasetTest(test.TestCase):
    ...
    def test_sampling_rate(self):
        ...
        for i, batch in enumerate(dataset):
            ...
            for j in range(min(5, len(batch[0]))):
                ...
                self.assertAllClose(batch[0][j], np.arange(start_index, end_index, 2))
    ...
    def test_sequence_stride(self):
        ...
        for i, batch in enumerate(dataset):
            ...
            for j in range(min(5, len(batch[0]))):
                ...
                self.assertAllClose(batch[0][j], np.arange(start_index, end_index))
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "data and targets to have the same number"
        ):
        ...
        with self.assertRaisesRegex(ValueError, "sequence_stride must be "):
            _ = timeseries.timeseries_dataset(np.arange(10), None, 3, sequence_stride=0)
