...
def copy_v2(src, dst, overwrite=False):
    ...
    Raises:
      errors.OpError: If the operation fails.
    """
    _pywrap_file_io.CopyFile(compat.as_bytes(src), compat.as_bytes(dst),
                             overwrite)
...
def rename_v2(src, dst, overwrite=False):
    ...
    Raises:
      errors.OpError: If the operation fails.
    """
    _pywrap_file_io.RenameFile(compat.as_bytes(src), compat.as_bytes(dst),
                               overwrite)
...
def walk_v2(top, topdown=True, onerror=None):
    ...
    for subdir in subdirs:
        for subitem in walk_v2(os.path.join(top, subdir),
                               topdown,
                               onerror=onerror):
            yield subitem
