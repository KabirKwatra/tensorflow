...
def _simple_loop(x, functor):
    ...
    def c(i, j): return i < 4
    def b(i, j): return (i + 1, functor(j))
...
def _loop_vars_intertwined(x0, y0, functor_x, functor_y):
    ...
    def c(i, j, x, y): return j < 4
    def b(i, j, x, y): return (j + 1, i + 1, functor_y(y), functor_x(x))
...
class AutoMixedPrecisionTest(test.TestCase):
    ...
    def _run_simple_loop_test(self, inp, body, out):
        ...
        if test.is_gpu_available(cuda_only=True):
            ...
            self.assertAllClose(output_val_ref, output_val,
                                atol=2e-3, rtol=1e-3)
    ...
    def test_ingraph_train_loop(self):
        ...
        if test.is_gpu_available(cuda_only=True):
            ...
            data = np.random.normal(
                size=(bs * num_iter, nchan)).astype(np.float32)
            ...
            begin, end = constant_op.constant(
                0), constant_op.constant(num_iter)
