...
class ConvUtilsTest(test.TestCase):
    ...
    def testConvertDataFormat(self):
        self.assertEqual(
            'NCDHW', utils.convert_data_format('channels_first', 5))
        self.assertEqual(
            'NCHW', utils.convert_data_format('channels_first', 4))
        ...
        self.assertEqual(
            'NDHWC', utils.convert_data_format('channels_last', 5))

        with self.assertRaises(ValueError):
            utils.convert_data_format('invalid', 2)

    def testNormalizeTuple(self):
        self.assertEqual((2, 2, 2), utils.normalize_tuple(
            2, n=3, name='strides'))
        self.assertEqual(
...
class ConstantValueTest(test.TestCase):
    ...
    def testConstantValue(self):
        def f1(): return constant_op.constant(5)
        def f2(): return constant_op.constant(32)

        # Boolean pred
        self.assertEqual(5, utils.constant_value(
            utils.smart_cond(True, f1, f2)))
        self.assertEqual(32, utils.constant_value(
            utils.smart_cond(False, f1, f2)))
