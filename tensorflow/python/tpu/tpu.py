...
def initialize_system(embedding_config=None,
    ...
    with ops.device(_tpu_system_device_name(job)):
        ...
        with ops.control_dependencies([topology]):
            embedding_init = tpu_ops.configure_tpu_embedding(
...
def tpu_replicated_input_resolver(op, resource_reads, resource_writes):
    ...
    return replace_with_unreplicated_resources(
        resource_reads) or replace_with_unreplicated_resources(resource_writes)
...
class TPUReplicateContext(control_flow_ops.XLAControlFlowContext):
    ...
    def __init__(self, name, num_replicas, pivot):
        ...
        self._tpu_relicate_attr_buf = c_api_util.ScopedTFBuffer(
            attr_value_pb2.AttrValue(
                s=self._name_as_bytes).SerializeToString())
    ...
    def get_replicated_var_handle(self, name, vars_, is_mirrored=False):
        ...
        if device_assignment is not None:
            ...
            devices_to_vars = {
                device_util.canonicalize(v.device): v
                for v in vars_
            }
            replicated_vars = []
            for replica_id in range(device_assignment.num_replicas):
                for logical_core in range(
                    ...
                    device = device_util.canonicalize(
                        device_assignment.tpu_device(replica=replica_id,
        ...
        with graph.as_default():
            ...
            handle = tpu_ops.tpu_replicated_input(
                [v.handle for v in replicated_vars],
                name=name + "/handle",
    ...
    def EnterGradientColocation(self, op, gradient_uid):
        if op is not None:
            ...
            if not self._outside_compilation_cluster:
                try:
                    outside_attr = op.get_attr(
                        _OUTSIDE_COMPILATION_ATTR).decode("ascii")
        ...
        if op is not None:
            ...
            if op is last_op:
                ...
                raise errors.InternalError(
                    op.node_def,
                    op,
                    "Badly nested gradient colocation, expected " + last_op +
                    ", got " + op.name,
                )
    ...
    def _EnterOutsideCompilationScope(self, cluster=None):
        ...
        if self._outside_compilation_cluster:
            raise NotImplementedError(
                "Cannot nest outside_compilation clusters")
        if cluster:
            self._outside_compilation_cluster = cluster
        else:
            self._outside_compilation_cluster = str(
                self._outside_compilation_counter)
        ...
        if (device.device_type == "TPU_REPLICATED_CORE"
...
class OutsideCompilationV2Context(control_flow_ops.ControlFlowContext):
    ...
    def to_control_flow_context_def(self, context_def, export_scope=None):
        raise NotImplementedError(
            "to_control_flow_context_def not implemented")
    ...
    for core_idx, inputs_per_core in enumerate(inputs):
        for idx, input_tensor in enumerate(inputs_per_core):
            ...
            if core_idx == 0:
                ...
                need_padding.append(
                    np.full_like(input_shape, False, dtype=bool))
            else:
...
def _flatten_and_filter_composite(maybe_composite,
                                  non_composite_output,
...
def split_compile_and_replicate(
    ...
    if device_assignment is not None:
        ...
        metadata_kwargs[
            "num_cores_per_replica"] = device_assignment.num_cores_per_replica
    ...
    if (not isinstance(inputs, list)) or any(
            not isinstance(inp, (list, tuple)) for inp in inputs):
        raise TypeError(
    ...
    is_composite = nest.flatten(
        nest.map_structure(
            lambda x: _flatten_and_filter_composite(x, False, True),
            inputs[0]))
    ...
    if maximum_shapes:
        ...
        flat_maximum_shapes = nest.flatten([
            _flatten_and_filter_composite(x, y) for x, y in zip(
                nest.flatten(inputs[0]), nest.flatten(maximum_shapes))
        ])
        flat_maximum_shapes = [
            tensor_shape.TensorShape(s) if s is not None else None
            for s in flat_maximum_shapes
        ]
        nest.assert_same_structure(flat_inputs[0],
                                   flat_maximum_shapes,
                                   check_types=False)

        flat_inputs, padding_maps = _pad_all_input(flat_inputs,
                                                   flat_maximum_shapes,
                                                   padding_spec)
        if padding_maps:
    ...
    metadata_kwargs["step_marker_location"] = getattr(computation,
    ...
    for i in range(0, len(flat_inputs[0])):
        replicas = [
            flat_inputs[replica][i] for replica in xrange(num_replicas)
        ]
    ...
    try:
        ...
        if outputs_is_flat:
            ...
            output_tensors, control_deps = _postprocess_non_flat_outputs(
                outputs)
    ...
    for i, t in enumerate(output_tensors):
        # Fan-out: Builds a TPUReplicatedOutput node for each output.
        ys = tpu_ops.tpu_replicated_output(t,
                                           num_replicas,
...
def _postprocess_flat_outputs(outputs):
    ...
    for t in output_tensors:
        with ops.device(t.device if t.device else core(0)):
            ...
            # pylint: disable=protected-access
            o.op._set_attr("_tpu_output_identity",
                           attr_value_pb2.AttrValue(b=True))
            # pylint: enable=protected-access
