...
class DeviceAssignment(object):
    ...
    def __init__(self, topology, core_assignment):
        ...
        if not isinstance(topology, Topology):
            raise ValueError(
                "topology must be a Topology object, got {}".format(
                    type(topology)))
    ...
    def host_device(self, replica=0, logical_core=0, job=None):
        ...
        coordinates = self.coordinates(replica, logical_core)
        return self._topology.cpu_device_name_at_coordinates(coordinates,
                                                             job=job)

    def tpu_device(self, replica=0, logical_core=0, job=None):
        """Returns the name of the TPU device assigned to a logical core."""
        coordinates = self.coordinates(replica, logical_core)
        return self._topology.tpu_device_name_at_coordinates(coordinates,
                                                             job=job)
    ...
    def build(topology,
              ...
              num_replicas=1):
        return device_assignment(topology, computation_shape,
                                 computation_stride, num_replicas)
...
def _ring_3d(x_size, y_size, z_size):
    ...
    if ((x_size > 1 and x_size % 2 != 0) or (y_size > 1 and y_size % 2 != 0)
...
def device_assignment(topology,
    ...
    if computation_shape.shape != (topology_rank, ):
        raise ValueError(
            "computation_shape must have shape [{}]; got {}".format(
                topology_rank, computation_shape.shape))
    if computation_stride.shape != (topology_rank, ):
        raise ValueError(
            "computation_stride must have shape [{}]; got {}".format(
                topology_rank, computation_stride.shape))

    if any(computation_shape < 1):
        raise ValueError(
            "computation_shape must be positive; got computation_shape={}".
            format(computation_shape))
    if any(computation_stride < 1):
        raise ValueError(
            "computation_stride must be positive; got computation_stride={}".
            format(computation_stride))
    ...
    if any(computation_footprint > mesh_shape):
        raise ValueError(
            "computation footprint {} does not fit in TPU topology shape {}".
            format(computation_footprint, mesh_shape))
    ...
    if topology.missing_devices.size == 0:
        ...
        if num_replicas > 0:
            ...
            for x, ni in sorted(((x, ((i + 1) % topology_rank))
                ...
                remaining_replicas = ceil_of_ratio(remaining_replicas,
                                                   replica_shape[i])
                remaining_dims -= 1
        ...
        replica_offsets = np.full([num_replicas, topology_rank],
                                  -1,
                                  dtype=np.int32)

        enable_3d_tiling = (
            topology_rank == 4
            and computation_shape[-1] == 2  # Only handle 3D case.
        ...
        if enable_3d_tiling:
            ...
            for replica in xrange(num_replicas):
                ...
                for dim in np.concatenate([[replica_shape[-1]],
                                           replica_shape[:-1]]):
                    pos.append(t % dim)
                    t //= dim
                ...
                replica_offsets[
                    replica, :] = outer * computation_footprint + inner
            ...
            indices = np.concatenate(
                [
                    i[..., np.newaxis]
                    for i in np.meshgrid(*indices, indexing="ij")
                ],
                axis=-1,
            )
        ...
        assert (num_replicas * np.prod(computation_shape) <=
