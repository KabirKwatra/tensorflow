    ...
    if topology.missing_devices.size == 0:
        ...
        if num_replicas > 0:
            ...
            for x, ni in sorted(((x, ((i + 1) % topology_rank))
                ...
                remaining_replicas = ceil_of_ratio(
                    remaining_replicas, replica_shape[i])
                remaining_dims -= 1

            assert remaining_replicas == 1 and remaining_dims == 0

        # Assigns an offset to each replica such that no two replicas overlap.
        replica_offsets = np.full(
            [num_replicas, topology_rank], -1, dtype=np.int32)
        ...
        if enable_3d_tiling:
            ...
            for replica in xrange(num_replicas):
                ...
                replica_offsets[replica, :] = outer * \
                    computation_footprint + inner
