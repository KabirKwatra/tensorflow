...
class FunctionCallOptions(object):
    ...
    def config_proto_serialized(self, config):
        if isinstance(config, config_pb2.ConfigProto):
            ...
            self._config_proto_serialized = config_pb2.ConfigProto(
            ).SerializeToString()
        else:
            raise ValueError(
...
try:
    from tensorflow.python.framework.is_tfrt_test_true import (
        is_tfrt_enabled, )  # pylint: disable=g-import-not-at-top
...
class Context(object):
    ...
    def __init__(self,
        ...
        if execution_mode not in (None, SYNC, ASYNC):
            raise ValueError(
                "execution_mode should be None/SYNC/ASYNC. Got %s" %
                execution_mode)
        ...
        try:
            ...
            for i in range(pywrap_tfe.TF_DeviceListCount(device_list)):
                ...
                logical_devices.append(
                    LogicalDevice(name=spec.to_string(),
    ...
    def set_server_def(self, server_def, keep_alive_secs=_KEEP_ALIVE_SECS):
        ...
        if self._context_handle:
            ...
            pywrap_tfe.TFE_ContextSetServerDef(self._context_handle,
    ...
    def check_alive(self, worker_name):
        ...
        if self._context_handle:
            return pywrap_tfe.TFE_ContextCheckAlive(self._context_handle,
                                                    worker_name)
        else:
    ...
    def enable_collective_ops(self, server_def):
        ...
        if self._context_handle is not None:
            ...
            with self._initialize_lock:
                ...
                pywrap_tfe.TFE_EnableCollectiveOps(self._context_handle,
                                                   server_def_str)
                self._initialize_logical_devices()
                self._clear_caches()
    ...
    def configure_collective_ops(
        ...
        if self._collective_leader is not None:
            if (self._collective_leader != collective_leader
                    or self._collective_scoped_allocator_enabled_ops !=
                    scoped_allocator_enabled_ops
                    or self._collective_use_nccl_communication !=
    ...
    def __str__(self):
        if self._context_handle is None:
            ...
            lines = [
                "Eager TensorFlow Context with %d devices" % (len(devices))
            ]
    ...
    def host_address_space(self):
        ...
        with c_api_util.tf_buffer() as buffer_:
            ...
            address_space = pywrap_tf_session.TF_GetBuffer(buffer_).decode(
                "utf-8")
    ...
    def _initialize_physical_devices(self):
        ...
        with self._device_lock:
            ...
            self._memory_growth_map = {
                d: None
                for d in self._physical_devices if d.device_type == "GPU"
            }
    ...
    def list_physical_devices(self, device_type=None):
        ...
        return [
            d for d in self._physical_devices if d.device_type == device_type
        ]
    ...
    def _import_config(self):
        ...
        if num_cpus != 1:
            cpus = [
                d for d in self._physical_devices if d.device_type == "CPU"
            ]
            if num_cpus == 0:
                ...
                self.set_logical_device_configuration(
                    cpus[0],
                    [LogicalDeviceConfiguration() for _ in range(num_cpus)])
        ...
        if visible_indices:
            for index in visible_indices.split(","):
                if int(index) >= len(gpus):
                    raise ValueError("Invalid visible device index: %s" %
                                     index)
    ...
    def list_logical_devices(self, device_type=None):
        ...
        return [
            d for d in self._logical_devices if d.device_type == device_type
        ]
    ...
    def get_visible_devices(self, device_type=None):
        ...
        return [
            d for d in self._visible_device_list
    ...
    def optimizer_jit(self):
        ...
        return (level == config_pb2.OptimizerOptions.ON_1
    ...
    def use_tfrt(self, tfrt):
        ...
        if self._use_tfrt != tfrt:
            if self._initialized:
                raise ValueError(
                    "use_tfrt should be set before being initialized.")
            self._use_tfrt = tfrt
    ...
    def export_run_metadata(self):
        ...
        if not self._context_handle:
            return None
        with c_api_util.tf_buffer() as buffer_:
            pywrap_tfe.TFE_ContextExportRunMetadata(self._context_handle,
                                                    buffer_)
            proto_data = pywrap_tf_session.TF_GetBuffer(buffer_)
...
class _EagerDeviceContext(object):
    ...
    def __enter__(self):
        ...
        try:
            ...
            _device_parsing_cache[cache_key] = (new_device_name,
                                                new_device_spec)
