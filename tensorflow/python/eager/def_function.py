        ...
        with self._lock:
            ...
            if counter.get_tracing_count() >= FREQUENT_TRACING_WARNING_THRESHOLD:
                logging.warning(
                    ...
                    " more details.".format(
                        counter.get_tracing_count(), counter.call_count, function_name
                    )
                )
...
class Function(object):
    ...
    def __call__(self, *args, **kwds):
        ...
        if self._experimental_compile and (
            not control_flow_util.GraphOrParentsInXlaContext(ops.get_default_graph())
        ):
        ...
        if tracing_count == self._get_tracing_count():
            _frequent_tracing_detector.called_without_tracing(self._key_for_call_stats)
        else:
