...
class FunctionTest(test.TestCase, parameterized.TestCase):
    ...
    def testDecoratedMethod(self):
        ...
        instance_call_two = m.call(inputs=array_ops.ones([1, 2]),
                                   training=True)
        class_call = DefunnedMiniModel.call(m,
                                            array_ops.ones([1, 2]),
                                            training=True)
    ...
    def testDecoratedMethodGetConcreteFunction(self):
        m = DefunnedMiniModel()
        instance_call_one = m.call.get_concrete_function(array_ops.ones([1,
                                                                         2]),
                                                         training=False)
        instance_call_two = m.call.get_concrete_function(inputs=array_ops.ones(
            [1, 2]),
                                                         training=False)
        self.assertAllEqual(
            instance_call_one(array_ops.ones([1, 2])),
...
class MultiDeviceTest(test.TestCase, parameterized.TestCase):
    ...
    def testStandardTrainingLoopInFunction(self):
        ...
        dataset = (dataset_ops.DatasetV2.from_tensors(
            (array_ops.ones([784]), array_ops.ones(
                [],
                dtypes.int32))).map(lambda x, y: (x, y)).repeat(10).batch(32))
        optimizer = adam.Adam()
