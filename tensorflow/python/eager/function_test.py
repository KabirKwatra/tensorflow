...
class FunctionTest(test.TestCase, parameterized.TestCase):
        ...
        with context.graph_mode(), self.cached_session():
            ...
            for f in fdefs:
                ...
                if 'forward' in name or 'backward' in name:
                    not_present += 1
                    self.assertNotIn(
                        function.IMPLEMENTS_ATTRIBUTE_NAME, f.attr, f)
                else:
                    present += 1
    ...
    def testDefunShapeInferenceWithCapturedResourceVariable(self):
        ...
        var_t = resource_variable_ops.read_variable_op(
    ...
    def testFuncListAttr(self):
        ...
        def test_function(val):
            ...
            def fn2(): return array_ops.ones([10]) * 2
    ...
    def testCacheTensorUnknownShapesCollisionRelaxedShapes(self):
        ...
        with context.graph_mode(), self.cached_session():
            ...
            self.assertEqual(tensor_shape.dimension_value(
                relaxed_shape[0]), None)
    ...
    def testRegisterConcreteFunction(self, function_decorator):
        ...
        with context.graph_mode(), self.cached_session():
            with ops.get_default_graph().as_default():
                ...
                self.assertAllEqual(
                    [[2, 4], [6, 8]], self.evaluate(composite_double))
                self.assertAllEqual(
                    [[1, 2], [3, 4]], self.evaluate(composite_t))
                # Make sure the pre registered function is used, and no other function
                # is added.
                self.assertLen(graph._functions, 6)
    ...
    def testShapeCaching(self):
        ...
        self.assertAllEqual([1, 1], self.evaluate(
            func(array_ops.zeros([1, 1]))))
        self.assertAllEqual([2, 2], self.evaluate(
            func(array_ops.zeros([2, 2]))))
        self.assertAllEqual(
            [3, 3],
    ...
    def testClearFunctionCallbacks(self):
        ...
        function.clear_function_callbacks()
        self.assertEmpty(
            function._function_callbacks)  # pylint:disable=protected-access
    ...
    def testPrettyPrintedSignature(self):
        ...
        self.assertEqual(c4.pretty_printed_signature(
            verbose=False), c4_summary)

        c5 = func2.get_concrete_function(8, vector)
        c5_summary = 'func2(x=8, y)'
        self.assertEqual(c5.pretty_printed_signature(
            verbose=False), c5_summary)
