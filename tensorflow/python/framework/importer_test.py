...
class ImportGraphDefTest(test.TestCase):
    ...
    """)
        ...
        with ops.Graph().as_default():
            # Initial import
            a, b = importer.import_graph_def(graph_def,
                                             return_elements=["A", "B"],
                                             name="")
            ...
            a1, b1 = importer.import_graph_def(graph_def,
                                               return_elements=["A", "B"],
                                               name="")
            ...
            a2, b2 = importer.import_graph_def(graph_def,
                                               return_elements=["A", "B"],
                                               name="")
            ...
            a3, b3 = importer.import_graph_def(graph_def,
                                               return_elements=["A", "B"],
                                               name="A")
            ...
            a_a, a_b = importer.import_graph_def(graph_def,
                                                 return_elements=["A", "B"],
                                                 name="A/")
            ...
            # Repeat the same import.
            a_a1, a_b1 = importer.import_graph_def(graph_def,
                                                   return_elements=["A", "B"],
                                                   name="A/")
          ...
          """),
            ...
            outer, inner, new_c, outer_inner, outer_inner_c = importer.import_graph_def(
                ...
                return_elements=[
                    "outer", "inner", "c", "outer/inner", "outer/inner/c"
                ],
                name="",
            )
    ...
    def testResources(self):
        ...
        with ops.Graph().as_default():
            ...
            (
                ...
                graph_def,
                return_elements=[
                    var.name, var_assign.name, var_shape.name, init.name
                ],
            )
    ...
    def testWhileLoop(self):
        ...
        graph = ops.Graph()
        with graph.as_default():
            r = control_flow_ops.while_loop(lambda i: i < 10, lambda i: i + 1,
                                            [0])
    ...
    def testImportWhileLoopInCond(self):
        ...
        with ops.Graph().as_default():
            ...
            def ImportFn():
                return importer.import_graph_def(graph_def,
                                                 return_elements=[r.name])[0]

            pred = array_ops.placeholder(dtypes.bool)
            out = control_flow_ops.cond(
                pred, ImportFn, lambda: constant_op.constant(1))
            with self.cached_session() as sess:
    ...
    def testShapeWhitelistViolation(self):
        ...
        with ops.Graph().as_default():
            ...
            """),
                ...
                self.assertTrue("Shapes () and (43,) are not compatible" in
                                str(e.exception))
    ...
    def testMissingReturnOperation(self):
        with ops.Graph().as_default():
            with self.assertRaisesRegexp(
                    ValueError,
                    "Requested return node 'B' not found in graph def"):
                importer.import_graph_def(
                    self._MakeGraphDef("""
            node { name: 'A' op: 'None' }
            """),
    ...
    def testMissingReturnTensor(self):
        with ops.Graph().as_default():
            ...
            with self.assertRaisesRegexp(
                    ValueError,
                    "Requested return tensor 'B:0' not found in graph def"):
                importer.import_graph_def(
                    self._MakeGraphDef("""
            node { name: 'A' op: 'IntOutput' }
            """),
    ...
    def testNamePrefixColocationAttrs(self):
        ...
        with ops.Graph().as_default():
            ...
            self.assertProtoEquals("list { s: 'loc:@imported_graph/A' }",
    ...
    def testNamePrefixColocationAttrsMultipleImport(self):
        ...
        with ops.Graph().as_default():
            a, b = importer.import_graph_def(original_graph_def,
                                             return_elements=["A", "B"],
                                             name="")
            a_1, b_1 = importer.import_graph_def(original_graph_def,
                                                 return_elements=["A", "B"],
                                                 name="")
    ...
    def testNamePrefixColocationAttrsNotFound(self):
        ...
        with ops.Graph().as_default():
            with self.assertRaisesRegexp(
                    ValueError,
                    "Node 'B' expects to be colocated with unknown node 'A'"):
                importer.import_graph_def(original_graph_def,
                                          return_elements=["B"],
                                          name="imported_graph")
    ...
    def testInvalidInputForGraphDef(self):
        with ops.Graph().as_default():
            with self.assertRaises(TypeError) as e:
                importer.import_graph_def("")
            self.assertEqual("graph_def must be a GraphDef proto.",
                             str(e.exception))
    ...
    def testInvalidInputForInputMap(self):
        with ops.Graph().as_default():
            with self.assertRaises(TypeError) as e:
                importer.import_graph_def(
                    self._MakeGraphDef(""),
                    input_map=[constant_op.constant(5.0)])
            self.assertEqual(
    ...
    def testWithExtensionAndAttr(self):
        ...
        with self.cached_session():
            (pack, ) = importer.import_graph_def(gdef,
                                                 return_elements=["pack"])
        ...
        for producer in v0, v1, v2:
            for min_consumer in v0, v1, v2:
                with ops.Graph().as_default():
                    ...
                    self.assertEqual(a.graph.graph_def_versions.producer,
                                     producer)
    ...
    def testVersionHigh(self):
        with ops.Graph().as_default():
            with self.assertRaisesRegexp(
                ...
                importer.import_graph_def(
                    self._MakeGraphDef("", min_consumer=1 << 30))
    ...
    """,
        ...
        with ops.Graph().as_default():
          ...
          """),
            ...
            with self.assertRaisesRegexp(
                    ValueError,
                    "Operation 'import/A' has no attr named 'default_int'."):
    ...
    def testImportGraphWithFunctionTwice(self):
        ...
        with ops.name_scope("first"):
            z1 = importer.import_graph_def(gdef,
                                           return_elements=["z:0"],
                                           input_map=input_map)[0]

        with ops.name_scope("second"):
            z2 = importer.import_graph_def(gdef,
                                           return_elements=["z:0"],
                                           input_map=input_map)[0]
