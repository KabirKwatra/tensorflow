...
class ImportGraphDefTest(test.TestCase):
    ...
    def testInvalidInputForReturnOperations(self):
        with ops.Graph().as_default():
            with self.assertRaisesRegexp(
                ...
                importer.import_graph_def(
                    self._MakeGraphDef(""), return_elements=[7])
    ...
    def testGradient(self):
        with ops.Graph().as_default() as g:
            ...
            biases = array_ops.placeholder(
                dtypes.float32, shape=[10], name="biases")
        ...
        with ops.Graph().as_default() as g:
            input_placeholder = array_ops.placeholder(
                dtypes.float32, shape=[32, 100])
            weights_var = variables.Variable(
                random_ops.truncated_normal([100, 10]), name="weights")
            biases_var = variables.Variable(
                array_ops.zeros([10]), name="biases")
            ...
            for min_consumer in v0, v1, v2:
                with ops.Graph().as_default():
                    a, = importer.import_graph_def(
                        ...
                        return_elements=["A"])
                    self.assertEqual(
                        a.graph.graph_def_versions.producer, producer)
    ...
    def testFunctions(self):
        ...
        with ops.Graph().as_default() as g2:
            ...
            with self.session(graph=g2) as sess:
                feed_dict = {p1: 1, p2: 2}
                a_val, b_val, grad_val = sess.run(
                    [a, b, grad], feed_dict=feed_dict)
