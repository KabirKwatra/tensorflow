...
from tensorflow.core.framework import (attr_value_pb2, function_pb2, graph_pb2,
                                       node_def_pb2, op_def_pb2, versions_pb2)
...
from tensorflow.python import pywrap_tensorflow, pywrap_tfe, tf2
from tensorflow.python.client import pywrap_tf_session
from tensorflow.python.eager import context, core, monitoring, tape
from tensorflow.python.framework import c_api_util, composite_tensor
from tensorflow.python.framework import device as pydev
from tensorflow.python.framework import (dtypes, errors, indexed_slices,
                                         registry, tensor_conversion_registry,
                                         tensor_shape, traceable_stack,
                                         versions)
from tensorflow.python.ops import control_flow_util
from tensorflow.python.platform import app
from tensorflow.python.platform import tf_logging as logging
from tensorflow.python.types import internal
from tensorflow.python.util import (compat, decorator_utils, deprecation,
                                    function_utils, lock_util, memory,
                                    object_identity, tf_contextlib, tf_stack)
from tensorflow.python.util.compat import collections_abc
from tensorflow.python.util.deprecation import deprecated_args
from tensorflow.python.util.lazy_loader import LazyLoader
from tensorflow.python.util.tf_export import kwarg_only, tf_export
...
def register_dense_tensor_like_type(tensor_type):
    ...
    """
    if not (hasattr(tensor_type, "name") and isinstance(tensor_type.name, property)):
        raise TypeError(
            "Type %s does not define a `name` property" % tensor_type.__name__
        )
    if not (hasattr(tensor_type, "dtype") and isinstance(tensor_type.dtype, property)):
        raise TypeError(
            "Type %s does not define a `dtype` property" % tensor_type.__name__
        )
    if not (hasattr(tensor_type, "shape") and isinstance(tensor_type.shape, property)):
        raise TypeError(
            "Type %s does not define a `shape` property" % tensor_type.__name__
        )
...
class name_scope_v2(object):
    ...
    def __enter__(self):
        ...
        if ctx.executing_eagerly():
            ...
            self._exit_fns.append(lambda *a: setattr(ctx, "scope_name", old_scope_name))
        else:
