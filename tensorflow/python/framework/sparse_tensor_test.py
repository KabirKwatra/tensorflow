...
class SparseTensorTest(test_util.TensorFlowTestCase):
    def testPythonConstruction(self):
        ...
        for sp in [
            ...
            with self.cached_session() as sess:
                ...
                self.assertAllEqual(sess_run_value.dense_shape,
                                    value.dense_shape)
    ...
    def testIsSparse(self):
        ...
        self.assertTrue(
            sparse_tensor.is_sparse(sparse_tensor.SparseTensor([[0]], [0],
                                                               [1])))
        self.assertTrue(
    ...
    def testConsumers(self):
        with context.graph_mode():
            sp = sparse_tensor.SparseTensor([[0, 0], [1, 2]], [1.0, 3.0],
                                            [3, 4])
...
class ConvertToTensorOrSparseTensorTest(test_util.TensorFlowTestCase):
    ...
    def test_convert_sparse(self):
        with self.cached_session():
            ...
            from_tensor = sparse_tensor.convert_to_tensor_or_sparse_tensor(
                st).eval()
            for convertee in [from_value, from_tensor]:
                self.assertAllEqual(sparse_tensor_value.indices,
                                    convertee.indices)
                self.assertAllEqual(sparse_tensor_value.values,
                                    convertee.values)
                self.assertAllEqual(sparse_tensor_value.dense_shape,
                                    convertee.dense_shape)
...
class SparseTensorShapeTest(test_util.TensorFlowTestCase):
    ...
    def test_unknown_shape(self):
        ...
        my_func.get_concrete_function(
            dense_shape=tensor_spec.TensorSpec(dtype=dtypes.int64, shape=[
                2,
            ]))
    ...
    def test_unknown_rank(self):
        ...
        my_func.get_concrete_function(dense_shape=tensor_spec.TensorSpec(
            dtype=dtypes.int64, shape=[None]))
