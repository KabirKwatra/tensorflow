...
def ExtractBitsFromBFloat16(x):
    return np.asarray(x, dtype=dtypes.bfloat16.as_numpy_dtype).view(
        np.uint16).item()
...
def FastAppendBFloat16ArrayToTensorProto(tensor_proto, proto_values):
    fast_tensor_util.AppendBFloat16ArrayToTensorProto(
        ...
        np.asarray(proto_values,
                   dtype=dtypes.bfloat16.as_numpy_dtype).view(np.uint16),
    )
...
def _generate_isinstance_check(expected_types):
    def inner(values):
        _ = [
            _check_failed(v) for v in nest.flatten(values)
...
def _AssertCompatible(values, dtype):
    ...
    try:
        ...
        if dtype is None:
            raise TypeError(
                "Expected any non-tensor type, got a tensor instead.")
        else:
            raise TypeError(
...
def _is_array_like(obj):  # pylint: disable=invalid-name
    ...
    if isinstance(obj,
                  ops.Tensor) and not isinstance(obj, ops._EagerTensorBase):  # pylint: disable=protected-access
...
def make_tensor_proto(values,
    ...
    if isinstance(values, (np.ndarray, np.generic)):
        ...
        if shape is not None and np.prod(shape, dtype=np.int64) == 0:
            ...
            if list(nparray.shape) != _GetDenseDimensions(
                    values) and not is_quantized:
                raise ValueError(
...
def MakeNdarray(tensor):
    ...
    if tensor_dtype == dtypes.float16 or tensor_dtype == dtypes.bfloat16:
        ...
        values = np.array([complex(x[0], x[1]) for x in zip(it, it)],
                          dtype=dtype)
...
def _ConstantValue(tensor, partial):
    ...
    if not isinstance(tensor, ops.Tensor):
        raise TypeError("%r is not a Tensor, has type %s" %
                        (tensor, type(tensor)))
...
def is_tensor(x):  # pylint: disable=invalid-name
    ...
    """
    return (isinstance(x, (tensor_like.TensorLike, core.Tensor))
            or ops.is_dense_tensor_like(x)
...
def maybe_set_static_shape(tensor, shape):  # pylint: disable=invalid-name
    ...
    """
    if (_ENABLE_MAYBE_SET_STATIC_SHAPE and not context.executing_eagerly()
            and ops.get_default_graph().building_function
            and not tensor.shape.is_fully_defined() and is_tensor(shape)):
