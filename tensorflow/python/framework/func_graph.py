...
def convert_structure_to_signature(structure, arg_names=None):
    ...
    def encode_arg(arg, path):
        ...
        if isinstance(arg, resource_variable_ops.BaseResourceVariable):
            name = "/".join(str(p) for p in path)
            return resource_variable_ops.VariableSpec(arg.shape, arg.dtype,
                                                      name)
        if isinstance(
                arg,
            (
...
class FuncGraph(ops.Graph):
    ...
    def capture_call_time_value(self, closure, spec, key=None):
        ...
        if key not in self._deferred_captures:
            ...
            placeholder = nest.map_structure(convert_to_placeholder,
                                             spec,
                                             expand_composites=True)

            def wrapped_closure():
                ret_nest = closure()
                nest.assert_same_structure(spec,
                                           ret_nest,
                                           expand_composites=True)
    ...
    def control_dependencies(self, control_inputs):
        ...
        for c in control_inputs:
            ...
            if isinstance(c, ops.IndexedSlices) or (hasattr(c, "_handle")
                                                    and hasattr(c, "op")):
                c = c.op
    ...
    def as_default(self):
        ...
        def inner_cm():
            ...
            uses_distribution_strategy = (
                ...
                and self._distribution_strategy_stack[-1].strategy.extended.
            ...
            if context.executing_eagerly():
                ...
                if uses_distribution_strategy or device_stack_has_callable(
            ...
            with outer_cm as g:
                try:
                    ...
                    try:
                        ...
                        self._auto_cast_variable_read_dtype = (
                            old_auto_cast_var_read_dtype)
    ...
    def capture(self, tensor, name=None, shape=None):
        ...
        if isinstance(tensor, ops.EagerTensor):
            ...
            if (tensor.dtype in dtypes.TF_VALUE_DTYPES
        ...
        if tensor.graph is not self:
            ...
            while inner_graph is not None and isinstance(
                    inner_graph, FuncGraph):
    ...
    def capture_eager_tensor(self, tensor, name):
        ...
        if capture is None:
            ...
            with ops.control_dependencies(None), self.device(tensor.device):
                graph_const = constant_op.constant(tensor.numpy(),
                                                   dtype=tensor.dtype,
                                                   shape=tensor.shape,
                                                   name=name)
    ...
    def variable_captures(self):
        ...
        return {
            ...
            for v in self.variables if id(v) in self._captures
        }
...
def func_graph_from_py_func(
    ...
    with func_graph.as_default(), deps_control_manager as deps_ctx:
        ...
        try:
            if autograph:
                from tensorflow.python import (
                    autograph, )  # pylint: disable=g-import-not-at-top
        ...
        inputs = []
        for arg in nest.flatten(func_args,
                                expand_composites=True) + nest.flatten(
    ...
    if add_control_dependencies:
        func_graph.control_outputs.extend(
            deps_control_manager.ops_which_must_run)
...
def maybe_captured(tensor):
    ...
    """
    if (not isinstance(tensor, ops.EagerTensor)
...
def _create_substitute_placeholder(value, name=None, dtype=None, shape=None):
    ...
    with ops.control_dependencies(None):
        placeholder = graph_placeholder(dtype=dtype or value.dtype,
                                        shape=shape,
                                        name=name)
            ...
            if isinstance(arg, (ops.Tensor, tensor_spec.TensorSpec)):
                ...
                if isinstance(arg, resource_variable_ops.VariableSpec):
                    ...
                    with func_graph.outer_graph.as_default():
                        placeholder = graph_placeholder(dtypes.resource,
                                                        arg.shape,
                                                        name=name)
