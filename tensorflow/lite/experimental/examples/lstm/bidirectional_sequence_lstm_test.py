...
class BidirectionalSequenceLstmTest(test_util.TensorFlowTestCase):
    ...
    def buildLstmLayer(self):
        return tf.keras.layers.StackedRNNCells([
            tf.compat.v1.lite.experimental.nn.TFLiteLSTMCell(
                self.num_units, use_peepholes=True, forget_bias=0,
                name="rnn1"),
            tf.compat.v1.lite.experimental.nn.TFLiteLSTMCell(self.num_units,
                                                             num_proj=8,
                                                             forget_bias=0,
                                                             name="rnn2"),
            tf.compat.v1.lite.experimental.nn.TFLiteLSTMCell(
    ...
    def saveAndRestoreModel(self, fw_lstm_layer, bw_lstm_layer, sess, saver,
        ...
        # Reset the graph.
        tf.compat.v1.reset_default_graph()
        x, prediction, output_class = self.buildModel(fw_lstm_layer,
                                                      bw_lstm_layer,
                                                      is_dynamic_rnn)
