...
class HTMLGenerator(object):
    ...
    def generate(
        ...
        html_dict["<!--CONVERSION_STATUS-->"] = (
            (r'<span class="label label-danger">Fail</span>') if toco_err_log
        ...
        for i in range(len(toco_conversion_log_before.op_list)):
            ...
            pre_op_profile += ("<tr><td>" +
                               toco_conversion_log_before.op_list[i] + "</td>")
        ...
        for op in toco_conversion_log_after.op_list:
            ...
            post_op_profile += ("<tr><td>" + op + "</td><td>" +
                                supported_type + "</td></tr>")
        ...
        for marker in html_dict:
            ...
            if template.find(marker) != -1:
                raise RuntimeError("Could not populate marker text %r" %
                                   marker)
...
def gen_conversion_log_html(conversion_log_dir, quantization_enabled,
    ...
    toco_log_before_path = os.path.join(conversion_log_dir,
                                        "toco_log_before.pb")
