...
class FromConcreteFunctionTest(lite_v2_test_util.ModelTest):
    ...
    def _getCalibrationQuantizeModel(self):
        ...
        @tf.function(input_signature=[
            tf.TensorSpec(shape=[1, 5, 5, 3], dtype=tf.float32)
        ])
        def func(inp):
    ...
    def _createV1SavedModel(self, shape):
        ...
        with tf.Graph().as_default():
            with tf.compat.v1.Session() as sess:
                in_tensor_1 = tf.compat.v1.placeholder(shape=shape,
                                                       dtype=tf.float32,
                                                       name="inputB")
                in_tensor_2 = tf.compat.v1.placeholder(shape=shape,
                                                       dtype=tf.float32,
                                                       name="inputA")
...
class FromKerasModelTest(lite_v2_test_util.ModelTest):
    ...
    def testSequentialMultiInputOutputModel(self):
        ...
        merged = tf.keras.layers.concatenate([interm_a, interm_b],
                                             name="merge")

        output_c = tf.keras.layers.Dense(3,
                                         activation="softmax",
                                         name="dense_2")(merged)
        output_d = tf.keras.layers.Dense(2,
                                         activation="softmax",
                                         name="dense_3")(merged)
...
class ControlFlowTest(lite_v2_test_util.ModelTest):
    ...
    def testStaticRnn(self):
        ...
        def model(x):
            seq = tf.split(x, 3, 0)
            return tf.compat.v1.nn.static_rnn(cell,
                                              seq,
                                              dtype=tf.float32,
                                              sequence_length=[1])
    ...
    def testDynamicRnn(self):
        ...
        @tf.function(input_signature=[
            tf.TensorSpec(shape=[3, 10, 10], dtype=tf.float32)
        ])
        def model(x):
    ...
    def testKerasBidirectionalRNN(self):
        ...
        model = tf.keras.models.Sequential()
        model.add(
            tf.keras.layers.Bidirectional(recurrent_v2.LSTM(
