...
class FromConcreteFunctionTest(TestModels):
    ...
    def testTrainingTimeQuantizeConversion(self):
        ...
        interpreter = Interpreter(model_content=quantized_tflite)
        self.assertEqual(
            np.float32, interpreter.get_input_details()[0]['dtype'])
    ...
    def testEmbeddings(self, enable_mlir):
        ...
        np.testing.assert_almost_equal(
            expected_value.numpy(), actual_value[0], 5)
...
class FromSavedModelTest(TestModels):
    ...
    def testV1SimpleModel(self):
        ...
        with context.graph_mode():
            ...
            self.assertEqual(np.float32, input_details[0]['dtype'])
            self.assertTrue(
                ([1, 16, 16, 3] == input_details[0]['shape']).all())
            self.assertEqual((0., 0.), input_details[0]['quantization'])

            self.assertEqual('inputB', input_details[1]['name'])
            self.assertEqual(np.float32, input_details[1]['dtype'])
            self.assertTrue(
                ([1, 16, 16, 3] == input_details[1]['shape']).all())
            ...
            self.assertEqual(np.float32, output_details[0]['dtype'])
            self.assertTrue(
                ([1, 16, 16, 3] == output_details[0]['shape']).all())
...
class FromKerasModelTest(TestModels):
    ...
    def testSequentialMultiInputOutputModel(self):
        ...
        model.compile(optimizer='sgd', loss='mean_squared_error')
        model.fit([input_a_np, input_b_np], [
                  output_c_np, output_d_np], epochs=1)
...
class GrapplerTest(TestModels):
    ...
    def testConstantFolding(self):
        ...
        actual_value = self._evaluateTFLiteModel(
            hybrid_tflite_model, [input_data])
        np.testing.assert_almost_equal(expected_value.numpy(), actual_value[0])
    ...
    def testBatchMatMul(self):
        ...
        @def_function.function(input_signature=[
            tensor_spec.TensorSpec(
                shape=[None, 256, 256], dtype=dtypes.float32),
            tensor_spec.TensorSpec(
                shape=[None, 256, 256], dtype=dtypes.float32)
        ])
