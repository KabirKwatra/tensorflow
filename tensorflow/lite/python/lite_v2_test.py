...
class FromConcreteFunctionTest(TestModels):
    ...
    def testScalarInput(self):
        root = self._getSimpleVariableModel()
        input_data = constant_op.constant(1.0, shape=[])
        concrete_func = root.f.get_concrete_function(input_data)
...
class FromSavedModelTest(TestModels):
    ...
    def testMultipleFunctionModel(self):
        ...
        with self.assertRaises(ValueError) as error:
            _ = converter.convert()
        self.assertIn(
            "This converter can only convert a single ConcreteFunction",
            str(error.exception),
        )
    ...
    def testKerasSequentialModel(self):
        ...
        input_data = constant_op.constant(1.0, shape=[1, 1])
...
class FromKerasModelTest(TestModels):
    ...
    def testSequentialMultiInputOutputModel(self):
        ...
        left_input_data = constant_op.constant(1.0, shape=[1, 3])
        right_input_data = constant_op.constant(1.0, shape=[1, 3])
        ...
        model.compile(optimizer="sgd", loss="mean_squared_error")
        model.fit([input_a_np, input_b_np], [output_c_np, output_d_np],
                  epochs=1)
...
class ControlFlowTest(TestModels):
    ...
    def testCond(self):
        ...
        def model(x, b):
            return control_flow_ops.cond(b,
    ...
    def testWhileLoop(self):
        input_data = constant_op.constant([1.0, 2.0, 3.0, 4.0], shape=[2, 2])
    ...
    def testKerasBidirectionalRNN(self):
        ...
        model = keras.models.Sequential()
        model.add(
            keras.layers.Bidirectional(recurrent_v2.LSTM(
...
class GrapplerTest(TestModels):
    ...
    def testConstantFolding(self):
        ...
        actual_value = self._evaluateTFLiteModel(hybrid_tflite_model,
                                                 [input_data])
        np.testing.assert_almost_equal(expected_value.numpy(), actual_value[0])
...
class UnknownShapes(TestModels):
    ...
    def testMatMul(self):
        ...
        def model(in_tensor):
            ...
            fill = array_ops.transpose_v2(array_ops.fill(shape, 1.0))
        ...
        actual_value = self._evaluateTFLiteModel(tflite_model, [input_data],
                                                 input_shapes=[([-1,
                                                                 4], [10, 4])])
        np.testing.assert_almost_equal(expected_value.numpy(),
                                       actual_value[0],
                                       decimal=6)
    ...
    def testBatchMatMul(self):
        ...
        @def_function.function(input_signature=[
            tensor_spec.TensorSpec(shape=[None, 256, 256],
                                   dtype=dtypes.float32),
            tensor_spec.TensorSpec(shape=[None, 256, 256],
                                   dtype=dtypes.float32),
        ])
