...
class FromConstructor(TestModels):
    ...
    def testValidConstructor(self):
        ...
        converter = lite.TFLiteConverter(
            None, ['input_tensor'], ['output_tensor'])
...
class FromSessionTest(TestModels, parameterized.TestCase):
    ...
    def testPostTrainingQuantizeDeprecatedAttribute(self, enable_mlir):
        with ops.Graph().as_default():
            ...
            in_tensor_2 = constant_op.constant(
                ...
                name='inputB')
            out_tensor = math_ops.matmul(
                in_tensor_1, in_tensor_2, name='output')
            sess = session.Session()
    ...
    def testQuantizeFloat16(self, use_rep_data, include_int8,
        ...
        interpreter.allocate_tensors()
        self.assertEqual(interpreter.get_tensor_details()
                         [idx]['name'], node_name)
    ...
    def testMultipleOutputNodeNames(self):
        ...
        self.assertEqual(1.0, interpreter.get_tensor(
            output_details[0]['index']))
        self.assertEqual(2.0, interpreter.get_tensor(
            output_details[1]['index']))
        self.assertEqual(3.0, interpreter.get_tensor(
            output_details[2]['index']))
        self.assertEqual(4.0, interpreter.get_tensor(
    ...
    def testResizingIntermediateDynamicTensor(self):
        ...
        with ops.Graph().as_default():
            input_tensor = array_ops.placeholder(
                shape=[1, 1], dtype=dtypes.float32)
            input2_tensor = array_ops.placeholder(
...
class FromSavedModelTest(TestModels):
    ...
    def _createSavedModel(self, shape):
        ...
        saved_model_dir = os.path.join(
            self.get_temp_dir(), 'simple_savedmodel')
        with ops.Graph().as_default():
...
class GrapplerTest(TestModels, parameterized.TestCase):
    ...
    def testGrapplerConstFolding(self):
        ...
        with ops.Graph().as_default():
            in_tensor = array_ops.placeholder(
                shape=[2, 2], dtype=dtypes.float32)
