...
def _parse_array(values, type_fn=str):
    if values is not None:
        return [
            type_fn(val) for val in six.ensure_str(values).split(",") if val
        ]
    return None
...
def _parse_set(values):
    if values is not None:
        return set(
            [item for item in six.ensure_str(values).split(",") if item])
    return None
...
def _check_tf1_flags(flags, unparsed):
    ...
    if unparsed:
        ...
        for flag in unparsed:
            output += _get_message_unparsed(flag, "--input_file",
                                            "--graph_def_file")
            output += _get_message_unparsed(flag, "--savedmodel_directory",
                                            "--saved_model_dir")
            output += _get_message_unparsed(flag, "--std_value",
                                            "--std_dev_values")
            output += _get_message_unparsed(flag, "--batch_size",
                                            "--input_shapes")
    ...
    if flags.graph_def_file and (not flags.input_arrays
                                 or not flags.output_arrays):
        raise ValueError(
            "--input_arrays and --output_arrays are required with "
            "--graph_def_file")
    ...
    if flags.std_dev_values or flags.mean_values:
        ...
        if flags.std_dev_values.count(",") != flags.mean_values.count(","):
            raise ValueError(
                "--std_dev_values, --mean_values must have the same "
                "number of items")

    if (flags.default_ranges_min is None) != (flags.default_ranges_max is
                                              None):
        raise ValueError(
            "--default_ranges_min and --default_ranges_max must be "
            "used together")
    ...
    if flags.custom_opdefs and not flags.allow_custom_ops:
        raise ValueError(
            "--custom_opdefs must be used with --allow_custom_ops")
...
class _ParseExperimentalNewConverter(argparse.Action):
    ...
    def __init__(self, option_strings, dest, nargs=None, **kwargs):
        ...
        super(_ParseExperimentalNewConverter, self).__init__(option_strings,
                                                             dest,
                                                             nargs=nargs,
                                                             **kwargs)
