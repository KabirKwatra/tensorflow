    ...
    for node in nodes:
        ...
        for a in attr:
            if a.startswith("_tflite_attr_"):
                call_def.params[a.replace(
                    "_tflite_attr_,", "")] = attr[a].tensor
...
def _find_children_hints(call, graph_def):
    ...
    reachable_by_input = _bfs_for_reachable_nodes(
        input_names, name_to_input_name)
