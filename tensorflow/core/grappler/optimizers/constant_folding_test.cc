...
TEST_F(ConstantFoldingTest, ConstantPushDown) {
    for (int is_add : {
        ...
        for (int is_parent_commutative : {
                    true, false
                }) {
            for (int is_child_commutative : {
                        true, false
                    }) {
                for (int is_left_child_const : {
                            true, false
                        }) {
                    for (int is_left_leaf_const : {
                                true, false
                            }) {
                        tensorflow::Scope s = tensorflow::Scope::NewRootScope();
                        Output c2 = ops::Const(s.WithOpName("c2"), 2.0f, {2});
...
TEST_F(ConstantFoldingTest, NeutralElement_PartialShape_UnknownOutputShape) {
    ...
    for (const auto* x : {
                &x_known, &x_partially_known, &x_unknown
            }) {
...
TEST_F(ConstantFoldingTest, NeutralElement_PartialShape_KnownOutputShape) {
    ...
    int count = 0;
    for (const auto& x : {
                x_partially_known, x_unknown
            }) {
        for (const auto& zeros : {
                    zeros_partially_known, zeros_unknown
                }) {
            const string name = strings::StrCat("mul_", count++);
...
TEST_F(ConstantFoldingCastConstTest, CastConstFolding) {
    for (bool fetch_const : {
                false, true
            }) {
        for (bool fetch_cast : {
                    false, true
                }) {
            for (bool fetch_const_child : {
                        false, true
                    }) {
                for (bool fetch_cast_child : {
                            false, true
                        }) {
