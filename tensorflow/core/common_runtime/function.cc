...
public:
    ...
    bool ColocateInputOutputIdentities() const override {
        return false;
    }
...
Status InlineFunctionBody(const FunctionLibraryDefinition& flib_def, Graph* g,
    ...
    int index) -> Node* {
        ...
        if (colocate_identity) {
            ...
            std::vector<string> {
                absl::StrCat(kColocationGroupPrefix,
                             input.node->name())
            });
        }
        return node;
    ...
    caller->out_edges(), [](const Edge* e) {
        return e->IsControlEdge();
    });
    ...
    if (keep_caller_node) {
        ...
        [](Node* n) {
            return NodeBuilder::NodeOut(n, 0);
        });
