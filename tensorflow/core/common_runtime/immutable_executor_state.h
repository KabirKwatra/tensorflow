...
public:
    ...
    const LocalExecutorParams& params() const {
        return params_;
    }
    ...
    const std::vector<const NodeItem*>& root_nodes() const {
        return root_nodes_;
    }
    ...
    const FrameInfo& get_root_frame_info() const {
        return *root_frame_info_;
    }

    bool requires_control_flow_support() const {
        return requires_control_flow_;
    }
