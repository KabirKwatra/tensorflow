...
Status MasterSession::ReffedClientGraph::RunPartitionsHelper(
    ...
    const bool success =
    cm->RegisterCallback(token, [&calls]() {
        calls.StartCancel();
    });
    if (!success) {
        calls.StartCancel();
...
void BuildBuildGraphOptions(const RunStepRequestWrapper& req,
    ...
    CopyAndSortStrings(
    req.num_feeds(), [&req](size_t i) {
        return req.feed_name(i);
    },
    callable_opts->mutable_feed());
    CopyAndSortStrings(
    req.num_fetches(), [&req](size_t i) {
        return req.fetch_name(i);
    },
    callable_opts->mutable_fetch());
    CopyAndSortStrings(
    req.num_targets(), [&req](size_t i) {
        return req.target_name(i);
    },
...
void BuildBuildGraphOptions(const PartialRunSetupRequest& req,
    ...
    CallableOptions* callable_opts = &opts->callable_options;
    CopyAndSortStrings(
    req.feed_size(), [&req](size_t i) {
        return req.feed(i);
    },
    callable_opts->mutable_feed());
    CopyAndSortStrings(
    req.fetch_size(), [&req](size_t i) {
        return req.fetch(i);
    },
    callable_opts->mutable_fetch());
    CopyAndSortStrings(
    req.target_size(), [&req](size_t i) {
        return req.target(i);
    },
    callable_opts->mutable_target());
