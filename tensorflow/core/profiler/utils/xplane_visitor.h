...
public:
    ...
    absl::optional<int64> Type() const {
        return type_;
    }
    ...
    absl::string_view StrValue() const {
...
public:
    ...
    absl::string_view Metadata() const {
        return metadata_->metadata();
    }

    double OffsetNs() const {
        return PicosToNanos(event_->offset_ps());
    }

    int64 OffsetPs() const {
        return event_->offset_ps();
    }

    int64 LineTimestampNs() const {
    ...
    double DurationNs() const {
        return PicosToNanos(event_->duration_ps());
    }

    int64 DurationPs() const {
